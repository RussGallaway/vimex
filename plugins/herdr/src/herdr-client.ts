...
export function detectHerdr(
  ...
  return env.HERDR_ENV === "1" && env.HERDR_PANE_ID
    ? {
        ...
        ...(env.HERDR_BIN_PATH ? { binPath: env.HERDR_BIN_PATH } : {}),
      }
    : undefined
}
...
export function createHerdrRunner(
  ...
  return (args) =>
    new Promise((resolve, reject) => {
      ...
      const timeoutError = () =>
        new Error(`Herdr request timed out after ${timeoutMs}ms`)
      ...
      child.once("exit", (code, signal) => {
        ...
        code === 0
          ...
          : finish(
              new Error(
                errors ||
                  `Herdr exited ${String(code)}${signal ? ` (${signal})` : ""}`,
              ),
            )
      })
