 ...
 */
export function createHerdrExternalActions(options: HerdrExternalActionOptions = {}): HerdrExternalActions {
  ...
  return {
    async openUrl(target: string) {
      ...
      const config = configDir ? await readExternalActionConfig(configDir) : undefined
...
export async function readExternalActionConfig(configDir: string): Promise<HerdrExternalActionConfig | undefined> {
  let source: string
  try { source = await readFile(join(configDir, "external-actions.json"), "utf8") }
  catch (error) {
    if (isMissing(error)) return undefined
    throw error
  }
  const value: unknown = JSON.parse(source)
  if (!isRecord(value)) throw new Error("Herdr external-actions.json must contain an object")
  const unknown = Object.keys(value).filter(key => key !== "openUrl")
  if (unknown.length) throw new Error(`Unknown Herdr external action setting: ${unknown[0]}`)
  if (value.openUrl === undefined) return {}
  if (!isRecord(value.openUrl) || !Array.isArray(value.openUrl.command)
    || value.openUrl.command.length === 0 || !value.openUrl.command.every(item => typeof item === "string" && item.length > 0)) {
    throw new Error("Herdr external action openUrl.command must be a non-empty argv string array")
  }
  return { openUrl: { command: value.openUrl.command as string[] } }
}

export async function openPlatformUrl(target: string, platform: NodeJS.Platform = process.platform): Promise<void> {
  const url = webUrl(target)
  if (platform === "darwin") return runCommand("open", [url])
  if (platform === "win32") return runCommand("rundll32.exe", ["url.dll,FileProtocolHandler", url])
...
function runCommand(command: string, args: readonly string[]): Promise<void> {
  return new Promise((resolve, reject) => {
    ...
    child.once("exit", code => code === 0 ? resolve() : reject(new Error(`External URL action exited ${String(code)}`)))
  })
}
