...
test("runs a configured argv command with URL substitution and no shell", async () => {
  const directory = await mkdtemp(join(tmpdir(), "vimex-herdr-"))
  await writeFile(
    join(directory, "external-actions.json"),
    JSON.stringify({
  ...
  expect(calls).toEqual([
    ["url-dispatch", ["--target", "https://example.com/a?b=1"]],
  ])
})
...
test("rejects unsupported protocols and malformed configuration", async () => {
  ...
  await expect(actions.openUrl("file:///tmp/secret")).rejects.toThrow(
    "Unsupported URL protocol",
  )
  await writeFile(
    join(directory, "external-actions.json"),
    JSON.stringify({ openUrl: { command: [] } }),
  )
  await expect(readExternalActionConfig(directory)).rejects.toThrow(
    "non-empty argv",
  )
})
