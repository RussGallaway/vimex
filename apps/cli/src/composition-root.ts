...
export function createCommandPorts(): CommandPorts {
  return {
    ...
    launch: async options => {
      ...
      const { runApplication } = await import("../../tui/src/composition-root"); await runApplication({ ...options, cwd }) },
    doctor: async config => {
      const { createDistributionCommands } = await import("@vimex/platform-node")
      const report = await createDistributionCommands({ version, config }).doctor()
      for (const check of report.checks) console.log(`${check.status.toUpperCase()} ${check.name}: ${check.detail}`)
      return report.healthy ? 0 : 1
    },
    upgrade: async target => {
      const { createDistributionCommands } = await import("@vimex/platform-node")
      console.log((await createDistributionCommands({ version }).upgrade(target)).message)
    },
  }
}
