...
test("restores the terminal before slow resource cleanup and detaches handlers last", async () => {
  ...
  const started = new Promise<void>(resolve => { resourceCleanupStarted = resolve })
  const closing = shutdownApplication({
    unmount() { calls.push("unmount") },
    restoreTerminal() { calls.push("restore terminal") },
    async releaseResources() { calls.push("release resources"); resourceCleanupStarted(); await blocked },
    detachHandlers() { calls.push("detach handlers") },
  })
  await started
...
test("continues shutdown after failures and retains handlers until every step ran", async () => {
  const calls: string[] = []
  await expect(shutdownApplication({
    unmount() { calls.push("unmount"); throw new Error("render") },
    restoreTerminal() { calls.push("restore") },
    releaseResources() { calls.push("release"); throw new Error("port") },
    detachHandlers() { calls.push("detach") },
...
test("deduplicates persistence notices until a successful write rearms them", () => {
  ...
  reporter.fail(new Error("disk full again"), message => messages.push(message))
