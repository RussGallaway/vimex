...
export function createDemoGateway(): ConversationGateway &
  ApprovalGateway &
  RuntimeConnection &
  ModelCatalog {
  ...
  const summary: ThreadSummary = {
    id: threadId("demo"),
    title: "Ember Tide · a quiet place to think",
    cwd: process.cwd(),
    gitBranch: "main",
    model: "demo",
  ...
  const add = (event: ConversationEvent) => {
    history.push(event)
    emit({ type: "conversation", event })
  }
  ...
  return {
    ...
    forkThread: async () => {
      throw new Error("Forking requires a live Codex thread")
    },
    async startTurn(id, text) {
      const turn = turnId(crypto.randomUUID()),
        answer = itemId(crypto.randomUUID())
      ...
      const timer = setInterval(() => {
        const chunk = output.slice(offset, offset + 24)
        offset += chunk.length
    ...
    steerTurn: async () => {
      throw new Error("Steering requires a live Codex thread")
    },
