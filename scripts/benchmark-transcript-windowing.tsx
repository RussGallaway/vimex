...
function printResult(result: object): void {
  console.log(
    JSON.stringify({
      ...
      fixtureVersion: "transcript-scaling-v1",
      ...result,
    }),
  )
}
...
const sharedPoint = Object.freeze({
...
function syntheticGeometry(
  ...
  return frame.window.blocks.map((block) =>
    Object.freeze({
      key: Object.freeze({
...
function timed<T>(
  operation: () => T,
): Readonly<{ value: T; milliseconds: number }> {
...
function observingHeightIndex(
  index: TranscriptHeightIndex,
  counters: { nodeVisits: number; nodesCopied: number },
): TranscriptHeightIndex {
  return Object.freeze({
    blockCount: index.blockCount,
    totalRows: index.totalRows,
    prefixRows: (blockIndex: number, explicit?: typeof counters) =>
      index.prefixRows(blockIndex, explicit ?? counters),
    blockAtRow: (row: number, explicit?: typeof counters) =>
      index.blockAtRow(row, explicit ?? counters),
    rowRange: (from: number, to: number, explicit?: typeof counters) =>
      index.rowRange(from, to, explicit ?? counters),
    ...
    replaceHeight: (
      ...
      explicit?: typeof counters,
    ) => index.replaceHeight(override, explicit ?? counters),
    replaceBlock: (
      ...
      rows: number,
      explicit?: typeof counters,
    ) => index.replaceBlock(blocks, previous, next, rows, explicit ?? counters),
...
const oversizedCommandFixture = buildOversizedTranscriptFixtures().find(
  (fixture) => fixture.shape === "command-output",
)!
const oversizedWindowedFixtures = buildOversizedTranscriptFixtures().filter(
  (
    fixture,
  ): fixture is OversizedTranscriptFixture & {
    readonly shape: "markdown" | "split-diff"
  } => fixture.shape === "markdown" || fixture.shape === "split-diff",
)
const oversizedCommandContentHash = benchmarkHash(
  oversizedCommandFixture.source,
)
...
function oversizedCommandSnapshot(
  ...
  const turnConversation = reduceConversationWithDiagnostics(
    fixture.before.conversation,
    {
      type: "turn.started",
      threadId: fixture.threadId,
      turnId: oversizedCommandFixture.item.turnId,
    },
    createConversationReductionDiagnostics(),
  )
  const itemConversation = reduceConversationWithDiagnostics(
    turnConversation,
    {
      type: "item.started",
      threadId: fixture.threadId,
      item: oversizedCommandFixture.item,
    },
    createConversationReductionDiagnostics(),
  )
...
function oversizedFragmentSnapshot(
  fixture: ReturnType<typeof buildTranscriptStructuralScalingFixture>,
  oversized: OversizedTranscriptFixture & {
    readonly shape: "markdown" | "split-diff"
  },
): TranscriptFixtureSnapshot {
  const turnConversation = reduceConversationWithDiagnostics(
...
const oversizedFragmentAddedBlockCounts = new Map(
  oversizedWindowedFixtures.map((oversized) => {
...
function oversizedFragmentProductionBaseline(
  ...
  const addedBlockCount = oversizedFragmentAddedBlockCounts.get(
    oversized.shape,
  )!
  ...
  try {
    ...
    assert(
      sourceSpans.every(
        (span, index) =>
          index === 0 || sourceSpans[index - 1]!.to === span.from,
      ),
    )
    const maxFragmentSourceUnits = Math.max(
      ...sourceSpans.map((span) => span.to - span.from),
    )
    ...
    const delta = Object.fromEntries(
      Object.entries(diagnostics).map(([name, value]) => [
        name,
        value - beforeMove[name as keyof typeof beforeMove],
      ]),
    ...
    const operationCounts = Object.freeze({
      ...
      retainedFragmentIdentities: referenceFragments.filter(
        (block, index) => block === fragments[index],
      ).length,
    ...
    printResult({
      ...
      fixture: {
        contentShape: oversized.shape,
        contentHash: benchmarkHash(oversized.source),
        chars: oversized.source.length,
        deterministicFixtureSegments: oversized.segments.length,
      ...
      timingsMs: {
        coldRuntimePlan: Number(constructed.milliseconds.toFixed(6)),
...
function oversizedCommandProductionBaseline(targetBlockCount: number): void {
  const fixture = buildTranscriptStructuralScalingFixture(
    targetBlockCount - oversizedCommandAddedBlockCount,
  )
  const snapshot = oversizedCommandSnapshot(fixture)
  ...
  try {
    ...
    assert.equal(
      followed.window.blocks.length,
      oversizedCommandFollowMountedCount,
    )
    ...
    const projection =
      transcript.projectionById[oversizedCommandFixture.item.id]!
    const point = Object.freeze({
      itemId: oversizedCommandFixture.item.id,
      graphemeOffset: Math.floor(projection.sourceSpans.length / 2),
    })
    ...
    assert.equal(moved.blocks, followed.blocks)
    assert.equal(
      moved.window.blocks.length,
      oversizedCommandDetachedMountedCount,
    )
    assert(moved.window.blocks.length <= 48)
    assert(pointIsMaterialized(moved.window.blocks, point))
    assert.equal(publications, 1)
    const delta = Object.fromEntries(
      Object.entries(runtimeDiagnostics).map(([name, value]) => [
        name,
        value - beforeMove[name as keyof typeof beforeMove],
      ]),
    ) as typeof runtimeDiagnostics
    assert.equal(delta.completePlanBuilds, 0)
    assert.equal(delta.completePlanBlockVisits, 0)
    assert.equal(delta.heightIndexBuilds, 0)
    assert.equal(delta.heightIndexBlockVisits, 0)
    ...
    printResult({
      ...
      mode: "follow-to-detached-reveal",
      fixture: {
        contentShape: oversizedCommandFixture.shape,
        contentHash: oversizedCommandContentHash,
        fragmentPlanHash: benchmarkHash(
          fragments
            .map(
              (block) =>
                `${blockKey(block)}:${"projection" in block ? `${block.sourceSpan.from}-${block.sourceSpan.to}` : ""}`,
            )
            .join("\n"),
        ),
        chars: oversizedCommandFixture.source.length,
        deterministicFixtureSegments: oversizedCommandFixture.segments.length,
        setupExcludedFromRevealTiming: true,
      },
      operationCounts: {
        producedFragments: fragments.length,
        ...
        maxFragmentSourceUnits: Math.max(
          ...fragments.map((block) =>
            "projection" in block
...
function boundedCanonicalIngressBaseline(
  ...
  const runtime = new TranscriptRuntime(
    runtimeInput(fixture, before, "follow", {
      canonicalDamage: { kind: "full" },
    }),
    {
      windowPolicy: { viewportRows: 24, overscanRows: 24 },
  ...
  try {
    ...
    const snapshot = Object.freeze({
      ...
      conversation: reduced.value,
      transcript: projected.value,
    })

    assert.equal(itemDiagnostics.conversationItemRecordNormalizations, 0)
    assert.equal(
      itemDiagnostics.conversationItemRecordNormalizationItemVisits,
      0,
    )
    ...
    assert(
      itemDiagnostics.conversationItemRecordLookupNodeVisits <=
        2 * logarithmicBound,
    )
    assert(itemDiagnostics.conversationItemRecordNodeVisits <= logarithmicBound)
    assert(
      itemDiagnostics.conversationItemRecordNodesCopied <= 2 * logarithmicBound,
    )
    ...
    const runtimeCounts = Object.fromEntries(
      Object.entries(runtimeDiagnostics).map(([name, value]) => [
        name,
        value - beforeRuntime[name as keyof typeof beforeRuntime],
      ]),
    ) as typeof runtimeDiagnostics
    assert.equal(publications, 1)
    ...
    assert.equal(
      ...
      JSON.stringify(fixture.afterTailDelta),
    )
    assert.equal(projected.value.order, before.transcript.order)
    let preservedCanonicalItems = 0,
      preservedProjections = 0
    for (const id of before.transcript.order) {
      if (id === fixture.tailItemId) continue
      if (snapshot.conversation.items[id] === before.conversation.items[id])
        preservedCanonicalItems++
      if (
        snapshot.transcript.projectionById[id] ===
        before.transcript.projectionById[id]
      )
    ...
    printResult({
      ...
      mode: "follow",
      fixture: {
        contentShape: "one-running-tail-item",
        contentHash: fixture.contentHash,
        setupExcludedFromTiming: true,
        excludedSetup:
          "bulk canonical snapshot construction, cold persistent normalization, disposable-index priming, and exhaustive equivalence checks",
      },
      operationCounts: {
        ...
        semanticProjection: {
          ...
          projectionRecordNodeVisits:
            syncDiagnostics.projectionRecordNodeVisits,
          projectionRecordNodesCopied:
            syncDiagnostics.projectionRecordNodesCopied,
      ...
      timingsMs: {
        ...
        completeIngressSettlement: Number(
          (
            reduced.milliseconds +
            changedItem.milliseconds +
            projected.milliseconds +
            reconciled.milliseconds
          ).toFixed(6),
        ),
      },
      samples: { warmup: 0, measured: 1 },
...
function boundedFollowRuntimeBaseline(
  fixture: ReturnType<typeof buildTranscriptScalingFixture>,
): void {
  forceGc()
  const diagnostics = createRuntimeDiagnostics()
  ...
  try {
    ...
    const followCounts = {
      completePlanBuilds:
        diagnostics.completePlanBuilds - beforeFollow.completePlanBuilds,
      completePlanBlockVisits:
        diagnostics.completePlanBlockVisits -
        beforeFollow.completePlanBlockVisits,
      projectionRecordUpdates:
        diagnostics.projectionRecordUpdates -
        beforeFollow.projectionRecordUpdates,
      projectionRecordNodeVisits:
        diagnostics.projectionRecordNodeVisits -
        beforeFollow.projectionRecordNodeVisits,
      projectionRecordNodesCopied:
        diagnostics.projectionRecordNodesCopied -
        beforeFollow.projectionRecordNodesCopied,
      blockPlanUpdates:
        diagnostics.blockPlanUpdates - beforeFollow.blockPlanUpdates,
      blockPlanNodeVisits:
        diagnostics.blockPlanNodeVisits - beforeFollow.blockPlanNodeVisits,
      blockPlanNodesCopied:
        diagnostics.blockPlanNodesCopied - beforeFollow.blockPlanNodesCopied,
      blockPlanWindowSliceItems:
        diagnostics.blockPlanWindowSliceItems -
        beforeFollow.blockPlanWindowSliceItems,
      heightIndexBuilds:
        diagnostics.heightIndexBuilds - beforeFollow.heightIndexBuilds,
      heightIndexBlockVisits:
        diagnostics.heightIndexBlockVisits -
        beforeFollow.heightIndexBlockVisits,
      heightIndexUpdates:
        diagnostics.heightIndexUpdates - beforeFollow.heightIndexUpdates,
      heightIndexNodeVisits:
        diagnostics.heightIndexNodeVisits - beforeFollow.heightIndexNodeVisits,
      heightIndexNodesCopied:
        diagnostics.heightIndexNodesCopied -
        beforeFollow.heightIndexNodesCopied,
      completeGeometryBlockVisits:
        diagnostics.completeGeometryBlockVisits -
        beforeFollow.completeGeometryBlockVisits,
      windowGeometryBlockVisits:
        diagnostics.windowGeometryBlockVisits -
        beforeFollow.windowGeometryBlockVisits,
      changedItemBuilds:
        diagnostics.changedItemBuilds - beforeFollow.changedItemBuilds,
    }
    ...
    assert(
      followCounts.projectionRecordNodeVisits <=
        2 * Math.ceil(Math.log2(fixture.blockCount + 1)) + 1,
    )
    assert(followCounts.projectionRecordNodesCopied > 0)
    assert(
      followCounts.projectionRecordNodesCopied <=
        2 * Math.ceil(Math.log2(fixture.blockCount + 1)) + 1,
    )
    assert(
      followCounts.blockPlanNodeVisits <=
        2 * (Math.ceil(Math.log2(fixture.blockCount)) + 1),
    )
    assert(followCounts.blockPlanNodesCopied > 0)
    assert(
      followCounts.blockPlanNodesCopied <=
        Math.ceil(Math.log2(fixture.blockCount)) + 1,
    )
    assert(
      followCounts.heightIndexNodeVisits <=
        Math.ceil(Math.log2(fixture.blockCount)) + 1,
    )
    assert(
      followCounts.heightIndexNodesCopied <=
        Math.ceil(Math.log2(fixture.blockCount)) + 1,
    )
    ...
    const reattachCounts = {
      completePlanBuilds:
        diagnostics.completePlanBuilds - beforeReattach.completePlanBuilds,
      completePlanBlockVisits:
        diagnostics.completePlanBlockVisits -
        beforeReattach.completePlanBlockVisits,
      projectionRecordUpdates:
        diagnostics.projectionRecordUpdates -
        beforeReattach.projectionRecordUpdates,
      projectionRecordNodeVisits:
        diagnostics.projectionRecordNodeVisits -
        beforeReattach.projectionRecordNodeVisits,
      projectionRecordNodesCopied:
        diagnostics.projectionRecordNodesCopied -
        beforeReattach.projectionRecordNodesCopied,
      blockPlanUpdates:
        diagnostics.blockPlanUpdates - beforeReattach.blockPlanUpdates,
      blockPlanNodeVisits:
        diagnostics.blockPlanNodeVisits - beforeReattach.blockPlanNodeVisits,
      blockPlanNodesCopied:
        diagnostics.blockPlanNodesCopied - beforeReattach.blockPlanNodesCopied,
      blockPlanWindowSliceItems:
        diagnostics.blockPlanWindowSliceItems -
        beforeReattach.blockPlanWindowSliceItems,
      heightIndexBuilds:
        diagnostics.heightIndexBuilds - beforeReattach.heightIndexBuilds,
      heightIndexBlockVisits:
        diagnostics.heightIndexBlockVisits -
        beforeReattach.heightIndexBlockVisits,
      heightIndexUpdates:
        diagnostics.heightIndexUpdates - beforeReattach.heightIndexUpdates,
      heightIndexNodeVisits:
        diagnostics.heightIndexNodeVisits -
        beforeReattach.heightIndexNodeVisits,
      heightIndexNodesCopied:
        diagnostics.heightIndexNodesCopied -
        beforeReattach.heightIndexNodesCopied,
      completeGeometryBlockVisits:
        diagnostics.completeGeometryBlockVisits -
        beforeReattach.completeGeometryBlockVisits,
      windowGeometryBlockVisits:
        diagnostics.windowGeometryBlockVisits -
        beforeReattach.windowGeometryBlockVisits,
      changedItemBuilds:
        diagnostics.changedItemBuilds - beforeReattach.changedItemBuilds,
    }
    assert.equal(publications, 1)
    assert.equal(
      reattached.value.displayedCanonicalRevision,
    ...
    assert(
      reattachCounts.projectionRecordNodeVisits <=
        2 * Math.ceil(Math.log2(fixture.blockCount + 1)) + 1,
    )
    assert(reattachCounts.projectionRecordNodesCopied > 0)
    assert(
      reattachCounts.projectionRecordNodesCopied <=
        2 * Math.ceil(Math.log2(fixture.blockCount + 1)) + 1,
    )
    assert(
      reattachCounts.blockPlanNodeVisits <=
        2 * (Math.ceil(Math.log2(fixture.blockCount)) + 1),
    )
    assert(reattachCounts.blockPlanNodesCopied > 0)
    assert(
      reattachCounts.blockPlanNodesCopied <=
        Math.ceil(Math.log2(fixture.blockCount)) + 1,
    )
    assert(
      reattachCounts.heightIndexNodeVisits <=
        Math.ceil(Math.log2(fixture.blockCount)) + 1,
    )
    assert(
      reattachCounts.heightIndexNodesCopied <=
        Math.ceil(Math.log2(fixture.blockCount)) + 1,
    )
    ...
    printResult({
      ...
      fixture: {
        ...
        setupExcludedFromTiming: true,
        excludedSetup:
          "canonical snapshot construction, cold persistent-plan normalization, and full-rebuild fallback",
      },
      operationCounts: {
        completeBlocks: fixture.blockCount,
      ...
      timingsMs: {
        ...
        reattachReconciliation: Number(reattached.milliseconds.toFixed(6)),
      },
      samples: { warmup: 0, measured: 1 },
    })
  } finally {
...
function structuralTailAdmissionBaseline(
  fixture: ReturnType<typeof buildTranscriptStructuralScalingFixture>,
): void {
  forceGc()
  const before = fixture.before
  ...
  try {
    ...
    const emptyTurnRuntime = timed(() =>
      runtime.update(
        runtimeInput(fixture, afterTurn, "follow", {
          canonicalDamage: { kind: "blocks", itemIds: [] },
        }),
      ),
    )
    const itemReduction = timed(() =>
      reduceConversationWithDiagnostics(
        afterTurn.conversation,
        itemEvent,
        canonicalDiagnostics,
      ),
    )
    const semanticProjection = timed(() =>
      syncTranscriptItem(afterTurn.transcript, item, semanticDiagnostics),
    )
    ...
    const itemRuntime = timed(() =>
      runtime.update(
        runtimeInput(fixture, afterItem, "follow", {
    ...
    const referenceConversation = reduceConversationReference(
      reduceConversationReference(before.conversation, turnEvent),
      itemEvent,
    )
    ...
    assert.deepEqual(
      itemRuntime.value.blocks.map(blockKey),
    ...
    for (let index = 0; index < fixture.blockCount; index++) {
      ...
      if (
        afterItem.conversation.items[itemId] ===
        before.conversation.items[itemId]
      )
        preservedItems++
      if (
        afterItem.conversation.turns[turnId] ===
        before.conversation.turns[turnId]
      )
        preservedTurns++
      if (
        afterItem.transcript.projectionById[itemId] ===
        before.transcript.projectionById[itemId]
      )
        preservedProjections++
      if (itemRuntime.value.blocks[index] === initial.blocks[index])
        preservedBlocks++
    }
    ...
    transcriptTextLengthRange(
      itemRuntime.value.transcript,
      0,
      itemRuntime.value.transcript.order.length,
      runtimeDiagnostics,
    )
    ...
    const runtimeCounts = {
      completePlanBuilds:
        ...
        runtimeBefore.completePlanBuilds,
      completePlanBlockVisits:
        runtimeDiagnostics.completePlanBlockVisits -
        runtimeBefore.completePlanBlockVisits,
      orderIndexBuilds:
        runtimeDiagnostics.orderIndexBuilds - runtimeBefore.orderIndexBuilds,
      orderIndexItemVisits:
        runtimeDiagnostics.orderIndexItemVisits -
        runtimeBefore.orderIndexItemVisits,
      heightIndexBuilds:
        runtimeDiagnostics.heightIndexBuilds - runtimeBefore.heightIndexBuilds,
      heightIndexBlockVisits:
        runtimeDiagnostics.heightIndexBlockVisits -
        runtimeBefore.heightIndexBlockVisits,
      completeGeometryBlockVisits:
        runtimeDiagnostics.completeGeometryBlockVisits -
        runtimeBefore.completeGeometryBlockVisits,
      blockPlanUpdates:
        runtimeDiagnostics.blockPlanUpdates - runtimeBefore.blockPlanUpdates,
      blockPlanNodeVisits:
        runtimeDiagnostics.blockPlanNodeVisits -
        runtimeBefore.blockPlanNodeVisits,
      blockPlanNodesCopied:
        runtimeDiagnostics.blockPlanNodesCopied -
        runtimeBefore.blockPlanNodesCopied,
      heightIndexUpdates:
        runtimeDiagnostics.heightIndexUpdates -
        runtimeBefore.heightIndexUpdates,
      heightIndexNodeVisits:
        runtimeDiagnostics.heightIndexNodeVisits -
        runtimeBefore.heightIndexNodeVisits,
      heightIndexNodesCopied:
        runtimeDiagnostics.heightIndexNodesCopied -
        runtimeBefore.heightIndexNodesCopied,
      windowGeometryBlockVisits:
        runtimeDiagnostics.windowGeometryBlockVisits -
        runtimeBefore.windowGeometryBlockVisits,
      blockPlanWindowSliceItems:
        runtimeDiagnostics.blockPlanWindowSliceItems -
        runtimeBefore.blockPlanWindowSliceItems,
      changedItemBuilds:
        runtimeDiagnostics.changedItemBuilds - runtimeBefore.changedItemBuilds,
      textLengthIndexBuilds:
        runtimeDiagnostics.textLengthIndexBuilds -
        runtimeBefore.textLengthIndexBuilds,
      textLengthItemVisits:
        runtimeDiagnostics.textLengthItemVisits -
        runtimeBefore.textLengthItemVisits,
      textLengthIndexUpdates:
        runtimeDiagnostics.textLengthIndexUpdates -
        runtimeBefore.textLengthIndexUpdates,
      urlIndexBuilds:
        runtimeDiagnostics.urlIndexBuilds - runtimeBefore.urlIndexBuilds,
      urlIndexItemVisits:
        runtimeDiagnostics.urlIndexItemVisits -
        runtimeBefore.urlIndexItemVisits,
      urlIndexUpdates:
        runtimeDiagnostics.urlIndexUpdates - runtimeBefore.urlIndexUpdates,
    }
    assert.equal(runtimeCounts.completePlanBuilds, 0)
    assert.equal(runtimeCounts.completePlanBlockVisits, 0)
    ...
    assert.equal(
      canonicalDiagnostics.conversationTurnIdSequenceNormalizations,
      0,
    )
    assert.equal(canonicalDiagnostics.conversationTurnRecordNormalizations, 0)
    assert.equal(
      canonicalDiagnostics.conversationTurnItemIdSequenceNormalizations,
      0,
    )
    ...
    printResult({
      ...
      materialization: "windowed-production",
      boundary:
        "canonical-structure-semantic-projection-runtime-window-publication",
      blockCount: fixture.blockCount,
      viewport: { width: 80, height: 24 },
      mode: "follow",
      fixture: {
        contentShape: "one-semantic-item-per-turn",
        contentHash: fixture.contentHash,
        setupExcludedFromTiming: true,
        excludedSetup:
          "bulk canonical snapshot construction, cold persistent normalization, disposable-index priming, full references, and exhaustive identity checks",
      },
      operationCounts: {
      ...
      timingsMs: {
        emptyTurnCanonicalReduction: Number(
          turnReduction.milliseconds.toFixed(6),
        ),
        emptyTurnRuntimeAdvance: Number(
          emptyTurnRuntime.milliseconds.toFixed(6),
        ),
        itemCanonicalReduction: Number(itemReduction.milliseconds.toFixed(6)),
        itemSemanticProjection: Number(
          semanticProjection.milliseconds.toFixed(6),
        ),
...
function tailTurnCompletionBaseline(
  fixture: ReturnType<typeof buildTranscriptStructuralScalingFixture>,
): void {
  forceGc()
  ...
  try {
    ...
    const runningTranscript = syncTranscriptItem(
      fixture.before.transcript,
      runningItem,
    )
    ...
    runtime.update(
      runtimeInput(fixture, runningSnapshot, "follow", {
        canonicalDamage: { kind: "blocks", itemIds: [fixture.nextItemId] },
      }),
    )
    ...
    const damage = timed(() =>
      incrementalConversationEventDamage(withItem, event),
    )
    assert.deepEqual(damage.value, { kind: "blocks", itemIds: [] })
    const reduction = timed(() =>
      reduceConversationWithDiagnostics(withItem, event, canonicalDiagnostics),
    )
    ...
    assert.deepEqual(
      reconciled.value.blocks.map(blockKey),
      reference.blocks.map(blockKey),
    )
    ...
    for (let index = 0; index < fixture.blockCount; index++) {
      assert.equal(
        reconciled.value.blocks[index],
        beforeCompletion.blocks[index],
      )
    }
    assert.notEqual(
      reconciled.value.blocks[fixture.blockCount],
      beforeCompletion.blocks[fixture.blockCount],
    )
    ...
    assert.deepEqual(
      reconciled.value.blocks[fixture.blockCount],
      reference.blocks[fixture.blockCount],
    )
    ...
    const logarithmicBound =
      12 * (Math.ceil(Math.log2(fixture.blockCount + 2)) + 1)
    ...
    printResult({
      ...
      mode: "follow",
      fixture: {
        contentShape: "one-semantic-item-per-turn-plus-one-running-tail",
        contentHash: fixture.contentHash,
        setupExcludedFromTiming: true,
        excludedSetup:
          "bulk fixture construction, tail admission, cold indexes, full reference, and identity audit",
      },
      operationCounts: {
      ...
      timingsMs: {
        diagnosticWorkbenchDamageSelection: Number(
          damage.milliseconds.toFixed(6),
        ),
        canonicalTurnReduction: Number(reduction.milliseconds.toFixed(6)),
        runtimeActivityReconciliation: Number(
          reconciled.milliseconds.toFixed(6),
        ),
      },
      samples: { warmup: 0, measured: 1 },
    })
...
function detachedUnseenAccumulationBaseline(
  fixture: ReturnType<typeof buildTranscriptStructuralScalingFixture>,
): void {
  forceGc()
  ...
  try {
    ...
    const hiddenDamageSeed = {
      merges:
        runtimeDiagnostics.hiddenDamageMerges! -
        (runtimeBeforeSeed.hiddenDamageMerges ?? 0),
      inputItemVisits:
        runtimeDiagnostics.hiddenDamageInputItemVisits! -
        (runtimeBeforeSeed.hiddenDamageInputItemVisits ?? 0),
      itemAdditions:
        runtimeDiagnostics.hiddenDamageItemAdditions! -
        (runtimeBeforeSeed.hiddenDamageItemAdditions ?? 0),
      snapshots:
        runtimeDiagnostics.hiddenDamageSnapshots! -
        (runtimeBeforeSeed.hiddenDamageSnapshots ?? 0),
      snapshotItemVisits:
        runtimeDiagnostics.hiddenDamageSnapshotItemVisits! -
        (runtimeBeforeSeed.hiddenDamageSnapshotItemVisits ?? 0),
    }
    assert.deepEqual(hiddenDamageSeed, {
      merges: 1,
      inputItemVisits: hiddenDamageBacklog.length,
      itemAdditions: hiddenDamageBacklog.length,
      snapshots: 0,
      snapshotItemVisits: 0,
    })
    const runtimeBefore = { ...runtimeDiagnostics }
    const canonicalDiagnostics = createConversationReductionDiagnostics()
    const semanticDiagnostics = createTranscriptItemSyncDiagnostics()
    ...
    const itemProjection = timed(() =>
      syncTranscriptItem(detachedTranscript, item, semanticDiagnostics),
    )
    const afterItem: TranscriptFixtureSnapshot = Object.freeze({
      canonicalRevision: afterTurn.canonicalRevision + 1,
      conversation: itemReduction.value,
    ...
    const deltaReduction = timed(() =>
      reduceConversationWithDiagnostics(
        afterItem.conversation,
        Object.freeze({
          type: "item.delta" as const,
          threadId: fixture.threadId,
    ...
    assert.equal(
      presentation.value.transcript.unseenItemIds,
      repeatProjection.value.unseenItemIds,
    )
    assert.equal(repeatProjection.value.unseenEntries, fixture.blockCount + 1)
    assert.equal(
      repeatProjection.value.unseenItemIds,
      itemProjection.value.unseenItemIds,
    )
    assert.deepEqual(
      [...repeatProjection.value.unseenItemIds],
      [...unseenItemIds, fixture.nextItemId],
    )

    assert.equal(semanticDiagnostics.unseenItemSequenceNormalizations, 0)
    assert.equal(
      semanticDiagnostics.unseenItemSequenceNormalizationItemVisits,
      0,
    )
    assert.equal(semanticDiagnostics.unseenItemMembershipChecks, 2)
    assert.equal(semanticDiagnostics.unseenItemAppends, 1)
    const logarithmicBound =
      4 * (Math.ceil(Math.log2(fixture.blockCount + 1)) + 1)
    assert(
      semanticDiagnostics.unseenItemMembershipNodeVisits <= logarithmicBound,
    )
    assert(semanticDiagnostics.unseenItemAppendNodeVisits <= logarithmicBound)
    assert(
      semanticDiagnostics.unseenItemIndexUpdateNodeVisits <= logarithmicBound,
    )
    assert(
      semanticDiagnostics.unseenItemIndexUpdateNodesCopied <=
        logarithmicBound * 2,
    )
    assert.equal(semanticDiagnostics.textLengthIndexBuilds, 0)
    assert.equal(semanticDiagnostics.textLengthItemVisits, 0)
    ...
    assert.equal(
      canonicalDiagnostics.conversationTurnIdSequenceNormalizationVisits,
      0,
    )
    assert.equal(canonicalDiagnostics.conversationTurnRecordNormalizations, 0)
    assert.equal(
      canonicalDiagnostics.conversationTurnRecordNormalizationVisits,
      0,
    )
    assert.equal(canonicalDiagnostics.conversationItemRecordNormalizations, 0)
    assert.equal(
      canonicalDiagnostics.conversationItemRecordNormalizationItemVisits,
      0,
    )
    assert.equal(
      runtimeDiagnostics.hiddenDamageMerges! -
        (runtimeBefore.hiddenDamageMerges ?? 0),
      3,
    )
    assert.equal(
      runtimeDiagnostics.hiddenDamageInputItemVisits! -
        (runtimeBefore.hiddenDamageInputItemVisits ?? 0),
      2,
    )
    assert.equal(
      runtimeDiagnostics.hiddenDamageItemAdditions! -
        (runtimeBefore.hiddenDamageItemAdditions ?? 0),
      1,
    )
    assert.equal(
      runtimeDiagnostics.hiddenDamageSnapshots! -
        (runtimeBefore.hiddenDamageSnapshots ?? 0),
      0,
    )
    assert.equal(
      runtimeDiagnostics.completePlanBuilds - runtimeBefore.completePlanBuilds,
      0,
    )
    assert.equal(
      runtimeDiagnostics.completePlanBlockVisits -
    ...
    assert.equal(
      runtimeDiagnostics.heightIndexUpdates - runtimeBefore.heightIndexUpdates,
      0,
    )
    assert.equal(
      runtimeDiagnostics.completeGeometryBlockVisits -
    ...
    printResult({
      ...
      materialization: "windowed-production",
      boundary:
        "canonical-semantic-unseen-runtime-and-presentation-publication",
      blockCount: fixture.blockCount,
      viewport: { width: 80, height: 24 },
      mode: "detached",
      fixture: {
        contentShape: "one-semantic-item-per-turn-with-complete-unseen-backlog",
        contentHash: fixture.contentHash,
        setupExcludedFromTiming: true,
        excludedSetup:
          "bulk canonical construction, persistent unseen backlog, and initial runtime/window settlement",
      },
      operationCounts: {
        completeBlocks: fixture.blockCount,
        ...
        semantic: {
          sequenceNormalizations:
            semanticDiagnostics.unseenItemSequenceNormalizations,
          normalizationItemVisits:
            semanticDiagnostics.unseenItemSequenceNormalizationItemVisits,
          membershipChecks: semanticDiagnostics.unseenItemMembershipChecks,
          membershipNodeVisits:
            semanticDiagnostics.unseenItemMembershipNodeVisits,
          appends: semanticDiagnostics.unseenItemAppends,
          appendNodeVisits: semanticDiagnostics.unseenItemAppendNodeVisits,
          indexUpdateNodeVisits:
            semanticDiagnostics.unseenItemIndexUpdateNodeVisits,
          indexUpdateNodesCopied:
            semanticDiagnostics.unseenItemIndexUpdateNodesCopied,
        },
        hiddenDamage: {
          ...
          snapshotItemVisits:
            runtimeDiagnostics.hiddenDamageSnapshotItemVisits! -
            (runtimeBefore.hiddenDamageSnapshotItemVisits ?? 0),
        },
        runtime: {
          completePlanBuilds:
            runtimeDiagnostics.completePlanBuilds -
          ...
          completePlanBlockVisits:
            ...
            runtimeBefore.completePlanBlockVisits,
          blockPlanUpdates:
            runtimeDiagnostics.blockPlanUpdates -
            runtimeBefore.blockPlanUpdates,
          heightIndexBuilds:
            runtimeDiagnostics.heightIndexBuilds -
            runtimeBefore.heightIndexBuilds,
          heightIndexUpdates:
            runtimeDiagnostics.heightIndexUpdates -
            runtimeBefore.heightIndexUpdates,
          completeGeometryBlockVisits:
          ...
          windowGeometryBlockVisits:
            ...
            runtimeBefore.windowGeometryBlockVisits,
          changedItemBuilds:
            runtimeDiagnostics.changedItemBuilds -
            runtimeBefore.changedItemBuilds,
        },
      },
      timingsMs: {
        emptyTurnCanonicalReduction: Number(
        ...
        emptyTurnRuntimeAdoption: Number(
          emptyTurnRuntime.milliseconds.toFixed(6),
        ),
        newItemCanonicalReduction: Number(
          itemReduction.milliseconds.toFixed(6),
        ),
        newItemSemanticProjection: Number(
          itemProjection.milliseconds.toFixed(6),
        ),
        newItemRuntimeAdoption: Number(itemRuntime.milliseconds.toFixed(6)),
        repeatCanonicalReduction: Number(
          deltaReduction.milliseconds.toFixed(6),
        ),
        repeatSemanticProjection: Number(
          repeatProjection.milliseconds.toFixed(6),
        ),
        ...
        presentationOnlyPublication: Number(
          presentation.milliseconds.toFixed(6),
        ),
      },
      samples: { warmup: 0, measured: 1 },
    })
...
function sideInheritedMembershipBaseline(blockCount: number): void {
  ...
  assert.equal(
    projected.value.workspaces[childId]!.transcript.projectionById[localItemId]
      ?.source,
    "seed delta",
  )
  ...
  printResult({
    ...
    mode: "detached",
    fixture: {
      contentShape: "inherited-parent-turns-plus-one-live-child-item",
      setupExcludedFromTiming: true,
      excludedSetup:
        "persistent inherited-turn construction and child workspace/item projection",
    },
    operationCounts: {
      ...
      membershipNodeVisits:
        diagnostics.conversationTurnIdSequenceLookupNodeVisits,
      sequenceNormalizations:
        diagnostics.conversationTurnIdSequenceNormalizations,
      normalizationVisits:
        diagnostics.conversationTurnIdSequenceNormalizationVisits,
    },
    timingsMs: {
      canonicalAndSemanticProjection: Number(projected.milliseconds.toFixed(6)),
    },
    samples: { warmup: 0, measured: 1 },
  })
}
...
function runtimeBaseline(
  ...
  try {
    ...
    const geometryPublication = timed(() =>
    ...
    assert.equal(
      geometryPublication.value.geometry.measuredBlockCount,
      fixture.blockCount,
    )
    ...
    assert.equal(publications, 1)
    let changedBlocks = 0,
      preservedBlockIdentities = 0,
      preservedGeometryIdentities = 0
    const tailKey = `item:${fixture.tailItemId}:root`
    for (let index = 0; index < fixture.blockCount; index++) {
      const beforeBlock = measured.blocks[index]!,
        afterBlock = followed.value.blocks[index]!
      if (beforeBlock === afterBlock) preservedBlockIdentities++
      else changedBlocks++
      const key = blockKey(afterBlock)
      if (
        key !== tailKey &&
        followed.value.geometry.byBlockKey[key] === historyGeometry[key]
      )
        preservedGeometryIdentities++
    }
    ...
    assert.equal(
      publications,
      2,
      "navigation and detach must each publish once",
    )
    const pinned = runtime.getSnapshot()
    publications = 0
    const hidden = appendTranscriptScalingTail(
    ...
    assert.equal(publications, 1)
    assert.equal(
      reveal.value.displayedCanonicalRevision,
      hidden.canonicalRevision,
    )
    assert.deepEqual(reveal.value.transcript.cursor, revealPoint)
    assert.deepEqual(reveal.value.transcript.viewport, {
      kind: "point",
      point: revealPoint,
      preferredScreenRow: 7,
    })
    ...
    assert.equal(
      publications,
      1,
      "hidden output after reveal must not publish the pinned presentation",
    )
    publications = 0
    const reattach = timed(() =>
      runtime.update(runtimeInput(fixture, latest, "follow")),
    )
    assert.equal(publications, 1)
    const reference = createTranscriptFrame(
      runtimeInput(fixture, latest, "follow", {
    ...
    assert.deepEqual(
      reattach.value.blocks.map(blockKey),
      reference.blocks.map(blockKey),
    )
    unsubscribe()
    ...
    printResult({
      ...
      operationCounts: {
        ...
        retainedMeasurementsAfterFollow:
          followed.value.geometry.measuredBlockCount,
        followPublications: 1,
      ...
      timingsMs: {
        ...
        geometryPublication: Number(
          geometryPublication.milliseconds.toFixed(6),
        ),
        followReconciliation: Number(followed.milliseconds.toFixed(6)),
...
function RuntimePublicationProbe(props: {
  ...
  return (
    <Profiler
      ...
      onRender={(_id, _phase, duration) => props.commits.push(duration)}
    >
...
function HiddenPresentationProbeInner(props: {
  ...
  observe: {
    frame?: TranscriptFrame
    renders?: number
    presentation?: WorkbenchState
  }
}) {
  const [visible, setVisible] = useState(true)
  const scrollRef = useRef<ScrollBoxRenderable>(null)
  props.control.setVisible = setVisible
  props.observe.presentation = useVisiblePresentationSnapshot(
    props.presentationHost,
    "main",
    visible,
  )
  const frame = useTranscriptRuntime(
    { transcriptRuntime: () => props.runtime },
    "main",
    undefined,
    visible,
  )
  ...
  return (
    <box width={primaryViewport.width} height={24}>
      {visible ? (
        <TranscriptViewport
          window={frame.window}
        ...
        <box id="hidden-presentation-retained-state" />
      )}
    </box>
  )
}

function HiddenPresentationProbe(
  props: Parameters<typeof HiddenPresentationProbeInner>[0] & {
    commits: number[]
  },
) {
  return (
...
async function hiddenPresentationResourceBaseline(
  fixture: ReturnType<typeof buildTranscriptScalingFixture>,
): Promise<void> {
  forceGc()
  const runtime = new TranscriptRuntime(
  ...
  const presentationHost: WorkbenchPublicationHost & { publishHidden(): void } =
    {
      getLayoutSnapshot: () => {
        throw new Error("layout is outside this presentation visibility cell")
      },
      subscribeLayout: () => () => {},
  ...
  runtime.reportMeasurements = (batch) => {
    measurementReports++
    return originalReportMeasurements(batch)
  }
  ...
  try {
    ...
    const hiddenTranscriptRoots = setup.renderer.root.findDescendantById(
      "transcript",
    )
      ? 1
      : 0
    const hiddenScroll = setup.renderer.root.findDescendantById(
      "transcript",
    ) as ScrollBoxRenderable | undefined
    const hiddenMountedBlocks = hiddenScroll
      ? mountedBlockRoots(hiddenScroll).size
      : 0
    const hiddenSpacerRoots = [
      "transcript-top-spacer",
    ...
    const stopPublication = runtime.subscribe(() => {
      runtimePublications++
    })
    ...
    stopPublication()
    const hiddenMeasurementReports =
      measurementReports - hiddenMeasurementReportsBefore
    ...
    assert.equal(
      setup.renderer.root.findDescendantById("transcript"),
      undefined,
    )
    ...
    assert.equal(
      observe.frame?.displayedCanonicalRevision,
      latest.displayedCanonicalRevision,
    )
    ...
    printResult({
      ...
      timingsMs: {
        hiddenRuntimePublicationAndReactFlush: Number(
          hiddenSettlementMs.toFixed(6),
        ),
      },
      samples: { warmup: 0, measured: 1 },
    })
...
async function reactPublicationBaseline(
  fixture: ReturnType<typeof buildTranscriptScalingFixture>,
): Promise<void> {
  forceGc()
  const runtime = new TranscriptRuntime(
  ...
  const commits: number[] = []
  const setup = await testRender(
    <RuntimePublicationProbe runtime={runtime} commits={commits} />,
    primaryViewport,
  )
  try {
    await act(async () => {
    ...
    printResult({
      ...
      mode: "follow",
      fixture: {
        contentShape: "constant-one-node-subscriber",
        contentHash: fixture.contentHash,
        setupExcludedFromTiming: true,
      },
      operationCounts: {
      ...
      timingsMs: {
        runtimeUpdate: Number(runtimeUpdateMs.toFixed(6)),
        postUpdateFlushAndFrame: Number(
          (performance.now() - started - runtimeUpdateMs).toFixed(6),
        ),
...
function runtimeCorrectionBaseline(
  fixture: ReturnType<typeof buildTranscriptScalingFixture>,
): void {
  forceGc()
  const runtime = new TranscriptRuntime(
  ...
  try {
    ...
    const corrected = timed(() =>
      runtime.reportMeasurements({ ...base, measurements: [measurement] }),
    )
    assert.equal(publications, 1)
    assert.equal(corrected.value.geometry.totalRows, fixture.blockCount + 3)
    assert.equal(
      corrected.value.geometry.blockRows.length,
      corrected.value.window.blocks.length,
    )
    assert(corrected.value.window.blocks.length <= 48)
    assert(
      Object.keys(corrected.value.geometry.byBlockKey).length <=
        corrected.value.window.blocks.length,
    )
    const replay = timed(() =>
      runtime.reportMeasurements({
        ...base,
        measurements: [{ ...measurement, nativeRevision: 2 }],
      }),
    )
    ...
    printResult({
      ...
      operationCounts: {
        ...
        retainedDetailedGeometry: Object.keys(
          corrected.value.geometry.byBlockKey,
        ).length,
        totalRowDelta:
          corrected.value.geometry.totalRows - before.geometry.totalRows,
      },
      timingsMs: {
...
function offWindowTargetBaseline(
  fixture: ReturnType<typeof buildTranscriptScalingFixture>,
): void {
  const anchor = Object.freeze({
  ...
  try {
    ...
    assert.deepEqual(motion.value?.point, {
    ...
    const revealed = timed(() =>
      runtime.update(
        runtimeInput(
          fixture,
          Object.freeze({ ...hidden, transcript: targetTranscript }),
          "detached",
          {
            presentationDamage: { kind: "view" },
    ...
    printResult({
      ...
      operationCounts: {
        ...
        retainedCompleteBlockPlanIdentity:
          revealed.value.blocks === before.blocks ? 1 : 0,
        completePlanBuilds,
        completePlanBlockVisits,
        orderIndexBuilds,
        orderIndexItemVisits,
...
function indexedUrlAndFoldBaseline(
  fixture: ReturnType<typeof buildTranscriptScalingFixture>,
): void {
  const anchor = Object.freeze({
    itemId: fixture.targets.quarter,
  ...
  try {
    const targetBlock = runtime
      .getSnapshot()
      .window.blocks.find(
        (block) => block.key.kind === "item" && block.key.itemId === foldTarget,
      )!
    runtime.reportMeasurements({
      ...runtime.measurementBase(),
      measurements: [
        {
          key: {
            ...
            width: 80,
            styleRevision: "stage-5.4-fold-path",
            folded: false,
          },
          nativeRevision: 1,
          rows: 8,
    ...
    const urlReference = moveByUrlReference(
      before.transcript,
      "backward",
    ...
    const urlCounts = {
      ...
      urlIndexItemVisits:
        diagnostics.urlIndexItemVisits - urlBefore.urlIndexItemVisits,
      urlIndexCacheHits:
        diagnostics.urlIndexCacheHits - urlBefore.urlIndexCacheHits,
      urlIndexNodeVisits:
        diagnostics.urlIndexNodeVisits - urlBefore.urlIndexNodeVisits,
    }
    ...
    assert(
      urlCounts.urlIndexNodeVisits <=
        4 * Math.ceil(Math.log2(fixture.blockCount)) + 5,
    )
    ...
    const fold = timed(() =>
      runtime.update(
        runtimeInput(fixture, foldedSnapshot, "detached", {
    ...
    const foldCounts = {
      completePlanBuilds:
        diagnostics.completePlanBuilds - foldBefore.completePlanBuilds,
      completePlanBlockVisits:
        diagnostics.completePlanBlockVisits -
        foldBefore.completePlanBlockVisits,
      heightIndexBuilds:
        diagnostics.heightIndexBuilds - foldBefore.heightIndexBuilds,
      heightIndexBlockVisits:
        diagnostics.heightIndexBlockVisits - foldBefore.heightIndexBlockVisits,
      heightIndexUpdates:
        diagnostics.heightIndexUpdates - foldBefore.heightIndexUpdates,
      heightIndexNodeVisits:
        diagnostics.heightIndexNodeVisits - foldBefore.heightIndexNodeVisits,
      heightIndexNodesCopied:
        diagnostics.heightIndexNodesCopied - foldBefore.heightIndexNodesCopied,
      completeGeometryBlockVisits:
        diagnostics.completeGeometryBlockVisits -
        foldBefore.completeGeometryBlockVisits,
      windowGeometryBlockVisits:
        diagnostics.windowGeometryBlockVisits -
        foldBefore.windowGeometryBlockVisits,
    }
    assert.equal(publications, 1)
    ...
    assert(
      foldCounts.heightIndexNodeVisits <=
        Math.ceil(Math.log2(fixture.blockCount)) + 1,
    )
    assert(foldCounts.heightIndexNodesCopied > 0)
    assert.equal(
      foldCounts.heightIndexNodesCopied,
      foldCounts.heightIndexNodeVisits,
    )
    ...
    printResult({
      ...
      operationCounts: {
        ...
        publications,
        retainedCompleteBlockPlanIdentity:
          fold.value.blocks === before.blocks ? 1 : 0,
        ...urlCounts,
        ...foldCounts,
      },
      timingsMs: {
...
function DetachedStatusNativeProbeInner(props: {
  ...
  useTranscriptLayout({
    ...
    onMeasurementDiagnostics: (diagnostics) => {
      ...
      props.measurementTotals.attemptedMeasurements +=
        diagnostics.attemptedMeasurements
    },
  })
  props.observe.unseenEntries =
    state.workspaces[props.threadId]?.transcript.unseenEntries
  props.observe.frame = frame
  return (
    <box
      width={props.viewport.width}
...
function DetachedStatusNativeProbe(
  props: Parameters<typeof DetachedStatusNativeProbeInner>[0] & {
    commits: number[]
  },
) {
  return (
    <Profiler
      id="detached-status-native"
      onRender={(_id, _phase, duration) => props.commits.push(duration)}
    >
...
function assertBoundedNativeShape(
  blocks: number,
  counts: Readonly<{ descendants: number; spacers: number }>,
): void {
  assert.equal(
    counts.spacers,
    2,
    "both stable transcript spacer roots must remain mounted",
  )
  assert(
    counts.descendants >= blocks + counts.spacers,
    "native tree must include each mounted block root",
  )
  assert(
    counts.descendants <= blocks * 24 + counts.spacers,
    "native descendants must remain bounded by materialized blocks",
  )
}

function assertMountedRoots(
  frame: TranscriptFrame,
  roots: ReadonlyMap<string, Renderable>,
): void {
  assert.equal(roots.size, frame.window.blocks.length)
  for (const block of frame.window.blocks)
    assert(
      roots.has(transcriptBlockRenderableId(block)),
      `missing mounted root ${blockKey(block)}`,
    )
}
...
async function detachedStatusNativePublicationBaseline(
  ...
  const runtime = new TranscriptRuntime(
    runtimeInput(fixture, initialSnapshot, "detached", {
      canonicalDamage: { kind: "full" },
    }),
    {
      windowPolicy: {
        viewportRows: viewport.height - 1,
        overscanRows: viewport.height - 1,
      },
    },
  )
  const originalReportMeasurements = runtime.reportMeasurements.bind(runtime)
  ...
  const presentationHost: WorkbenchPublicationHost = {
    getLayoutSnapshot: () => {
      throw new Error("layout is outside the detached status publication cell")
    },
    subscribeLayout: () => () => {},
    getPresentationSnapshot: () => presentationSnapshot,
    subscribePresentation: (_presentationId, listener) => {
  ...
  try {
    for (let pass = 0; pass < 8; pass++) {
      ...
      if (
        runtime.getSnapshot().geometry.measuredBlockCount > 0 &&
    ...
    assert(
      runtime.getSnapshot().geometry.measuredBlockCount > 0,
      "connected detached probe must settle native geometry",
    )
    ...
    for (let pass = 0; pass < 8 && quiescentPasses < 2; pass++) {
      ...
      const attempted =
        measurementTotals.attemptedMeasurements - attemptedBefore
      quiescentPasses =
        attempted === 0 && diagnostics.pendingAfter === 0
          ? quiescentPasses + 1
          : 0
    }
    assert.equal(
      quiescentPasses,
      2,
      "connected detached probe must drain native measurement work before timing",
    )
    ...
    const item = Object.freeze({
      ...
      turnId: fixture.nextTurnId,
      kind: "assistant" as const,
      markdown: "Detached status-only publication.",
      status: "running" as const,
    })
    commits.length = 0
    ...
    const firstAttemptedMeasurementsBefore =
      measurementTotals.attemptedMeasurements
    let runtimePublications = 0
    ...
    const firstMeasurementPasses =
      measurementTotals.passes - firstMeasurementPassesBefore
    const firstAttemptedMeasurements =
      measurementTotals.attemptedMeasurements - firstAttemptedMeasurementsBefore
    const rootsAfterFirst = mountedBlockRoots(scroll)
    let retainedAfterFirst = 0
    for (const [id, root] of rootsBefore)
      if (rootsAfterFirst.get(id) === root) retainedAfterFirst++
    assert.equal(
      state.workspaces[fixture.threadId]!.transcript.unseenEntries,
      fixture.blockCount + 1,
    )
    ...
    const repeatAttemptedMeasurementsBefore =
      measurementTotals.attemptedMeasurements
    ...
    const repeatMeasurementPasses =
      measurementTotals.passes - repeatMeasurementPassesBefore
    const repeatAttemptedMeasurements =
      measurementTotals.attemptedMeasurements -
      repeatAttemptedMeasurementsBefore
    stopRuntime()
    const rootsAfterRepeat = mountedBlockRoots(scroll)
    let retainedAfterRepeat = 0
    for (const [id, root] of rootsBefore)
      if (rootsAfterRepeat.get(id) === root) retainedAfterRepeat++
    assert.equal(
      state.workspaces[fixture.threadId]!.transcript.unseenEntries,
      fixture.blockCount + 1,
    )
    ...
    printResult({
      ...
      materialization: "windowed-production",
      boundary:
        "workbench-selector-react-runtime-native-root-and-measurement-scheduler",
      blockCount: fixture.blockCount,
      viewport,
      mode: "detached",
      fixture: {
        ...
        setupExcludedFromTiming: true,
        excludedSetup:
          "initial Workbench/runtime construction, React/OpenTUI mount, and native geometry settlement",
      },
      operationCounts: {
        ...
        firstDistinctItem: {
          workbenchPublications: 1,
          runtimePublications: 0,
          reactCommits: firstCommits,
          measurementPasses: firstMeasurementPasses,
          measurementReports: firstMeasurementReports,
          attemptedMeasurements: firstAttemptedMeasurements,
          retainedNativeRoots: retainedAfterFirst,
          mountedNativeRoots: rootsAfterFirst.size - retainedAfterFirst,
        ...
        repeatedItemDelta: {
          ...
          runtimePublications,
          reactCommits: commits.length,
          measurementPasses: repeatMeasurementPasses,
          measurementReports,
          attemptedMeasurements: repeatAttemptedMeasurements,
          retainedNativeRoots: retainedAfterRepeat,
          mountedNativeRoots: rootsAfterRepeat.size - retainedAfterRepeat,
      ...
      timingsMs: {
        ...
        repeatedItemNoPublicationSettlement: Number(
          repeatSettlementMs.toFixed(6),
        ),
      },
      samples: { warmup: 0, measured: 1 },
    })
...
async function nativeStructuralAdmissionBaseline(
  ...
  try {
    ...
    assert(
      seededLayout,
      "seeded structural geometry must produce a layout before admission",
    )
    assert.equal(
      seedDiagnostics.pendingAfter,
      0,
      "seeded structural geometry must drain pending measurements",
    )
    const initial = runtime.getSnapshot()
    const rootsBefore = mountedBlockRoots(scroll)
    assertMountedRoots(initial, rootsBefore)
    assertBoundedNativeShape(
      initial.window.blocks.length,
      mountedTreeCounts(scroll),
    )
    ...
    const item = Object.freeze({
      ...
      turnId: fixture.nextTurnId,
      kind: "assistant" as const,
      markdown: "New structural native tail block.",
      status: "running" as const,
    })
    const canonicalDiagnostics = createConversationReductionDiagnostics()
    const afterTurn: TranscriptFixtureSnapshot = Object.freeze({
      canonicalRevision: fixture.before.canonicalRevision + 1,
      conversation: reduceConversationWithDiagnostics(
        fixture.before.conversation,
        turnEvent,
        canonicalDiagnostics,
      ),
    ...
    for (const [id, root] of rootsBefore)
      assert.equal(rootsAfterEmptyTurn.get(id), root)
    ...
    assertBoundedNativeShape(
      admitted.window.blocks.length,
      mountedTreeCounts(scroll),
    )
    let retainedRoots = 0
    for (const [id, root] of rootsAfterEmptyTurn)
      if (rootsAfterItem.get(id) === root) retainedRoots++
    ...
    let measurement!: ReturnType<
      typeof timed<ReturnType<typeof measureRenderedTranscript>>
    >
    commits.length = 0
    await act(async () => {
      measurement = timed(() =>
    ...
    assert.equal(
      measurementPublications,
      measurementDiagnostics.changedMeasurements > 0 ? 1 : 0,
    )
    await act(async () => {
      ...
      assert(
        layout,
        "structural native geometry must settle on its acknowledgement pass",
      )
      await setup.flush()
      await setup.renderOnce()
    })
    ...
    const stopCompletionRuntime = runtime.subscribe(() => {
      completionRuntimePublications++
    })
    commits.length = 0
    ...
    assertBoundedNativeShape(
      completed.window.blocks.length,
      mountedTreeCounts(scroll),
    )
    let completionRetainedRoots = 0
    for (const [id, root] of rootsBeforeCompletion)
      if (rootsAfterCompletion.get(id) === root) completionRetainedRoots++
    const completionMountedRoots =
      rootsAfterCompletion.size - completionRetainedRoots
    const completionUnmountedRoots =
      rootsBeforeCompletion.size - completionRetainedRoots
    ...
    const stopCompletionMeasurement = runtime.subscribe(() => {
      completionMeasurementPublications++
    })
    const completionMeasurementDiagnostics = createDiagnostics()
    commits.length = 0
    let completionMeasurement!: ReturnType<
      typeof timed<ReturnType<typeof measureRenderedTranscript>>
    >
    await act(async () => {
      completionMeasurement = timed(() =>
        measureRenderedTranscript(setup.renderer, scroll, {
          frame: completed,
          runtime,
          styleRevision,
          diagnostics: completionMeasurementDiagnostics,
        }),
      )
      await setup.flush()
      await setup.renderOnce()
    ...
    assert.equal(
      completionMeasurementDiagnostics.trackedMountedRoots,
      rootsAfterCompletion.size,
    )
    assert.equal(
      completionMeasurementPublications,
    ...
    await act(async () => {
      ...
      assert(
        layout,
        "completion activity geometry must settle on its acknowledgement pass",
      )
      await setup.flush()
      await setup.renderOnce()
    })
    ...
    printResult({
      ...
      fixture: {
        ...
        setupExcludedFromTiming: true,
        excludedSetup:
          "initial runtime/React/OpenTUI mount, tail admission, and settled native geometry",
      },
      operationCounts: {
        completeBlocksBefore: beforeCompletion.blocks.length,
        ...
        acceptedHeightCorrections:
          completionMeasurementDiagnostics.changedMeasurements,
      ...
      timingsMs: {
        ...
        postRuntimeReactNativeSettlement: Number(
          (completionSettlementMs - completionRuntimeUpdateMs).toFixed(6),
        ),
        completeRuntimeReactNativeSettlement: Number(
          completionSettlementMs.toFixed(6),
        ),
        measurementPublication: Number(
          completionMeasurement.milliseconds.toFixed(6),
        ),
      },
      samples: { warmup: 0, measured: 1 },
    })
    ...
    printResult({
      ...
      fixture: {
        ...
        setupExcludedFromTiming: true,
        excludedSetup:
          "initial runtime construction, React/OpenTUI mount, and initial native geometry settlement",
      },
      operationCounts: {
        completeBlocksBefore: initial.blocks.length,
        completeBlocksAfter: admitted.blocks.length,
        mountedBlocksBefore: rootsBefore.size,
        mountedBlocksAfter: rootsAfterItem.size,
        retainedRoots,
        mountedRoots,
        unmountedRoots,
        runtimePublications,
        emptyTurnReactCommits,
        itemReactCommits,
        measurementPublications,
        measuredBlocks: measurementDiagnostics.attemptedMeasurements,
      ...
      timingsMs: {
        emptyTurnRuntimeReactNativeSettlement: Number(
          emptyTurnSettlementMs.toFixed(6),
        ),
        ...
        itemPostRuntimeReactNativeSettlement: Number(
          (itemSettlementMs - itemRuntimeUpdateMs).toFixed(6),
        ),
        itemRuntimeReactNativeSettlement: Number(itemSettlementMs.toFixed(6)),
        postAdmissionMeasurementPublication: Number(
          measurement.milliseconds.toFixed(6),
        ),
      },
      samples: { warmup: 0, measured: 1 },
    })
...
async function oversizedCommandNativeBaseline(
  ...
  try {
    ...
    const stopFollowMeasurement = runtime.subscribe(() => {
      followMeasurementPublications++
    })
    let followMeasured!: ReturnType<
      typeof timed<ReturnType<typeof measureRenderedTranscript>>
    >
    await act(async () => {
      followMeasured = timed(() =>
        measureRenderedTranscript(setup.renderer, scroll, {
          frame: followFrame,
          runtime,
          styleRevision,
    ...
    const stopFollowAck = runtime.subscribe(() => {
      followAckPublications++
    })
    let followAcknowledged!: ReturnType<
      typeof timed<ReturnType<typeof measureRenderedTranscript>>
    >
    await act(async () => {
      followAcknowledged = timed(() =>
        measureRenderedTranscript(setup.renderer, scroll, {
          frame: runtime.getSnapshot(),
          runtime,
          styleRevision,
          diagnostics: followAckDiagnostics,
        }),
      )
      assert(
        followAcknowledged.value,
        "oversized command follow geometry did not settle",
      )
      await setup.flush()
      await setup.renderOnce()
    })
    ...
    assertBoundedNativeShape(
      settledFollowFrame.window.blocks.length,
      mountedTreeCounts(scroll),
    )
    assert(settledFollowRoots.size <= viewport.height * 2)
    oversizedNativeFollowRoots.set(
      viewportKey,
      oversizedNativeFollowRoots.get(viewportKey) ?? settledFollowRoots.size,
    )
    assert.equal(
      settledFollowRoots.size,
      oversizedNativeFollowRoots.get(viewportKey),
    )
    const followMeasurementAttempts =
      followDiagnostics.attemptedMeasurements +
      followAckDiagnostics.attemptedMeasurements
    oversizedNativeFollowMeasurements.set(
      viewportKey,
      oversizedNativeFollowMeasurements.get(viewportKey) ??
        followMeasurementAttempts,
    )
    assert.equal(
      followMeasurementAttempts,
      oversizedNativeFollowMeasurements.get(viewportKey),
    ...
    const stopReveal = runtime.subscribe(() => {
      revealPublications++
    })
    ...
    const stopDetachedMeasurement = runtime.subscribe(() => {
      detachedMeasurementPublications++
    })
    let detachedMeasured!: ReturnType<
      typeof timed<ReturnType<typeof measureRenderedTranscript>>
    >
    await act(async () => {
      detachedMeasured = timed(() =>
        measureRenderedTranscript(setup.renderer, scroll, {
          frame: detachedFrame,
          runtime,
          styleRevision,
    ...
    let detachedAckPublications = 0,
      detachedAckMilliseconds = 0
    ...
    for (let pass = 0; pass < 4; pass++) {
      ...
      const stopDetachedAck = runtime.subscribe(() => {
        passPublications++
      })
      let measured!: ReturnType<
        typeof timed<ReturnType<typeof measureRenderedTranscript>>
      >
      await act(async () => {
        measured = timed(() =>
      ...
      if (
        detachedAcknowledged &&
        passDiagnostics.changedMeasurements === 0 &&
        passDiagnostics.pendingAfter === 0
      )
        break
    }
    ...
    assert(
      detachedAcknowledged,
      "oversized command detached geometry did not settle",
    )
    ...
    assertBoundedNativeShape(
      settledDetachedFrame.window.blocks.length,
      mountedTreeCounts(scroll),
    )
    assert(settledDetachedRoots.size <= viewport.height * 2)
    oversizedNativeDetachedRoots.set(
      viewportKey,
      oversizedNativeDetachedRoots.get(viewportKey) ??
        settledDetachedRoots.size,
    )
    assert.equal(
      settledDetachedRoots.size,
      oversizedNativeDetachedRoots.get(viewportKey),
    )
    const detachedAckAttemptedMeasurements = detachedAckPasses.reduce(
      (sum, pass) => sum + pass.attemptedMeasurements,
      0,
    )
    const detachedAckChangedMeasurements = detachedAckPasses.reduce(
      (sum, pass) => sum + pass.changedMeasurements,
      0,
    )
    const detachedAckCandidateBlocks = detachedAckPasses.reduce(
      (sum, pass) => sum + pass.candidateBlocks,
      0,
    )
    const detachedMeasurementAttempts =
      detachedDiagnostics.attemptedMeasurements +
      detachedAckAttemptedMeasurements
    oversizedNativeDetachedMeasurements.set(
      viewportKey,
      oversizedNativeDetachedMeasurements.get(viewportKey) ??
        detachedMeasurementAttempts,
    )
    assert.equal(
      detachedMeasurementAttempts,
      oversizedNativeDetachedMeasurements.get(viewportKey),
    )
    const completeRevealMeasurementSettlementMs =
      performance.now() - completeSettlementStarted
    const measurementSettlementReactCommits =
      commits.length - revealReactCommits
    assert.equal(
    ...
    printResult({
      ...
      operationCounts: {
        ...
        followAckAttemptedMeasurements:
          followAckDiagnostics.attemptedMeasurements,
        ...
        targetMaterialized: pointIsMaterialized(
          settledDetachedFrame.window.blocks,
          point,
        ),
      },
      timingsMs: {
        followMeasurementPublication: Number(
          followMeasured.milliseconds.toFixed(6),
        ),
        followMeasurementAcknowledgement: Number(
          followAcknowledged.milliseconds.toFixed(6),
        ),
        revealRuntimeReactNativeSettlement: Number(
          revealSettlementMs.toFixed(6),
        ),
        detachedMeasurementPublication: Number(
          detachedMeasured.milliseconds.toFixed(6),
        ),
        detachedMeasurementAcknowledgement: Number(
          detachedAckMilliseconds.toFixed(6),
        ),
        completeRevealMeasurementSettlement: Number(
          completeRevealMeasurementSettlementMs.toFixed(6),
        ),
      },
      samples: { warmup: 0, measured: 1 },
    })
...
async function oversizedFragmentNativeBaseline(
  ...
  try {
    ...
    const fragments = initialFrame.blocks.filter(
      (block) =>
        block.key.kind === "item" && block.key.itemId === oversized.item.id,
    )
    assert(fragments.length > 1)
    ...
    const settle = async (
      trackedBefore: ReadonlySet<string>,
    ): Promise<
      Readonly<{
    ...
    > => {
      ...
      for (let pass = 0; pass < 32; pass++) {
        ...
        const currentKeys = new Set(
          runtime.getSnapshot().window.blocks.map(blockKey),
        )
        assert.equal(rootsBefore.size, currentKeys.size)
        const expectedPruned = [...tracked]
        ...
        assert(
          diagnostics.changedMeasurements <= diagnostics.attemptedMeasurements,
        )
        ...
        const windowStable =
          rootsAfter.size === rootsBefore.size &&
          keysAfter.size === currentKeys.size &&
          [...keysAfter].every((key) => currentKeys.has(key))
        const geometryCoversWindow =
          frameAfter.geometry.measuredBlockCount === rootsAfter.size
        if (
          layout &&
          diagnostics.changedMeasurements === 0 &&
          diagnostics.pendingAfter === 0 &&
          windowStable &&
          geometryCoversWindow
        ) {
          settled = true
          break
        }
      ...
      assert(
        settled,
        `${oversized.shape} native geometry did not settle within the bounded acknowledgement budget: ${JSON.stringify(passes)}`,
      )
      ...
      assert.equal(
        publications,
        passes.filter((pass) => pass.changedMeasurements > 0).length,
      )
      return Object.freeze({
        passes: Object.freeze(passes),
        publications,
        milliseconds: performance.now() - started,
        prunedKeys: Object.freeze(prunedKeys),
      })
    }
    ...
    const followRetainedRows = followFrame.geometry.blockRows.reduce(
      (sum, block) => sum + block.rows,
      0,
    )
    ...
    const targetBlock = revealedFrame.window.blocks.find(
      (block) =>
        "projection" in block &&
    ...
    await act(async () => {
      ...
      const blockRow =
        revealedFrame.geometry.rowByBlockKey[key] ??
        revealedFrame.window.topSpacerRows
      const localRow =
        revealedFrame.geometry.byBlockKey[key]?.points[point.graphemeOffset]
          ?.row ?? 0
      const renderable = scroll.getRenderable(
        transcriptBlockRenderableId(targetBlock),
      )
      if (
        !renderable ||
        renderable.screenY < scroll.viewport.screenY ||
        ...
        scroll.scrollTo(
          Math.max(
            ...
            blockRow +
              localRow -
              detachedTranscript.viewport.preferredScreenRow,
          ),
        )
      }
      await setup.flush()
    ...
    const departedFollowKeys = followFrame.window.blocks
      .map(blockKey)
      .filter((key) => !revealKeys.has(key))
      .sort()
    const detachedSettlement = await settle(
      new Set(followFrame.window.blocks.map(blockKey)),
    )
    assert(detachedSettlement.passes.length <= 8)
    assert.deepEqual(
      detachedSettlement.passes[0]?.prunedRoots,
      departedFollowKeys.length,
    )
    assert.deepEqual(
      detachedSettlement.prunedKeys.slice(0, departedFollowKeys.length).sort(),
      departedFollowKeys,
    )
    const detachedFrame = runtime.getSnapshot()
    ...
    const detachedRetainedRows = detachedFrame.geometry.blockRows.reduce(
      (sum, block) => sum + block.rows,
      0,
    )
    let retainedOldRootIdentities = 0
    for (const [key, root] of followRoots)
      if (detachedRoots.get(key) === root) retainedOldRootIdentities++
    ...
    assert(
      followFrame.geometry.totalPoints <=
        followRoots.size * maxRetainedPointsPerRoot,
    )
    assert(
      detachedFrame.geometry.totalPoints <=
        detachedRoots.size * maxRetainedPointsPerRoot,
    )
    ...
    const compactPasses = (passes: readonly SettlementPass[]) =>
      Object.freeze(
        passes.map((pass) =>
          Object.freeze({
    ...
    const operationCounts = Object.freeze({
      ...
      prunedRootsThroughDetachedSettlement:
        detachedSettlement.prunedKeys.length,
    ...
    const nativeWorkDiagnostics = Object.freeze({
      ...
      followMeasurementAttempts: followSettlement.passes.reduce(
        (sum, pass) => sum + pass.attemptedMeasurements,
        0,
      ),
      detachedPasses: compactPasses(detachedSettlement.passes),
      detachedMeasurementAttempts: detachedSettlement.passes.reduce(
        (sum, pass) => sum + pass.attemptedMeasurements,
        0,
      ),
    })
    ...
    printResult({
      ...
      operationCounts: {
        ...
        ...nativeWorkDiagnostics,
        followCompleteTotalRows: followFrame.geometry.totalRows,
        detachedCompleteTotalRows: detachedFrame.geometry.totalRows,
        targetMaterialized: pointIsMaterialized(
          detachedFrame.window.blocks,
          point,
        ),
      },
      timingsMs: {
        coldRuntimePlan: Number(coldRuntimePlanMs.toFixed(6)),
        coldReactNativeMount: Number(coldNativeMountMs.toFixed(6)),
        followMeasurementSettlement: Number(
          followSettlement.milliseconds.toFixed(6),
        ),
        revealRuntimeReactNativeSettlement: Number(
          revealSettlementMs.toFixed(6),
        ),
        detachedMeasurementSettlement: Number(
          detachedSettlement.milliseconds.toFixed(6),
        ),
      },
      samples: { warmup: 0, measured: 1 },
    })
...
async function nativeMountBaseline(
  fixture: ReturnType<typeof buildTranscriptScalingFixture>,
  viewport: Readonly<{ width: number; height: number }>,
): Promise<void> {
  forceGc()
  ...
  try {
    ...
    if (process.env.VIMEX_WINDOWING_GEOMETRY_BASELINE === "1") {
      ...
      assert.equal(
        measurement.value,
        undefined,
        "the first scheduler pass must publish geometry before returning a layout",
      )
      ...
      assert(
        measuredFrame.geometry.measuredBlockCount <=
          measuredFrame.window.blocks.length,
      )
      assert.equal(
        measuredFrame.geometry.blockRows.length,
        measuredFrame.window.blocks.length,
      )
      assert.equal(
        diagnostics.attemptedMeasurements,
        frame.window.blocks.length,
      )
      ...
      await act(async () => {
        settledLayout = measureRenderedTranscript(setup.renderer, scroll, {
          frame: measuredFrame,
          runtime,
          styleRevision,
        })
      ...
      assert(
        settledLayout,
        "published native geometry must build a layout on the next scheduler pass",
      )
      steadyRoots = mountedBlockRoots(scroll)
      assertMountedRoots(runtime.getSnapshot(), steadyRoots)
      const blocksWithPoints = Object.values(
        measuredFrame.geometry.byBlockKey,
      ).filter(
        (geometry) =>
          (geometry.pointCount ?? Object.keys(geometry.points).length) > 0,
      ).length
      printResult({
        ...
        mode: "follow",
        fixture: {
          contentShape: "rendered-mixed-semantic-root-blocks",
          contentHash: fixture.contentHash,
          setupExcludedFromTiming: true,
        },
        operationCounts: {
          mountedBlocksBeforeCorrection: rootsBefore.size,
          measuredBlocks,
          changedBlocks: 0,
          publications: measurementPublications,
          materializedWindowBlocksAfterCorrection:
            measuredFrame.window.blocks.length,
          acceptedHeightCorrections: diagnostics.changedMeasurements,
          retainedDetailedGeometryAfterCorrection:
            measuredFrame.geometry.measuredBlockCount,
          candidateBlocks: diagnostics.candidateBlocks,
          visibleCandidates: diagnostics.visibleCandidates,
          overscanCandidates: diagnostics.overscanCandidates,
          attemptedMeasurements: diagnostics.attemptedMeasurements,
          trackedMountedRoots: diagnostics.trackedMountedRoots,
          pendingMeasurementAcknowledgements: diagnostics.pendingAfter,
          mountedNativeDescendantsBeforeCorrection:
            nativeTreeBefore.descendants,
          stableSpacerRoots: nativeTreeBefore.spacers,
          blocksWithPoints,
          retainedZeroPointGeometry:
            measuredFrame.geometry.measuredBlockCount - blocksWithPoints,
          measuredPoints: measuredFrame.geometry.totalPoints,
        },
        timingsMs: {
          measurementSchedulerAndGeometryPublication: Number(
            measurement.milliseconds.toFixed(6),
          ),
        },
        samples: { warmup: 0, measured: 1 },
    ...
    for (const [id, root] of steadyRoots)
      if (rootsAfter.get(id) === root) retainedBlockRoots++
    ...
    printResult({
      ...
      fixture: {
        contentShape: "rendered-mixed-semantic-root-blocks",
        contentHash: fixture.contentHash,
        setupExcludedFromTiming: false,
      },
      operationCounts: {
        mountedBlocks: steadyRoots.size,
        measuredBlocks: 0,
        changedBlocks: 1,
        publications: runtimePublications,
        completeBlocks: frame.blocks.length,
        materializedWindowBlocks: runtime.getSnapshot().window.blocks.length,
        observedMountedBlockRoots: steadyRoots.size,
        retainedBlockRoots,
        mountedRootsDuringFollow: rootsAfter.size - retainedBlockRoots,
        unmountedRootsDuringFollow: steadyRoots.size - retainedBlockRoots,
        runtimePublications,
        reactCommits: commits.length,
      },
      timingsMs: {
        testRendererReactSetupAndNativeMount: Number(
          nativeMountAndLayoutMs.toFixed(6),
        ),
        runtimeUpdate: Number(runtimeUpdateMs.toFixed(6)),
        postUpdateReactAndNativeFrame: Number(
          (endToEndFollowSettlementMs - runtimeUpdateMs).toFixed(6),
        ),
        presentationSettlement: Number(endToEndFollowSettlementMs.toFixed(6)),
        reactCommitDurations: stats(commits),
      },
      samples: { warmup: 0, measured: 1 },
    })
    ...
    if (process.env.VIMEX_WINDOWING_GEOMETRY_BASELINE === "1") {
      const beforeFollowMeasurement = new Map(
        runtime.getSnapshot().window.blocks.map((block) => {
          const root = rootsAfter.get(transcriptBlockRenderableId(block))!
          return [blockKey(block), blockNativeRevision(root)] as const
        }),
      )
      let followMeasurementPublications = 0
      const unsubscribeFollowMeasurement = runtime.subscribe(() => {
        followMeasurementPublications++
      })
      let followMeasurement!: ReturnType<
        typeof timed<ReturnType<typeof measureRenderedTranscript>>
      >
      const followDiagnostics = createDiagnostics()
      await act(async () => {
        followMeasurement = timed(() =>
          measureRenderedTranscript(setup.renderer, scroll, {
            frame: runtime.getSnapshot(),
            runtime,
            styleRevision,
      ...
      const measuredBlocks = followMeasuredFrame.window.blocks.filter(
        (block) => {
          const root = rootsAfter.get(transcriptBlockRenderableId(block))!
          return (
            blockNativeRevision(root) >
            (beforeFollowMeasurement.get(blockKey(block)) ?? 0)
          )
        },
      ).length
      assert.equal(measuredBlocks, 1)
      assert(
        followDiagnostics.candidateBlocks <= viewport.height,
        "follow candidates must remain viewport-bounded",
      )
      assert(
        followDiagnostics.attemptedMeasurements <= viewport.height,
        "follow measurement attempts must remain viewport-bounded",
      )
      assert.equal(followMeasurementPublications, 1)
      let followedLayout: ReturnType<typeof measureRenderedTranscript>
      await act(async () => {
        followedLayout = measureRenderedTranscript(setup.renderer, scroll, {
          frame: followMeasuredFrame,
          runtime,
          styleRevision,
        })
        await setup.flush()
      ...
      assert(
        followedLayout,
        "changed tail geometry must settle on the next scheduler pass",
      )
      ...
      printResult({
        ...
        operationCounts: {
          ...
          measuredBlocks,
          changedBlocks: 1,
          publications: followMeasurementPublications,
          materializedWindowBlocks: followMeasuredFrame.window.blocks.length,
          retainedMeasurements: followMeasuredFrame.geometry.measuredBlockCount,
          candidateBlocks: followDiagnostics.candidateBlocks,
          visibleCandidates: followDiagnostics.visibleCandidates,
          overscanCandidates: followDiagnostics.overscanCandidates,
          attemptedMeasurements: followDiagnostics.attemptedMeasurements,
          trackedMountedRoots: followDiagnostics.trackedMountedRoots,
          pendingMeasurementAcknowledgements: followDiagnostics.pendingAfter,
          measuredPoints: followMeasuredFrame.geometry.totalPoints,
        },
        timingsMs: {
          measurementSchedulerAndGeometryPublication: Number(
            followMeasurement.milliseconds.toFixed(6),
          ),
        },
        samples: { warmup: 0, measured: 1 },
      })
    ...
    for (let pass = 0; pass < 8; pass++) {
      ...
      await act(async () => {
        movementSeedLayout = measureRenderedTranscript(setup.renderer, scroll, {
          ...
          runtime,
          styleRevision,
          diagnostics: movementSeedDiagnostics,
        })
        await setup.flush()
        await setup.renderOnce()
      })
      if (movementSeedLayout && movementSeedDiagnostics.pendingAfter === 0)
        break
    }
    assert(
      movementSeedLayout,
      "pre-move native geometry must settle before detached cleanup measurement",
    )
    ...
    const unsubscribeMovement = runtime.subscribe(() => {
    ...
    let overlap = 0
    for (const [id, root] of settledFollowRoots)
      if (detachedRootsBefore.get(id) === root) overlap++
    const targetBlock =
      runtime
        .getSnapshot()
        .window.blocks.find(
          (block) =>
            block.key.kind === "item" &&
            block.key.itemId === fixture.targets.middle,
        ) ??
      runtime.getSnapshot().window.blocks[
        Math.floor(runtime.getSnapshot().window.blocks.length / 2)
      ]!
    const targetRoot = detachedRootsBefore.get(
      transcriptBlockRenderableId(targetBlock),
    )!
    await act(async () => {
    ...
    const visibleTop = scroll.viewport.screenY,
      visibleBottom = visibleTop + scroll.viewport.height
    const independentlyVisibleKeys = runtime
      .getSnapshot()
      .window.blocks.flatMap((block) => {
        const root = detachedRootsBefore.get(transcriptBlockRenderableId(block))
        return root &&
          root.screenY < visibleBottom &&
          root.screenY + root.height > visibleTop
          ? [blockKey(block)]
          : []
      })
    assert(
      independentlyVisibleKeys.length > 0,
      "detached movement must put planned roots in the native viewport",
    )
    ...
    for (; settlementPasses < 4; settlementPasses++) {
      ...
      await act(async () => {
        ...
        assert(
          layout,
          "detached movement geometry must settle on an acknowledgement pass",
        )
        await setup.flush()
        await setup.renderOnce()
      })
    ...
    const departedMeasuredKeys = settledFollowFrame.window.blocks
      .map(blockKey)
      .filter((key) => !detachedBlockKeys.has(key))
      .sort()
    assert.equal(shiftDiagnostics.prunedRoots, departedMeasuredKeys.length)
    assert.deepEqual(
      shiftDiagnostics.prunedKeys?.slice().sort(),
      departedMeasuredKeys,
    )
    assert(shiftDiagnostics.attemptedMeasurements <= detachedRootsBefore.size)
    assert.equal(
      shiftDiagnostics.visibleCandidates,
      independentlyVisibleKeys.length,
    )
    assert.deepEqual(
      shiftDiagnostics.attemptedKeys?.slice(0, independentlyVisibleKeys.length),
      independentlyVisibleKeys,
    )
    ...
    printResult({
      ...
      operationCounts: {
        mountedBlocks: detachedRootsBefore.size,
        measuredBlocks: shiftDiagnostics.changedMeasurements,
        changedBlocks: 0,
        publications: movementPublications,
        retainedBlockRoots: overlap,
        mountedRootsDuringMove: detachedRootsBefore.size - overlap,
        unmountedRootsDuringMove: settledFollowRoots.size - overlap,
        candidateBlocks: shiftDiagnostics.candidateBlocks,
        attemptedMeasurements: shiftDiagnostics.attemptedMeasurements,
        trackedMountedRoots: shiftDiagnostics.trackedMountedRoots,
        prunedRoots: shiftDiagnostics.prunedRoots,
        visibleCandidates: shiftDiagnostics.visibleCandidates,
        overscanCandidates: shiftDiagnostics.overscanCandidates,
        pendingMeasurementsAfterAcknowledgement:
          settledDiagnostics.pendingAfter,
        acknowledgementPasses: settlementPasses,
        mountedNativeDescendants: mountedTreeCounts(scroll).descendants,
        stableSpacerRoots: mountedTreeCounts(scroll).spacers,
      },
      timingsMs: {
        runtimeReactNativeRoot: Number(runtimeReactNativeRootMs.toFixed(6)),
        postMoveMeasurementPublication: Number(
          measurementPublicationMs.toFixed(6),
        ),
    ...
    const backloggedTranscript = Object.freeze({
      ...
      unseenEntries: fixture.blockCount,
      unseenItemIds: persistentTranscriptUnseenItemIds(
        detachedTranscript.order,
      ),
    })
    const backloggedSnapshot = Object.freeze({
      ...detachedSnapshot,
      transcript: backloggedTranscript,
    })
    const hidden = appendTranscriptScalingTail(
      backloggedSnapshot,
      fixture.tailItemId,
    ...
    assert.equal(
      hidden.transcript.unseenItemIds,
      backloggedTranscript.unseenItemIds,
    )
    let detachedPublications = 0
    const unsubscribeDetached = runtime.subscribe(() => {
    ...
    for (const [id, root] of pinnedRoots)
      if (detachedRootsAfter.get(id) === root) detachedRetainedRoots++
    ...
    printResult({
      ...
      fixture: {
        contentShape:
          "rendered-mixed-semantic-root-blocks-with-complete-unseen-backlog",
        contentHash: fixture.contentHash,
        setupExcludedFromTiming: true,
      },
      operationCounts: {
        mountedBlocks: pinnedRoots.size,
        changedBlocks: 0,
        publications: detachedPublications,
        canonicalChangedBlocks: 1,
        unseenBacklog: backloggedTranscript.unseenItemIds.length,
        retainedBlockRoots: detachedRetainedRoots,
        mountedRootsDuringHiddenDelta: 0,
        unmountedRootsDuringHiddenDelta: 0,
        runtimePublications: detachedPublications,
        reactCommits: commits.length,
      },
      timingsMs: {
        hiddenDeltaPresentationSettlement: Number(
          detachedSettlementMs.toFixed(6),
        ),
      },
      samples: { warmup: 0, measured: 1 },
    })
...
if (
  process.env.VIMEX_WINDOWING_NATIVE_BASELINE === "1" &&
  !process.env.VIMEX_WINDOWING_SIZES
) {
  throw new Error(
    "Native production-window baselines require an explicit VIMEX_WINDOWING_SIZES selection; see the file header",
  )
}
for (const blockCount of requestedSizes) {
  assert(
    transcriptScalingBlockCounts.includes(
      blockCount as (typeof transcriptScalingBlockCounts)[number],
    ),
    `unsupported block count ${blockCount}`,
  )
  ...
  for (const oversized of oversizedWindowedFixtures)
    oversizedFragmentProductionBaseline(blockCount, oversized)
...
for (const fixture of buildOversizedTranscriptFixtures())
  printResult({
    ...
    mode: "follow-and-detached",
    fixture: {
      contentShape: fixture.shape,
      chars: fixture.source.length,
      stableSegments: fixture.segments.length,
      rootBlocksBeforeSubBlockPlanning: 1,
    },
    operationCounts: {
      fixtureItems: 1,
      deterministicFixtureSegments: fixture.segments.length,
    },
    samples: { warmup: 0, measured: 1 },
  })
