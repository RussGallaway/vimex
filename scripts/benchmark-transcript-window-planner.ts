...
function timed<Value>(operation: () => Value): Timed<Value> {
  ...
  return Object.freeze({
    value,
    milliseconds: Number((performance.now() - started).toFixed(6)),
  })
}
...
function stableSubBlockFixture(blockCount: number) {
  ...
  const blocks: readonly TranscriptBlock[] = Object.freeze(
    Array.from({ length: blockCount }, (_, index): TranscriptItemBlock =>
      Object.freeze({
        key: Object.freeze({
          kind: "item" as const,
          itemId: id,
          blockId: `segment:${index}`,
        }),
        turnId: itemTurn,
        item,
        renderItem: item,
...
function observingIndex(
  ...
  return Object.freeze({
    ...
    prefixRows: (blockIndex: number, explicit?: HeightIndexDiagnostics) =>
      index.prefixRows(blockIndex, explicit ?? counters),
    blockAtRow: (row: number, explicit?: HeightIndexDiagnostics) =>
      index.blockAtRow(row, explicit ?? counters),
    rowRange: (from: number, to: number, explicit?: HeightIndexDiagnostics) =>
    ...
    replaceHeight: (
      ...
      explicit?: HeightIndexDiagnostics,
...
function plan(
  ...
  counters: HeightIndexDiagnostics,
  input: Omit<
    PlanTranscriptWindowInput,
    "blocks" | "heights" | "viewportRows" | "overscanRows"
  >,
): TranscriptWindow {
...
function windowFacts(
  ...
  assert(
    ...
    "a nonempty scaling fixture must produce a nonempty planned window",
  )
  const firstIndex = heights.blockIndex(blockKey(window.blocks[0]!))
  assert(
    firstIndex !== undefined,
    "planned first block is absent from the height index",
  )
  ...
  for (let offset = 0; offset < window.blocks.length; offset++) {
    if (window.blocks[offset] === blocks[firstIndex + offset])
  ...
  assert.equal(
    window.topSpacerRows + range.rows + window.bottomSpacerRows,
    heights.totalRows,
  )
  return Object.freeze({
...
for (const blockCount of transcriptScalingBlockCounts) {
  ...
  assert(
    heights,
    "deterministic fixture unexpectedly produced duplicate stable block keys",
  )
  ...
  const followed = timed(() =>
    plan(blocks, heights, followDiagnostics, { attachment: { kind: "tail" } }),
  )
  const followFacts = windowFacts(
    blocks,
    heights,
    followed.value,
    followDiagnostics,
  ...
  assert(
    pointIsMaterialized(followed.value.blocks, {
  ...
  const detached = timed(() =>
    plan(blocks, heights, detachedDiagnostics, {
      attachment: {
        kind: "point",
        point: { itemId: fixture.targets.middle, graphemeOffset: 0 },
        preferredScreenRow: 7,
      },
      diagnostics: detachedTargetDiagnostics,
    }),
  )
  const detachedFacts = windowFacts(
    blocks,
    heights,
    detached.value,
    detachedDiagnostics,
    detachedTargetDiagnostics,
  )
  ...
  assert(
    pointIsMaterialized(detached.value.blocks, {
  ...
  const revealed = timed(() =>
    plan(blocks, heights, revealDiagnostics, {
      attachment: {
        kind: "point",
        point: { itemId: fixture.targets.quarter, graphemeOffset: 0 },
        preferredScreenRow: 7,
      },
      reveal: revealPoint,
  ...
  const revealFacts = windowFacts(
    blocks,
    heights,
    revealed.value,
    revealDiagnostics,
    revealTargetDiagnostics,
  )
  ...
  const replacementIndex = heights.blockIndex(
    blockKey(blocks[Math.floor(blocks.length / 2)]!),
  )
  ...
  const replacementFacts = windowFacts(
    blocks,
    replacement.value.updated,
    replacement.value.window,
    replacementDiagnostics,
    replacementTargetDiagnostics,
  )
  ...
  assert(
    pointIsMaterialized(replacement.value.window.blocks, {
      itemId: fixture.targets.middle,
      graphemeOffset: 0,
    }),
  )
  assert.equal(
    heights.totalRows,
    blockCount,
    "persistent height replacement mutated the prior index",
  )
  ...
  assert(
    subBlockHeights,
    "stable same-item sub-blocks unexpectedly failed height indexing",
  )
  assert.equal(
    subBlockHeights.itemBlockIndexes(subBlocks.itemId)?.length,
    blockCount,
  )
  const subBlockHeightDiagnostics = diagnostics()
  const subBlockTargetDiagnostics = windowDiagnostics()
  const subBlockPlan = timed(() =>
    plan(subBlocks.blocks, subBlockHeights, subBlockHeightDiagnostics, {
      attachment: {
        kind: "point",
        point: subBlocks.target,
        preferredScreenRow: 7,
      },
    ...
    JSON.stringify({
      ...
      operationCounts: {
        indexBuild: {
          ...
          indexedBlocks: heights.blockCount,
          totalRows: heights.totalRows,
        },
        follow: followFacts,
        ...
        noOpHeightUpdate: {
          ...
          changedBlocks: 0,
          publications: 0,
          nodeVisits: noOpDiagnostics.nodeVisits,
          nodesCopied: noOpDiagnostics.nodesCopied,
          retainedIndexIdentity: noOp.value === heights,
        },
        ...
        sameItemSubBlockTarget: {
          ...subBlockFacts,
          indexedItemBlocks: subBlockHeights.itemBlockIndexes(subBlocks.itemId)
            ?.length,
          logarithmicTargetVisitBound: treeDepthBound + 3,
        },
      },
      fallback: {
        unsupportedRelationshipUsesPassThrough:
          fallback.blocks === unrelatedBlocks,
