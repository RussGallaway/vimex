...
function runtimeInput(
  ...
  canonicalDamage: TranscriptRuntimeInput["canonicalDamage"] = { kind: "none" },
...
const largeDetail = Array.from(
  { length: 1500 },
  (_, index) => `${index}: ${"result ".repeat(12)}`,
).join("\n")
...
function largeSource(): Source {
  let source: Source = Object.freeze({
    conversation: createConversation(largeThread),
    transcript: initialTranscript(),
    revision: 0,
  })
  source = apply(source, {
    type: "turn.started",
    threadId: largeThread,
    turnId: largeTurn,
  })
  return apply(source, {
    type: "item.started",
...
function largeTranscript(runtime: TranscriptRuntime, scrollId: string) {
  const block = runtime
    .getSnapshot()
    .window.blocks.find((candidate) => candidate.key.kind === "item")
  if (!block || !("renderItem" in block))
    throw new Error("large benchmark item block is missing")
  assert(
    block.renderItem.kind === "command" || block.renderItem.kind === "tool",
  )
  return (
    <scrollbox id={scrollId} width="100%" height="100%">
...
async function coldGeometrySample(): Promise<
  Readonly<{ nativeFirstFrameMs: number; runtimeGeometryRebuildMs: number }>
> {
  const runtime = new TranscriptRuntime(
    runtimeInput(largeSource(), largeThread),
  )
  ...
  try {
    ...
    const first = measureRenderedTranscript(setup.renderer, scroll, {
      frame,
      runtime,
      styleRevision,
    })
    ...
    assert.equal(
      first,
      undefined,
      "the first runtime-owned measure publishes geometry before it returns a layout",
    )
    assert.equal(published.geometry.measuredBlockCount, 1)
    assert.equal(
      published.geometry.totalPoints,
      published.transcript.projectionById[largeItemId]!.sourceSpans.length + 1,
    )
    assert(
      layout,
      "the published geometry must build a layout on the next read",
    )
...
async function benchmarkColdGeometry(): Promise<void> {
  const warmups = 1,
    samples = 12
  const nativeFirstFrames: number[] = [],
    runtimeGeometryRebuilds: number[] = []
  ...
  printResult({
    ...
    viewport,
    fixture: {
      contentShape: "single-expanded-command",
      items: 1,
      blocks: 1,
      kind: "command",
      status: "complete",
      lines: 1500,
      chars: largeDetail.length,
      folded: false,
      cacheScope:
        "fresh runtime, renderer, and scroll caches per sample; process and JIT warm after discarded sample",
    },
...
async function benchmarkWarmNavigation(): Promise<void> {
  ...
  try {
    ...
    const sample = async (record: boolean) => {
      ...
      if (record) {
        frames.push(frameMs)
        layouts.push(layoutMs)
        anchors.push(anchorMs)
      }
    }
    ...
    printResult({
      ...
      fixture: {
        ...
        chars: largeDetail.length,
        folded: false,
        scroll: { direction: "down", stepRows: 15 },
        cacheScope:
          "runtime geometry and scroll caches warmed before recorded samples",
      },
      samples: { warmup: warmups, measured: samples },
      metricsMs: {
        nativeFrame: stats(frames),
        cachedLayout: stats(layouts),
        visibleAnchor: stats(anchors),
      },
    })
  } finally {
    runtime.dispose()
...
function followSources(): Readonly<{ before: Source; after: Source }> {
  let source: Source = Object.freeze({
    conversation: createConversation(followThread),
    transcript: initialTranscript(),
    revision: 0,
  })
  source = apply(source, {
...
function syntheticGeometry(
  ...
  return runtime.getSnapshot().blocks.map((block) =>
    Object.freeze({
      key: Object.freeze({
...
function followReconciliationSample(
  sources: ReturnType<typeof followSources>,
): number {
  const runtime = new TranscriptRuntime(
    runtimeInput(sources.before, followThread),
  )
  try {
    runtime.reportMeasurements({
      ...runtime.measurementBase(),
      measurements: syntheticGeometry(runtime),
    })
    const before = runtime.getSnapshot()
    assert.equal(before.blocks.length, 301)
    assert.equal(before.geometry.measuredBlockCount, 301)
    const blocksByKey = new Map(
      before.blocks.map((block) => [blockKey(block), block]),
    )
    const geometryByKey = new Map(Object.entries(before.geometry.byBlockKey))
    const tailKey = `item:${followTail}:root`
    const afterInput = runtimeInput(sources.after, followThread, {
      kind: "blocks",
      itemIds: [followTail],
    })
    ...
    const changed = after.blocks
      .filter((block) => blocksByKey.get(blockKey(block)) !== block)
      .map(blockKey)
    assert.deepEqual(
      changed,
      [tailKey],
      "follow reconciliation must replace only the streaming tail block",
    )
    for (const block of after.blocks) {
      const key = blockKey(block)
      if (key === tailKey) continue
      assert.equal(
        after.geometry.byBlockKey[key],
        geometryByKey.get(key),
        `history geometry identity changed for ${key}`,
      )
    }
    assert.equal(
      after.geometry.byBlockKey[tailKey],
      undefined,
      "changed tail geometry must be invalidated",
    )
...
function benchmarkFollowReconciliation(): void {
  ...
  const totalChars = (source: Source) =>
    Object.values(source.transcript.projectionById).reduce(
      (sum, projection) => sum + projection.source.length,
      0,
    )
  ...
  printResult({
    ...
    fixture: {
      ...
      charsBefore:
        sources.before.transcript.projectionById[followTail]!.source.length,
      charsAfter:
        sources.after.transcript.projectionById[followTail]!.source.length,
      ...
      modeAfter: "follow",
      cacheScope:
        "fresh runtime per sample; canonical projection and geometry seeding excluded from timing",
    },
    samples: { warmup: warmups, measured: samples },
