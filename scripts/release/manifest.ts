...
for (const file of (await readdir(directory))
  .filter((name) => /^vimex-v.*-(darwin|linux)-(arm64|x64)\.json$/.test(name))
  .sort()) {
  const fragment = parseRelease(
    JSON.parse(await readFile(join(directory, file), "utf8")),
  )
  if (fragment.version !== version)
    throw new Error(`Mismatched artifact version: ${file}`)
  for (const artifact of fragment.artifacts) {
    ...
    if (artifact.name !== expected)
      throw new Error(`Unexpected artifact filename: ${artifact.name}`)
    const sha = createHash("sha256")
      .update(await readFile(join(directory, artifact.name)))
      .digest("hex")
...
if (release.artifacts.length !== 4)
  throw new Error("Publishing requires all four native platform artifacts")
await writeFile(
  join(directory, "vimex-release.json"),
