...
The tag triggers publication. Monitor that workflow rather than manually duplicating its release uploads or formula changes. Check that the release contains the expected platform archives, manifest, checksums, installer, and provenance. For stable releases, verify that the formula on main references the published version and hashes; prereleases must leave the stable formula unchanged.
