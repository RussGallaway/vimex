...
export class RpcClient {
  ...
  onNotification(
    listener: (notification: ServerNotificationMessage) => void,
  ): () => void {
  ...
  private receive(value: unknown): void {
    ...
    if (routed.type === "request") {
      for (const listener of this.requestListeners) listener(routed.value)
      return
    }
    if (routed.type === "notification") {
      for (const listener of this.notificationListeners) listener(routed.value)
      return
    }
    ...
    if (
      isRecord(routed.value.error) &&
      typeof routed.value.error.code === "number" &&
      typeof routed.value.error.message === "string"
    ) {
      pending.reject(
        new RpcRequestError(
          pending.method,
          routed.value.error.code,
          routed.value.error.message,
          routed.value.error.data,
        ),
      )
