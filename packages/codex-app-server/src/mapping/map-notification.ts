...
export type CodexAdapterEvent =
  ...
  | {
      type: "thread.goal"
      threadId: ReturnType<typeof threadId>
      goal: import("@vimex/conversation").ThreadGoal | null
    }
  | {
      type: "connection"
...
export function mapNotification(
  ...
  switch (notification.method) {
    ...
    case "thread/started": {
      ...
      return isThreadLike(thread)
        ? {
            ...
            relation: mapThreadRelation(thread),
          }
    ...
    case "thread/status/changed":
      if (typeof params.threadId === "string" && isThreadStatus(params.status))
        return {
          type: "thread.status",
          threadId: threadId(params.threadId),
    ...
    case "turn/completed": {
      const turn = params.turn
      const outcome = isRecord(turn)
        ? mapTerminalTurnStatus(turn.status)
        : undefined
      if (
        typeof params.threadId === "string" &&
        isRecord(turn) &&
...
export function mapNotificationEvents(
  ...
  if (
    ...
    isRecord(params) &&
    isThreadLike(params.thread)
  ) {
    const relation = mapThreadRelation(params.thread)
    if (relation.parentThreadId)
      events.push({
        type: "subagent.link",
        link: {
          ownerThreadId: relation.parentThreadId,
          agentThreadId: relation.threadId,
          itemId: itemId(`thread:${relation.threadId}`),
          relation: "spawned",
        },
      })
  }
  if (
    ...
    const normalized =
      ...
      (primary.event.type === "item.started" ||
        primary.event.type === "item.completed")
        ? primary.event.item
        : undefined
    if (normalized?.kind === "agent" && normalized.action === "activity")
      events.push({
        ...
        link: {
          ...
          agentThreadId: normalized.agentThreadIds[0]!,
...
function safeMapThreadItem(
  value: Record<string, unknown>,
  ownerTurnId: string,
  completed: boolean,
) {
  try {
    return mapThreadItem(value as ThreadItem, ownerTurnId, completed)
  } catch {
    return undefined
  }
}
