...
test("uses command action metadata without hiding mixed unknown operations", () => {
  ...
  expect(
    commandTitle("raw", [{ type: "listFiles", command: "ls", path: "/repo" }]),
  ).toBe("List files · /repo")
  expect(
    commandTitle("raw", [
      { type: "search", command: "rg needle", query: "needle", path: "src" },
    ]),
  ).toBe("Search · needle · src")
  expect(
    commandTitle("raw", [
      { type: "read", command: "cat a", name: "a", path: "a" },
      { type: "unknown", command: "rm b" },
    ]),
  ).toBe("Read a · rm b")
})

test("conservatively unwraps literal shell commands and retains ambiguous syntax", () => {
  expect(displayCommand("/bin/zsh -lc 'git status --short'")).toBe(
    "git status --short",
  )
  ...
  for (const command of [
    ...
    expect(displayCommand(command)).toBe(command)
})
