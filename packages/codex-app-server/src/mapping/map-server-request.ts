...
export type ServerRequestEvent =
  ...
  | {
      ...
      isBlocking: boolean
      questions: readonly NormalizedUserQuestion[]
    }
  ...
  switch (request.method) {
    case "item/commandExecution/requestApproval": {
      ...
      return {
        ...
        choices: commandChoices(params.availableDecisions),
        status: "pending",
      }
    }
    case "item/fileChange/requestApproval": {
      ...
      return {
        ...
        detail: [params.reason, params.grantRoot].filter(Boolean).join("\n"),
        choices: defaultDecisionChoices,
        status: "pending",
      }
    }
    case "item/permissions/requestApproval": {
      ...
      return {
        ...
        detail: [params.reason, params.cwd, safeStringify(params.permissions)]
          .filter(Boolean)
          .join("\n"),
        choices: [
          { id: "grant-turn", label: "Grant for turn" },
          { id: "grant-session", label: "Grant for session" },
    ...
    case "execCommandApproval": {
      ...
      return {
        id: requestId,
        threadId: threadId(params.conversationId),
        kind: "legacy",
        title: params.command.join(" "),
        detail: [params.reason, params.cwd].filter(Boolean).join("\n"),
        choices: legacyDecisionChoices,
        status: "pending",
      }
    }
    case "applyPatchApproval": {
      ...
      return {
        ...
        kind: "legacy",
        title: "Apply patch",
        detail: [params.reason, ...Object.keys(params.fileChanges)]
          .filter(Boolean)
          .join("\n"),
        choices: legacyDecisionChoices,
        status: "pending",
...
const legacyDecisionChoices: readonly ApprovalChoice[] = [
  ...
  { id: "approved_for_session", label: "Approve for session" },
  { id: "denied", label: "Deny" },
  { id: "abort", label: "Abort" },
]
function commandChoices(
  decisions: CommandExecutionRequestApprovalParams["availableDecisions"],
): readonly ApprovalChoice[] {
  if (!decisions?.length) return defaultDecisionChoices
  return decisions.map((decision) => {
    if (typeof decision === "string")
      return { id: decision, label: labelDecision(decision) }
    return {
      id: JSON.stringify(decision),
      label:
        "acceptWithExecpolicyAmendment" in decision
          ? "Accept with command policy change"
          : "Apply network policy change",
    }
  })
}
