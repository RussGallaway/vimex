...
export function commandTitle(
  ...
  return actions
    .map((action) => {
      switch (action.type) {
        case "read":
          return `Read ${action.path || action.name}`
        case "listFiles":
          return action.path ? `List files · ${action.path}` : "List files"
        case "search":
          return ["Search", action.query, action.path]
            .filter(Boolean)
            .join(" · ")
        case "unknown":
          return displayCommand(action.command || command)
        default:
