  ...
  switch (item.type) {
    ...
    case "collabAgentToolCall": {
      if (!isAgentTool(item.tool) || typeof item.senderThreadId !== "string" || !Array.isArray(item.receiverThreadIds) || !item.receiverThreadIds.every(id => typeof id === "string")) {
        return { id, turnId: owner, kind: "unknown", title: "Invalid agent activity", detail: safeStringify(item), status: fallbackStatus, transcript: "diagnostic" }
      }
      const action = mapAgentAction(item.tool)
      const states = isRecord(item.agentsStates) ? item.agentsStates : {}
      return {
        id, turnId: owner, kind: "agent", action,
        detail: typeof item.prompt === "string" ? item.prompt : "", status: mapItemStatus(item.status, fallbackStatus), senderThreadId: threadId(item.senderThreadId),
        agentThreadIds: item.receiverThreadIds.map(threadId),
        agentStates: Object.entries(states).flatMap(([id, state]) => isRecord(state) && isAgentStateStatus(state.status) ? [{ threadId: threadId(id), status: mapAgentStateStatus(state.status), ...(typeof state.message === "string" && state.message ? { message: state.message } : {}) }] : []),
      }
    }
    ...
    case "subAgentActivity": return typeof item.agentThreadId === "string" && typeof item.agentPath === "string" && isAgentActivity(item.kind) ? {
      id, turnId: owner, kind: "agent", action: "activity", activity: item.kind,
      detail: "", agentThreadIds: [threadId(item.agentThreadId)], agentPath: item.agentPath, status: fallbackStatus,
      } : { id, turnId: owner, kind: "unknown", title: "Invalid subagent activity", detail: safeStringify(item), status: fallbackStatus, transcript: "diagnostic" }
...
export function hydrateTurns(turns: readonly Turn[], ownerThreadId: string): ConversationEvent[] {
  ...
  for (const turn of turns) {
    ...
    events.push({ type: "turn.started", threadId: owner, turnId: key, startedAt: timestampMs(turn.startedAt) })
    ...
    if (outcome) events.push({ type: "turn.completed", threadId: owner, turnId: key, outcome, startedAt: timestampMs(turn.startedAt), completedAt: timestampMs(turn.completedAt), durationMs: durationMs(turn.durationMs) })
  }
  return events
}
...
export function mapTerminalTurnStatus(status: unknown): TurnOutcome | undefined {
...
function mapItemStatus(status: unknown, fallback: ItemStatus = "running"): ItemStatus {
  switch (status) {
    case "completed": return "complete"
    case "failed":
...
function mapAgentAction(tool: Extract<ThreadItem, { type: "collabAgentToolCall" }>["tool"]): AgentCoordinationAction {
  switch (tool) {
    case "spawnAgent": return "spawn"
    case "sendInput": return "send-input"
    case "sendMessage": return "send-message"
    case "followupTask": return "follow-up"
    case "resumeAgent": return "resume"
    case "wait": return "wait"
    case "interruptAgent": return "interrupt"
    case "closeAgent": return "close"
    case "listAgents": return "list"
  }
}
function isAgentTool(value: unknown): value is Extract<ThreadItem, { type: "collabAgentToolCall" }>["tool"] {
  return value === "spawnAgent" || value === "sendInput" || value === "sendMessage" || value === "followupTask" || value === "resumeAgent" || value === "wait" || value === "interruptAgent" || value === "closeAgent" || value === "listAgents"
}
...
function isAgentActivity(value: unknown): value is Extract<ThreadItem, { type: "subAgentActivity" }>["kind"] {
  return value === "started" || value === "interacted" || value === "interrupted" || value === "completed"
}
