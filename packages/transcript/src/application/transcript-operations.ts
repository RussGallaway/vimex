...
export function moveCursor(
  ...
  if (
    !projection ||
    !transcriptOrderIndex(state.order).has(point.itemId) ||
    !Number.isInteger(point.graphemeOffset) ||
    !Number.isFinite(preferredScreenRow)
  )
    return state
  point = {
    ...point,
  ...
  const selection = state.selection
    ? state.selection.head.itemId === point.itemId &&
      state.selection.head.graphemeOffset === point.graphemeOffset
      ? state.selection
      : { ...state.selection, head: point }
    : undefined
  if (
    state.cursor?.itemId === point.itemId &&
    state.cursor.graphemeOffset === point.graphemeOffset &&
    state.selection === selection &&
    state.viewport.kind === "point" &&
    state.viewport.point.itemId === point.itemId &&
    state.viewport.point.graphemeOffset === point.graphemeOffset &&
    state.viewport.preferredScreenRow === preferredScreenRow
  )
    return state
...
export function anchorViewport(
  ...
  if (
    !projection ||
    !transcriptOrderIndex(state.order).has(point.itemId) ||
    !Number.isFinite(point.graphemeOffset) ||
    !Number.isFinite(preferredScreenRow)
  )
    return state
  const anchored = {
    ...
    graphemeOffset: Math.max(
      0,
      Math.min(Math.trunc(point.graphemeOffset), projection.sourceSpans.length),
    ),
  }
  preferredScreenRow = Math.trunc(preferredScreenRow)
  if (
  ...
  return {
    ...state,
    viewport: { kind: "point", point: anchored, preferredScreenRow },
  }
}
const JUMP_LIMIT = 100
function validLocation(
  state: TranscriptState,
  location: JumpLocation | undefined,
): location is JumpLocation {
  return Boolean(
    location &&
    state.projectionById[location.point.itemId] &&
    transcriptOrderIndex(state.order).has(location.point.itemId) &&
    Number.isInteger(location.point.graphemeOffset) &&
    Number.isFinite(location.preferredScreenRow),
  )
}
function normalizeLocation(
  ...
  return {
    point: {
      ...
      graphemeOffset: Math.max(
        0,
        Math.min(location.point.graphemeOffset, length),
      ),
    },
...
function currentLocation(state: TranscriptState): JumpLocation | undefined {
  if (state.viewport.kind === "point") {
    const viewport = normalizeLocation(state, {
      point: state.viewport.point,
      preferredScreenRow: state.viewport.preferredScreenRow,
    })
    if (viewport) return viewport
  }
  if (state.cursor) {
    const cursor = normalizeLocation(state, {
      point: state.cursor,
      preferredScreenRow: 0,
    })
    if (cursor) return cursor
  }
  const itemId = state.order.at(-1),
    projection = itemId ? state.projectionById[itemId] : undefined
  return itemId && projection
    ? {
        point: { itemId, graphemeOffset: projection.sourceSpans.length },
        preferredScreenRow: 0,
      }
    : undefined
}
function sameLocation(
  a: JumpLocation | undefined,
  b: JumpLocation | undefined,
): boolean {
  return Boolean(
    a &&
    b &&
    a.point.itemId === b.point.itemId &&
    a.point.graphemeOffset === b.point.graphemeOffset &&
    a.preferredScreenRow === b.preferredScreenRow,
  )
}
function atLocation(
  state: TranscriptState,
...
function jumpTo(
  ...
  if (clearActiveSelection && state.selection)
...
function jumpHistory(
  ...
  if (!target)
    return source.length === state.jumps[direction].length
      ? state
      : { ...state, jumps: { ...state.jumps, [direction]: source } }
  const opposite = direction === "back" ? "forward" : "back"
  const current = normalizeLocation(state, origin) ?? currentLocation(state)
  const destination = current
    ? [...state.jumps[opposite], current].slice(-JUMP_LIMIT)
    : state.jumps[opposite]
  return {
    ...atLocation(state, target),
    jumps: { ...state.jumps, [direction]: source, [opposite]: destination },
  }
}
export function attachTail(state: TranscriptState): TranscriptState {
  ...
  const cursor =
    last && projection
      ? { itemId: last, graphemeOffset: graphemeCount(projection.plain) }
      : state.cursor
  if (
    state.viewport.kind === "tail" &&
    state.unseenEntries === 0 &&
    state.unseenItemIds.length === 0 &&
    state.cursor?.itemId === cursor?.itemId &&
    state.cursor?.graphemeOffset === cursor?.graphemeOffset
  )
    return state
  return {
    ...state,
...
export function beginSelection(
  state: TranscriptState,
  shape: TranscriptSelection["shape"],
): TranscriptState {
  return state.cursor
    ? {
        ...state,
        selection: { anchor: state.cursor, head: state.cursor, shape },
      }
    : state
}
export function swapSelection(state: TranscriptState): TranscriptState {
  ...
  const preferredScreenRow =
    state.viewport.kind === "point" ? state.viewport.preferredScreenRow : 0
  return moveCursor(
    { ...state, selection: { ...state.selection, anchor: head, head: anchor } },
    anchor,
    preferredScreenRow,
  )
}
...
export function setFold(
  ...
  if (
    !transcriptOrderIndex(state.order).has(id) ||
    state.projectionById[id]?.nodeKind === "message"
  )
    return state
  if (Object.hasOwn(state.folded, id) && state.folded[id] === folded)
    return state
...
export function setDefaultFolds(
  ...
  const sameDefaults =
    state.foldDefaults.reasoning === defaults.reasoning &&
    state.foldDefaults.tools === defaults.tools
  if (!additions.length && sameDefaults) return state
  let folded = state.folded
  for (const [itemId, value] of additions)
    folded = setTranscriptFoldValue(folded, itemId, value)
  return {
    ...state,
    folded,
    foldDefaults: sameDefaults
      ? state.foldDefaults
...
export function selectedText(
  ...
  const forward =
    comparePoint(state, state.selection.anchor, state.selection.head) <= 0
...
export function selectedGraphemeCount(
  ...
  return (
    startTo - startFrom + middle + (endTo - endFrom) + (endIndex - startIndex)
  )
}
export function urlAt(
  ...
  return state.projectionById[point.itemId]?.links.find(
    (link) =>
      point.graphemeOffset >= link.from && point.graphemeOffset < link.to,
  )?.url
}
export function reduceTranscript(
  ...
  switch (command.type) {
    case "search.set":
      return state.search?.query === command.query &&
        state.search.direction === command.direction
        ? state
        : {
            ...state,
            search: { query: command.query, direction: command.direction },
          }
    case "search.jump": {
      const searched =
        command.search &&
        (state.search?.query !== command.search.query ||
          state.search.direction !== command.search.direction)
          ? {
              ...state,
              search: {
                query: command.search.query,
                direction: command.search.direction,
              },
            }
          : state
    ...
    case "cursor.move":
      return moveCursor(state, command.point, command.preferredScreenRow)
    case "cursor.reveal":
      return moveCursor(
        setFold(state, command.point.itemId, false),
        command.point,
        command.preferredScreenRow,
      )
    case "jump.to":
      return jumpTo(
        state,
    ...
    case "jump.forward":
      return jumpHistory(state, "forward", command.origin)
    case "mark.set": {
      const target = normalizeLocation(state, command.target)
      return target && /^[a-zA-Z]$/.test(command.name)
        ? { ...state, marks: { ...state.marks, [command.name]: target } }
        : state
    }
    case "mark.jump": {
    ...
    case "viewport.anchor":
      return anchorViewport(state, command.point, command.preferredScreenRow)
    case "tail.attach":
    ...
    case "fold.set":
      return setFold(state, command.itemId, command.folded)
    case "fold.toggle":
