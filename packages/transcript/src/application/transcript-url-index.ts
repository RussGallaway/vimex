...
function lowerBound(links: readonly LinkTarget[], offset: number, field: "from" | "to", inclusive: boolean): number {
  let low = 0, high = links.length
  while (low < high) {
...
function node(count: number, left?: UrlCountNode, right?: UrlCountNode): UrlCountNode {
  return Object.freeze({ count, ...(left ? { left } : {}), ...(right ? { right } : {}) })
}

function buildNode(counts: readonly number[], from: number, to: number): UrlCountNode | undefined {
...
function buildIndex(state: TranscriptState, diagnostics?: TranscriptUrlIndexDiagnostics): TranscriptUrlIndex {
  ...
  return Object.freeze({ order: state.order, root: buildNode(counts, 0, capacity), size: counts.length, capacity })
}

function indexFor(state: TranscriptState, diagnostics?: TranscriptUrlIndexDiagnostics): TranscriptUrlIndex {
...
export function primeTranscriptUrlIndex(state: TranscriptState, diagnostics?: TranscriptUrlIndexDiagnostics): void {
...
function prefix(nodeValue: UrlCountNode | undefined, from: number, to: number, end: number, diagnostics?: TranscriptUrlIndexDiagnostics): number {
  if (!nodeValue || end <= from) return 0
  if (diagnostics) diagnostics.urlIndexNodeVisits += 1
  if (to <= end) return nodeValue.count
  const middle = (from + to) >>> 1
  return prefix(nodeValue.left, from, middle, end, diagnostics)
    + prefix(nodeValue.right, middle, to, end, diagnostics)
}

function itemAtOrdinal(nodeValue: UrlCountNode | undefined, from: number, to: number, ordinal: number, diagnostics?: TranscriptUrlIndexDiagnostics): number | undefined {
  ...
  return ordinal < leftCount
    ...
    : itemAtOrdinal(nodeValue.right, middle, to, ordinal - leftCount, diagnostics)
}
...
export function inheritTranscriptUrlIndexChanges(
  ...
  if (previous.order !== next.order && (previous.order.length !== next.order.length
    || previous.order.some((itemId, position) => next.order[position] !== itemId))) return
  ...
  for (const itemId of new Set(changedItemIds)) {
    ...
    root = updateNode(root, 0, prior.capacity, position, next.projectionById[itemId]?.links.length ?? 0)
    if (diagnostics) diagnostics.urlIndexUpdates += 1
  }
  urlIndexes.set(next.projectionById, Object.freeze({ order: next.order, root, size: next.order.length, capacity: prior.capacity }))
}
...
export function indexedUrlTarget(
  ...
  const before = prefix(index.root, 0, index.capacity, itemPosition, diagnostics)
  const insertion = lowerBound(projection.links, point.graphemeOffset, "from", true)
  const preceding = insertion - 1
  const containing = preceding >= 0 && point.graphemeOffset < projection.links[preceding]!.to ? preceding : -1
  let base: number
  if (containing >= 0) base = before + containing
  else if (direction === "forward") {
    base = before + insertion - 1
  } else {
    const previous = lowerBound(projection.links, point.graphemeOffset, "from", false) - 1
  ...
  const targetPosition = itemAtOrdinal(index.root, 0, index.capacity, ordinal, diagnostics)
  if (targetPosition === undefined) return undefined
  const itemId = state.order[targetPosition]
  if (!itemId) return undefined
  const localOrdinal = ordinal - prefix(index.root, 0, index.capacity, targetPosition, diagnostics)
...
export function indexedUrlTargets(
  ...
  const total = index.root?.count ?? 0
  if (!total) return []
  let fromOrdinal = 0, toOrdinal = total
  if (range) {
    const order = transcriptOrderIndex(state.order)
    const fromItem = order.get(range.from.itemId), toItem = order.get(range.to.itemId)
    if (fromItem === undefined || toItem === undefined || fromItem > toItem) return []
    const fromLinks = state.projectionById[range.from.itemId]?.links ?? []
    const toLinks = state.projectionById[range.to.itemId]?.links ?? []
    fromOrdinal = prefix(index.root, 0, index.capacity, fromItem, diagnostics)
      + lowerBound(fromLinks, range.from.graphemeOffset, "to", true)
    toOrdinal = prefix(index.root, 0, index.capacity, toItem, diagnostics)
  ...
  for (let ordinal = fromOrdinal; ordinal < toOrdinal; ordinal++) {
    const itemPosition = itemAtOrdinal(index.root, 0, index.capacity, ordinal, diagnostics)
    if (itemPosition === undefined) break
    const itemId = state.order[itemPosition]
    if (!itemId) continue
    const local = ordinal - prefix(index.root, 0, index.capacity, itemPosition, diagnostics)
