...
describe("canonical Markdown source projection", () => {
  ...
  test("matches CommonMark delimiter rules for intraword underscores and nested emphasis", () => {
    ...
    expect(projectMarkdown("***bold*** and **a *nested* value**").plain).toBe("bold and a nested value")
  })
  ...
  test("normalizes code-span spaces and keeps their complete source envelope", () => {
    ...
    let state = syncTranscriptItem(initialTranscript(), message("code", markdown))
  ...
  test("resolves full, collapsed, and shortcut reference links", () => {
    const markdown = "[one][target] [two][] [three]\n\n[target]: https://one.test\n[two]: https://two.test\n[three]: https://three.test"
    const projection = projectMarkdown(markdown)
    expect(projection.plain).toBe("one two three\n\n")
    expect(projection.links.map(({ url }) => url)).toEqual(["https://one.test", "https://two.test", "https://three.test"])
  })

  test("resolves multiline reference destinations and titles", () => {
    const markdown = "Read [the guide][docs].\n\n[docs]:\n  <https://example.test/guide>\n  \"Guide title\""
    const projection = projectMarkdown(markdown)
    expect(projection.plain).toBe("Read the guide.\n\n")
    expect(projection.links).toEqual([{ from: 5, to: 14, url: "https://example.test/guide" }])
    let state = syncTranscriptItem(initialTranscript(), message("reference", markdown))
    state = moveCursor(state, { itemId: itemId("reference"), graphemeOffset: 5 })
    state = beginSelection(state, "character")
    state = moveCursor(state, { itemId: itemId("reference"), graphemeOffset: 13 })
  ...
  test("normalizes line endings inside multiline code spans", () => {
    ...
    expect(projection.sourceRegions).toContainEqual({ from: 7, to: 17, sourceFrom: 7, sourceTo: 19 })
  })

  test("preserves entities literally to match OpenTUI's Marked text tokens", () => {
    expect(projectMarkdown("A &copy; &eacute; &NotEqualTilde; &#x1F600;").plain)
      .toBe("A &copy; &eacute; &NotEqualTilde; &#x1F600;")
  })

  test("only consumes CommonMark punctuation escapes", () => {
    expect(projectMarkdown(String.raw`C:\Users \alpha \*literal*`).plain).toBe(String.raw`C:\Users \alpha *literal*`)
  })
  ...
  test("projects thematic breaks as the stable native rule glyph with exact source mapping", () => {
    for (const marker of ["---", "***", "___", "- - -", "* * *", "_ _ _", "  ---  "]) {
      ...
      expect(projection.source.slice(projection.sourceSpans[rule]!.from, projection.sourceSpans[rule]!.to)).toBe(marker)
    }
  })
