...
import { initialWorkbench, transitionWorkbench, type AvailableModel } from "@vimex/workbench"
import { VimexRoot } from "../index"
import { inertController, type VimexUiController } from "../contracts"

for (const scenario of [
  { name: "loading", models: undefined, error: undefined, text: "Loading models" },
  { name: "empty", models: [] as readonly AvailableModel[], error: undefined, text: "No models available" },
  { name: "failed", models: undefined, error: "Service unavailable", text: "Could not load models" },
]) {
  test(`model picker stays open on Enter when ${scenario.name}`, async () => {
    ...
    let initial = transitionWorkbench(initialWorkbench(), { type: "thread.open", summary: {
      id, title: "Models", cwd: "/work", model: "test", reasoningEffort: "high", status: "idle",
    } }).state
    initial = transitionWorkbench(initial, { type: "interaction.command", command: { type: "overlay.open", overlay: "models" } }).state
    initial = { ...initial, availableModels: scenario.models, modelCatalogError: scenario.error }
    ...
    try {
      ...
      await act(async () => { setup.mockInput.pressKey("RETURN"); await setup.flush() })
