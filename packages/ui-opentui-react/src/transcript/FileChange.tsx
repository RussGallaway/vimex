...
export function FileChange(props: { item: Extract<ConversationItem, { kind: "edit" }>; sourceItem?: Extract<ConversationItem, { kind: "edit" }>; folded: boolean; syntax: SyntaxStyle; blockId?: string; fragment?: TranscriptItemFragment }) {
  ...
  const fragmented = props.fragment?.kind === "edit-header" || props.fragment?.kind === "edit-file"
  ...
  const changes = fragmented && item.changes ? item.changes
    : item.changes?.map(change => change.patch).filter(Boolean).join("\n") === item.patch
  ...
  return <box backgroundColor={emberTide.backgroundRaised} paddingX={1}
    paddingTop={props.folded || !firstFragment ? 0 : 1} paddingBottom={props.folded || !finalFragment ? 0 : 1}>
    {firstFragment ? <box id={`decoration:edit-header:${item.id}${suffix}`} height={1} flexDirection="row" gap={1}>
      ...
      <text fg={emberTide.text} flexGrow={1} flexShrink={1} minWidth={0} wrapMode="none" truncate>{sourceItem.changes && sourceItem.changes.length > 1 ? `${sourceItem.changes.length} files changed` : sourceItem.title}</text>
    ...
    {!props.folded ? changes.map((change, index) => {
      ...
      return <box key={`${index}:${change.path}`}>
        {item.changes ? <box id={`decoration:edit-file:${item.id}${suffix}:${index}`} flexDirection="row" gap={1}>
        ...
        {change.patch ? <diff id={index === 0 ? `diff:${item.id}${suffix}` : `diff:${item.id}${suffix}:${index}`} diff={change.patch} view={width >= 120 ? "split" : "unified"} filetype={diffFiletype(change.movePath ?? change.path)} syntaxStyle={props.syntax}
          ...
          addedLineNumberBg={emberTide.diffAdded} removedLineNumberBg={emberTide.diffRemoved} /> : <text id={`decoration:edit-empty:${item.id}${suffix}:${index}`} fg={emberTide.textMuted}>No textual diff available.</text>}
      </box>
    }) : null}
  </box>
}
