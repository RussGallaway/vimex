...
for (const folded of [true, false]) {
  test(`command ${folded ? "collapsed" : "expanded"} shows concise fold header and exact body`, async () => {
    ...
    try {
      ...
      if (folded) {
        expect(
          h.renderer.root.findDescendantById(`command-source:${command.id}`),
        ).toBeUndefined()
        expect(
          h.renderer.root.findDescendantById(`tool-output:${command.id}`),
        ).toBeUndefined()
      } else {
        expect(
          (
        ...
        ).toBe(command.executionCommand!)
        expect(
          (
            h.renderer.root.findDescendantById(
              `tool-output:${command.id}`,
            ) as TextRenderable
          ).plainText,
        ).toBe(command.detail)
      }
...
test("only command items render execution command metadata", async () => {
  const h = await testRender(
    <ToolCall item={{ ...command, kind: "tool" }} folded={false} />,
    { width: 80, height: 18 },
  )
  try {
    await act(async () => h.flush())
...
test("unknown and zero durations stay out of compact tool headers", async () => {
  const h = await testRender(
    <ToolCall item={{ ...command, durationMs: 0 }} folded />,
    { width: 70, height: 6 },
  )
  try {
    await act(async () => h.flush())
    expect(
      h.renderer.root.findDescendantById(`decoration:duration:${command.id}`),
    ).toBeUndefined()
...
test("command and output cursor geometry excludes matching decorative labels", async () => {
  ...
  state = transitionWorkbench(state, {
    ...
    event: { type: "item.started", threadId: thread, item: command },
  }).state
  const h = await testRender(
  ...
  try {
    ...
    expect(projection.source).toBe(
      [command.title, command.executionCommand, command.detail].join("\n"),
    )
    ...
    const output = h.renderer.root.findDescendantById(
      `tool-output:${command.id}`,
    )!
    const commandStart = graphemes(`${command.title}\n`).length
    const outputStart = graphemes(
...
for (const folded of [false, true])
  test(`truncated long tool title keeps every logical cursor inside the narrow terminal (${folded ? "folded" : "expanded"})`, async () => {
    const thread = threadId("narrow-tool")
    const item = {
      ...command,
      title: "Read " + "longpath/".repeat(15) + "file.ts",
    }
    let state = transitionWorkbench(initialWorkbench(), {
      type: "thread.open",
      summary: {
    ...
    state = transitionWorkbench(state, {
      ...
      command: { type: "fold.set", itemId: item.id, folded },
    }).state
    const h = await testRender(
      <VimexRoot state={state} controller={inertController} />,
      { width: 50, height: 30 },
    ...
    try {
      ...
      if (folded)
        expect(Object.keys(points).length).toBeLessThan(
          graphemes(projection.plain).length,
        )
      else
      ...
      expect(
        Object.values(points).every(
          (point) => point.screenX >= 0 && point.screenX < 50,
        ),
      ).toBe(true)
      expect(projection.source).toBe(
        [item.title, item.executionCommand, item.detail].join("\n"),
      )
      if (!folded) {
        const source = h.renderer.root.findDescendantById(
          `command-source:${item.id}`,
        )!
        const output = h.renderer.root.findDescendantById(
          `tool-output:${item.id}`,
        )!
...
test("native tool output cursor positions count combining graphemes without losing columns", async () => {
  ...
  try {
    ...
    expect(
      [0, 1, 2, 3].map((offset) => points[start + offset]!.screenX - origin),
    ).toEqual([0, 1, 2, 4])
  } finally {
    await act(async () => h.renderer.destroy())
  }
