...
test("Visual line copy omits Markdown syntax and focus exit clears semantic selection", async () => {
  ...
  try {
    ...
    await act(async () => {
      h.mockInput.pressKey("j", { ctrl: true })
      await h.flush()
    })
    await act(async () => {
      await h.flush()
...
test("distant Visual selection clips native highlight to each bounded window", async () => {
  ...
  try {
    ...
    h.renderer.startSelection = (...args) => {
      selectionStarts += 1
      return startSelection(...args)
    }
    h.renderer.updateSelection = (...args) => {
      selectionUpdates += 1
      return updateSelection(...args)
    }
      ...
      ids.forEach((id, index) =>
        h.emit({
          ...
          event: {
            ...
            item: {
              id,
              turnId: turnId(`clip-turn-${index}`),
              kind: "assistant",
              markdown: `clip row ${index}`,
              status: "complete",
            },
          },
        }),
    ...
    for (let count = 0; count < 10 && !h.renderer.getSelection(); count++)
      await act(async () => {
        await Bun.sleep(5)
        await h.flush()
        await h.renderOnce()
    ...
    expect(swappedFrame.transcript.selection).toEqual({
      anchor: end,
      head: start,
      shape: "character",
    })
    expect(swappedFrame.transcript.viewport).toEqual({
      kind: "point",
      point: start,
      preferredScreenRow: 2,
    })
    expect(
      swappedFrame.window.blocks.some(
        (block) =>
          block.key.kind === "item" && block.key.itemId === start.itemId,
      ),
    ).toBe(true)
    ...
    expect(h.workspace().transcript.selection).toEqual({
      anchor: end,
      head: start,
      shape: "character",
    })
    ...
    expect(h.captureCharFrame()).toContain(
      `${graphemeCount(semanticText)} selected`,
    )
...
test("transcript hardware cursor follows Normal motions, Visual head, resize and focus", async () => {
  ...
  try {
    ...
    expect(resized.y).toBeLessThan(
      h.renderer.root.findDescendantById("composer-shell")!.y + 1,
    )
    await act(async () => {
      h.mockInput.pressKey("j", { ctrl: true })
      await h.flush()
    ...
    await act(async () => {
      h.mockInput.pressKey("k", { ctrl: true })
      await h.flush()
    })
    await act(async () => {
      await h.flush()
...
test("batched vwwy observes each mode and motion before copying", async () => {
  ...
  try {
    await h.keys("gg")
    await act(async () => {
      await h.mockInput.pressKeys(["v", "w", "w", "y"], 0)
      await h.flush()
    })
    await h.controller.settle()
...
test("transcript yanks paste into composer Normal mode through the shared Vim register", async () => {
  ...
  try {
    ...
    expect(h.workspace().composer.text).toBe("Alpha\nAlpha bold text.")
    await act(async () => {
      h.controller.changeDraft("before\nafter", 0)
      await h.flush()
    })
    await h.keys("p")
...
test("arrow focus exits transcript Visual selection and composer Visual selection", async () => {
  ...
  const arrow = async (key: string) => {
    await act(async () => {
      h.mockInput.pressArrow(key as "up" | "down")
      await h.flush()
    })
    await act(async () => {
      await h.flush()
  ...
  try {
    ...
    expect(h.workspace().transcript.selection).toBeUndefined()
    await act(async () => {
      h.controller.changeDraft("draft", 0)
      await h.flush()
    })
    await h.keys("vl")
    expect(h.workspace().interaction.mode).toBe("visual")
  ...
  test(`composer ${mode} scrolls transcript with Ctrl-E/Y/D/U without moving its cursor`, async () => {
    ...
    try {
      ...
      for (const [key, direction] of [
        ...
        if (key === "e" || key === "y")
          expect(Math.abs(scrollbox.scrollTop - before)).toBe(1)
  ...
  test(`${encoding} navigate transcript blocks and extend Visual selection`, async () => {
    ...
    try {
      ...
      expect(
        h
          .workspace()
          .transcript.projectionById[answer]!.plain.slice(
            second.graphemeOffset,
          ),
      ).toStartWith("Second")
...
test("Escape interrupts active turns in Normal mode after dismissing editing modes and overlays", async () => {
  ...
  try {
    ...
    await start()
    expect(h.captureCharFrame()).toContain("Working")
    expect(h.captureCharFrame()).not.toContain("stop-reasoning")
    await h.keys("i")
    ...
    expect(h.captureCharFrame()).not.toContain("Reasoning")
    expect(h.captureCharFrame()).toContain("Stopped")
    expect(h.interrupted).toHaveLength(1)
  } finally {
    await h.close()
  }
...
for (const key of ["ctrl-k", "up"] as const)
  test(`${key} enters the bottom visible transcript row after scrolling from the composer`, async () => {
    const h = await visualHarness()
    try {
      await act(async () => {
        h.emit({
      ...
      const measured = measureRenderedTranscript(
        h.renderer,
        scrollbox,
        h.workspace().transcript,
      )!
      const rows = Object.values(measured.points!)
        .flatMap((item) => Object.values(item))
        .map((point) => measuredPoint(measured, point)!)
        .filter(
          (point) =>
            point.screenY >= scrollbox.viewport.screenY &&
            point.screenY <
      ...
      expect(
        measuredPoint(after, h.workspace().transcript.cursor)!.screenY,
      ).toBe(bottom)
...
async function flashKey(
  h: Awaited<ReturnType<typeof visualHarness>>,
  key: string,
  ctrl = false,
) {
  await act(async () => {
    h.mockInput.pressKey(key, { ctrl })
...
test("Flash jumps from Insert composer to labeled transcript text, preserving draft and supporting jump-back", async () => {
  const h = await visualHarness()
  try {
    const transcriptBlock = h.renderer.root.findDescendantById(
      `transcript-block:${answer}:root`,
    ) as Renderable
    await act(async () => {
      await h.flush()
      await h.renderOnce()
...
for (const prefix of ["/", "?"])
  test(`${prefix} preserves Visual selection while searching transcript`, async () => {
    const h = await visualHarness()
    try {
      await h.keys("ggv")
...
for (const prefix of ["/", "?"])
  test(`${prefix} in composer Normal searches transcript without changing draft`, async () => {
    const h = await visualHarness()
    try {
      await flashKey(h, "ARROW_DOWN")
      await act(async () => {
        h.controller.changeDraft("keep this", 3)
        await h.flush()
      })
      await h.keys(prefix + "bold")
      expect(h.workspace().interaction.mode).toBe("command")
