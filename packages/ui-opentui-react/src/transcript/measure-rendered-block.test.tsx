...
function blockFor(markdown: string): TranscriptItemBlock {
  const thread = threadId("measure-thread"),
    turn = turnId("measure-turn"),
    id = itemId("measure-item")
  ...
  if (!block || !("projection" in block))
    throw new Error("Expected an item block")
  return block
}
...
test("measures immutable block-local geometry and reuses unchanged identity", async () => {
  ...
  const setup = await testRender(
    <box id="offset" paddingLeft={5} paddingTop={2}>
      <box id="block" width={8}>
        <text>{block.projection.plain}</text>
      </box>
    </box>,
    { width: 40, height: 10 },
  )
  try {
    ...
    expect(
      Math.max(...Object.values(first.points).map((point) => point.x)),
...
test("native line events dirty only their owning block and preserve identity when geometry is unchanged", async () => {
  ...
  try {
    ...
    expect(takeDirtyRenderedBlocks()).toEqual([
      expect.objectContaining({ renderable }),
    ])
...
test("folded geometry uses boundary sentinels instead of hidden per-grapheme points", async () => {
  ...
  try {
    ...
    const geometry = measureRenderedBlock({
      renderer: setup.renderer,
      renderable,
      block,
      width: 60,
    ...
    expect(geometry.points[block.projection.sourceSpans.length]?.hidden).toBe(
      true,
    )
  } finally {
    await act(async () => setup.renderer.destroy())
...
test("activity-hidden blocks publish zero-row geometry without semantic points", async () => {
  ...
  const setup = await testRender(<box id="block" height={0} flexShrink={0} />, {
    width: 60,
    height: 6,
  })
  try {
...
test("activity batch leads expose only logical boundary sentinels", async () => {
  const block = blockFor(
    "canonical lead text that must not map onto the batch summary",
  )
  const setup = await testRender(
    <box id="block">
      <text>Web research · 3 searches</text>
    </box>,
    { width: 60, height: 6 },
  )
  try {
    ...
    expect(Object.keys(geometry.points).map(Number)).toEqual([
      0,
      block.projection.sourceSpans.length,
    ])
    expect(geometry.points[block.projection.sourceSpans.length]?.hidden).toBe(
      true,
    )
...
test("source-less activity blocks report their complete outer footprint", async () => {
  ...
  const setup = await testRender(
    <box id="activity-block" height={2}>
      <text>Worked for 2s</text>
    </box>,
    { width: 30, height: 8 },
  )
  try {
...
test("empty semantic items retain a synthetic offset-zero native anchor", async () => {
  const block = blockFor("")
  const setup = await testRender(<box id="empty-block" height={1} />, {
    width: 30,
    height: 8,
  })
  try {
    ...
    expect(geometry.points).toEqual({
      0: { graphemeOffset: 0, x: 0, y: 0, row: 0, column: 0 },
    })
...
test("an item sub-block measures only its source span in global logical offsets", async () => {
  ...
  const setup = await testRender(
    <box id="block" width={30} height={1}>
      <text>def</text>
    </box>,
    { width: 30, height: 8 },
  )
...
test("same-size native child replacement invalidates and observes the new tree", async () => {
  ...
  try {
    ...
    expect(
      (renderable.getRenderable("native") as TextRenderable).plainText,
    ).toBe("a\nbcd")
    expect(
      takeDirtyRenderedBlocks().some(
...
test("a remounted native root advances the revision for the same logical block", async () => {
  ...
  function Harness() {
    ...
    return (
      <box>
        {changed ? (
          <box key="new" id="block" width={30} height={2}>
            <text content={"a\nbcd"} />
          </box>
        ) : (
          <box key="old" id="block" width={30} height={2}>
            <text content={"ab\ncd"} />
          </box>
        )}
      </box>
  ...
  try {
    ...
    const firstRoot = setup.renderer.root.findDescendantById(
      "block",
    ) as Renderable
    const first = measureRenderedBlock({
      renderer: setup.renderer,
      renderable: firstRoot,
      block,
      width: 30,
      styleRevision: 1,
      folded: false,
    ...
    const secondRoot = setup.renderer.root.findDescendantById(
      "block",
    ) as Renderable
    const second = measureRenderedBlock({
      renderer: setup.renderer,
      renderable: secondRoot,
      block,
      width: 30,
      styleRevision: 1,
      folded: false,
