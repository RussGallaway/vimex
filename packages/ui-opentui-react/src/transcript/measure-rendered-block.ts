...
interface ScreenCell { readonly char: string; readonly x: number; readonly y: number }
interface TableLayout { readonly columnOffsets: readonly number[]; readonly rowOffsets: readonly number[] }
interface TableCell { readonly textBufferView: TextBufferView }
interface TableRuntime { readonly _layout?: TableLayout; readonly _cells?: readonly (readonly TableCell[])[] }
...
function observeText(state: RenderedBlockState, view: TextBufferRenderable): void {
  if (state.observedText.has(view)) return
  state.observedText.add(view)
  view.on("line-info-change", () => markDirty(state))
}

function observeRenderable(state: RenderedBlockState, renderable: Renderable): void {
  if (renderable === state.renderable || state.observedRenderables.has(renderable)) return
...
function measureRaw(renderer: CliRenderer, renderable: Renderable, text: string): Record<number, NativePoint> {
  const cells = cellsIn(renderer, renderable)
  const parts = graphemes(text)
  const result: Record<number, NativePoint> = {}
  let cellIndex = 0
  let last: ScreenCell | undefined
  if (parts.length === 0 && cells[0]) result[0] = { graphemeOffset: 0, x: cells[0].x, y: cells[0].y }
  for (let offset = 0; offset < parts.length && cellIndex < cells.length; offset += 1) {
  ...
  if (last) result[parts.length] = { graphemeOffset: parts.length, x: last.x + 1, y: last.y }
  return result
}

function fillPointGaps(text: string, points: Record<number, NativePoint>): Record<number, NativePoint> {
...
function foldedPointFallback(textLength: number, points: Record<number, NativePoint>): Record<number, NativePoint> {
  const visible = Object.values(points).sort((a, b) => a.graphemeOffset - b.graphemeOffset)
  ...
  if (textLength > 0) points[textLength] = { ...fallback, graphemeOffset: textLength, hidden: true }
  return points
}
...
function cellPoints(view: TextBufferView, originX: number, originY: number): readonly { x: number; y: number }[] {
  const parts = graphemes(view.getPlainText())
  const info = view.lineInfo
  const candidates: number[] = []
  info.lineSources.forEach((source, index) => { if (source === 0) candidates.push(index) })
  let visualIndex = 0
  let column = 0
  const points: { x: number; y: number }[] = []
  for (let offset = 0; offset <= parts.length; offset += 1) {
    while (visualIndex + 1 < candidates.length && (info.lineStartCols[candidates[visualIndex + 1]!] ?? 0) <= column) visualIndex += 1
    const visual = candidates[visualIndex] ?? 0
    points.push({ x: originX + column - (info.lineStartCols[visual] ?? 0), y: originY + visual })
    column += graphemeCellWidth(parts[offset] ?? "")
  }
  return points
}

function measureTable(table: TextTableRenderable, text: string, fillGaps: boolean): Record<number, NativePoint> {
  ...
  for (let row = 0; row < Math.min(logicalLines.length, cells.length); row += 1) {
    ...
    const pipes: number[] = []
    line.forEach((part, index) => { if (part === "|") pipes.push(index) })
    ...
    for (let columnIndex = 0; columnIndex < Math.min(cells[row]!.length, Math.max(0, pipes.length - 1)); columnIndex += 1) {
      ...
      for (let local = 0; local < logical.length; local += 1) {
        while (sourceOffset < source.length && !sameCell(logical[local]!, { char: [...source[sourceOffset]!][0] ?? source[sourceOffset]! })) sourceOffset += 1
...
function measureMarkdown(renderer: CliRenderer, markdown: MarkdownRenderable, text: string, fillGaps: boolean): Record<number, NativePoint> {
  const result: Record<number, NativePoint> = {}
  let sourceCursor = 0
  let logicalCursor = 0
...
function measureDiff(renderer: CliRenderer, diff: DiffRenderable): Record<number, NativePoint> {
  const runtime = diff as unknown as DiffRuntime
  const sides = { left: runtime.leftCodeRenderable ?? undefined, right: runtime.rightCodeRenderable ?? undefined }
  const sideState = Object.fromEntries(Object.entries(sides).map(([name, side]) => {
    ...
    return [name, { side, lines, starts, cursor: 0, points: side ? measureRaw(renderer, side, side.plainText) : {} }]
  })) as Record<"left" | "right", { side?: TextBufferRenderable; lines: string[]; starts: number[]; cursor: number; points: Record<number, NativePoint> }>
  const result: Record<number, NativePoint> = {}
  ...
  const consume = (sideName: "left" | "right", content: string, at: number, map: boolean) => {
    ...
    let row = -1
    for (let index = side.cursor; index < side.lines.length; index += 1) if (side.lines[index] === content) { row = index; break }
    if (row < 0) return
    side.cursor = row + 1
    if (!map) return
    ...
    const first = side.points[base] ?? (side.side && visualRow >= 0
      ? { graphemeOffset: base, x: side.side.screenX, y: side.side.screenY + visualRow }
      : undefined)
...
function measureItem(renderer: CliRenderer, renderable: Renderable, itemId: string, blockId: string, text: string, fillGaps: boolean): Record<number, NativePoint> {
  ...
  if (diffs.length) {
    ...
    for (const diff of diffs) {
      ...
      if (start < 0) continue
      logicalCursor += graphemes(text.slice(sourceCursor, start)).length
      for (const [offset, point] of Object.entries(measureDiff(renderer, diff))) {
...
function sameKey(left: BlockGeometry["key"], right: BlockGeometry["key"]): boolean {
...
function sameGeometryShape(left: BlockGeometry, right: BlockGeometry): boolean {
  if (left.rows !== right.rows || left.lines.length !== right.lines.length) return false
  ...
  for (const offset of leftOffsets) {
    ...
    if (!a || !b || a.graphemeOffset !== b.graphemeOffset || a.row !== b.row || a.column !== b.column || a.hidden !== b.hidden) return false
  }
  return left.lines.every((line, index) => {
    const other = right.lines[index]
    return Boolean(other && line.from === other.from && line.to === other.to && line.row === other.row)
  })
}

/** Measure one mounted render block into immutable, renderer-neutral local geometry. */
export function measureRenderedBlock(input: MeasureRenderedBlockInput): BlockGeometry {
  ...
  if ("item" in input.block && range.to < input.block.projection.sourceSpans.length) delete nativePoints[textLength]

  const points: Record<number, { graphemeOffset: number; x: number; y: number; row: number; column: number; hidden?: boolean }> = {}
  ...
  const lines = [...byRow.entries()]
    ...
    .map(([row, offsets]) => ({ from: Math.min(...offsets), to: Math.max(...offsets), row }))
  ...
  state.dirty = false
  dirtyStates.delete(state)
  if (state.geometry && sameKey(state.geometry.key, key) && sameGeometryShape(state.geometry, measured)) return state.geometry
