...
test("split diff geometry maps repeated context and header-like code by hunk side", async () => {
  ...
  try {
    ...
    const points = measureRenderedTranscript(setup.renderer, scroll, state)!
      .points![item.id]!
    const offset = (token: string, from = 0) =>
      graphemeCount(patch.slice(0, patch.indexOf(token, from)))
...
test("split padding does not capture later blank changed lines", async () => {
  ...
  try {
    ...
    const points = measureRenderedTranscript(setup.renderer, scroll, state)!
      .points![item.id]!
    const contextOffset = graphemeCount(
      patch.slice(0, patch.indexOf("CONTEXT")),
    )
    const blankRemoveOffset = graphemeCount(
      patch.slice(0, patch.indexOf("\n-\n") + 1),
    )
    const blankAddOffset = graphemeCount(
      patch.slice(0, patch.indexOf("\n+\n") + 1),
    )
    expect(points[blankRemoveOffset]!.screenY).toBeGreaterThan(
      points[contextOffset]!.screenY,
    )
    expect(points[blankAddOffset]!.screenY).toBe(
      points[blankRemoveOffset]!.screenY,
    )
  } finally {
    syntax.destroy()
    await act(async () => setup.renderer.destroy())
