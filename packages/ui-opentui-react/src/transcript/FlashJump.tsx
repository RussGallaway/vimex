...
export function FlashJump(props: {
  ...
  const results = viewport
    ? flashTargets(props.transcript, props.layout, viewport, query, page)
    : { matches: [], labels: [], pages: 1 }
  useEffect(() => {
    input.current?.focus()
  }, [])
  const jump = (target?: FlashTarget) => {
    ...
    let current = measureRenderedTranscript(
      renderer,
      scroll,
      sourceFrame
        ? {
            ...
            styleRevision: props.styleRevision ?? "default",
          }
        : props.transcript,
    )
    const latestFrame = props.runtime?.getSnapshot()
    if (latestFrame && latestFrame !== sourceFrame)
      current = measureRenderedTranscript(renderer, scroll, {
        frame: latestFrame,
        runtime: props.runtime,
        styleRevision: props.styleRevision ?? "default",
      })
    ...
    if (
      ...
      point.screenY >= scroll.viewport.screenY + scroll.viewport.height
    )
      return
    const origin =
      props.fromComposer && current
        ? bottomVisiblePoint(current, scroll)
        : undefined
    flushSync(() => {
      props.controller.transcript({
        ...
        originPreferredScreenRow: origin
          ? origin.screenY - scroll.viewport.screenY
          : undefined,
      })
  ...
  useBindings(
    () => ({
      ...
      bindings: [
        ...
        {
          key: "return",
          cmd: () => jump(results.labels[0] ?? results.matches[0]),
        },
        {
          key: "tab",
          cmd: () =>
            flushSync(() => setPage((value) => (value + 1) % results.pages)),
        },
      ],
    }),
  ...
  return (
    <>
      {results.labels.map((target) => (
        <text
          key={`${target.itemId}:${target.graphemeOffset}`}
          id={`flash-label:${target.label}`}
          position="absolute"
          left={target.screenX - dimensions.x}
          top={target.screenY - dimensions.y}
          height={1}
          width={1}
          zIndex={50}
      ...
      >
        ...
        <input
          ...
          onInput={(value) => {
            const label =
              value.startsWith(query) && value.length === query.length + 1
                ? results.labels.find(
                    (target) => target.label === value.slice(-1),
                  )
                : undefined
            if (label) jump(label)
            else
        ...
        <text flexShrink={0} fg={emberTide.textMuted}>
          {dimensions.width < 70
            ? "↵ jump · Esc"
            : `${query ? results.matches.length + " matches · " : ""}Enter jump · Tab labels · Esc cancel`}
        </text>
      </box>
    </>
  )
