...
test("sent user panels preserve exact selectable Markdown without a role label through narrow reflow", async () => {
  ...
  const h = await testRender(<TranscriptViewport window={{ blocks, activityBatches: [], activityBatchByItem: {}, activityPresentation: {}, topSpacerRows: 2, bottomSpacerRows: 3, overscanRows: 1 }} state={state} surface="transcript" syntax={syntax} scrollRef={scrollRef} />, { width: 80, height: 20 })
  try {
