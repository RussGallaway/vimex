...
test("sent user panels preserve exact selectable Markdown without a role label through narrow reflow", async () => {
  const item = {
    ...
    kind: "user" as const,
    markdown:
      "You asked for **clearer** text 🙂 with a longer line that wraps.",
  ...
  const to = {
    itemId: item.id,
    graphemeOffset: graphemeCount(base.projectionById[item.id]!.plain) - 1,
  }
  const state = {
    ...base,
    cursor: from,
    selection: { anchor: from, head: to, shape: "character" as const },
  }
  const scrollRef = createRef<ScrollBoxRenderable>()
  const syntax = createEmberTideSyntax()
  const blocks = [
    {
      key: { kind: "item" as const, itemId: item.id, blockId: "root" as const },
      turnId: item.turnId,
      item,
      renderItem: item,
      projection: state.projectionById[item.id]!,
      sourceSpan: { from: 0, to: item.markdown.length },
      contentRevision: 1,
      estimatedRows: 1,
  ...
  try {
    for (const width of [80, 38]) {
      ...
      expect(
        h.renderer.root.findDescendantById(`decoration:user-label:${item.id}`),
      ).toBeUndefined()
      expect(
        h.renderer.root.findDescendantById("transcript-top-spacer")?.height,
      ).toBe(2)
      expect(
        h.renderer.root.findDescendantById("transcript-bottom-spacer")?.height,
      ).toBe(3)
      ...
      expect(selectedText(state, "plain")).toBe(
        "You asked for clearer text 🙂 with a longer line that wraps.",
      )
