...
export function flashTargets(state: TranscriptState, layout: TranscriptLayout, viewport: FlashViewport, query: string, page = 0) {
  ...
  const visible = (p: MeasuredPoint | undefined): p is MeasuredPoint => Boolean(p && !p.hidden && p.screenX >= viewport.screenX && p.screenX < viewport.screenX + viewport.width && p.screenY >= viewport.screenY && p.screenY < viewport.screenY + viewport.height)
  ...
  for (let offset = low; offset < index.length; offset++) {
    ...
    if (!text) { text = graphemes(projection.plain); textCache.set(projection, text) }
    if (/\s/u.test(text[raw.graphemeOffset] ?? "")) continue
    const start = raw.graphemeOffset
    if (!needle.every((part, i) => text![start + i] !== undefined && compare(text![start + i]!) === compare(part)
      && visible(measuredPoint(layout, { itemId: raw.itemId, graphemeOffset: start + i })))) continue
  ...
  const labels = keys.length ? matches.slice((page % pages) * size, (page % pages + 1) * size).map((point, i) => ({ ...point, label: keys[i]! })) : []
