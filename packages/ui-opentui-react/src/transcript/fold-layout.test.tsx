...
test("folding one item reuses and translates unchanged item geometry", async () => {
  ...
  for (const item of items)
    base = {
      ...base,
      order: [...base.order, item.id],
      projectionById: { ...base.projectionById, [item.id]: projectItem(item) },
    }
  ...
  try {
    ...
    for (const [counter, item] of [items[0], items[2]].entries()) {
      const markdown = h.renderer.root.findDescendantById(
        `markdown:${item!.id}`,
      ) as MarkdownRenderable
      const block = (
        markdown as unknown as {
          _blockStates: readonly { renderable: TextBufferRenderable }[]
        }
      )._blockStates[0]!.renderable
      let prototype: object | null = block
      let getter: (() => string) | undefined
      while (prototype && !getter) {
        getter = Object.getOwnPropertyDescriptor(prototype, "plainText")
          ?.get as (() => string) | undefined
        prototype = Object.getPrototypeOf(prototype)
      }
      expect(getter).toBeDefined()
      Object.defineProperty(block, "plainText", {
        configurable: true,
        get() {
    ...
    expect(folded.points![items[2]!.id]![0]!.screenY).toBeLessThan(
      initialAfterY,
    )
...
test("folded point fallback reads a large projection a bounded number of times", async () => {
  const item = {
    id: itemId("large-fold"),
    turnId: turnId("large-fold-turn"),
    kind: "command" as const,
    title: "Large output",
    detail: Array.from({ length: 200 }, (_, index) => `${index}: output`).join(
      "\n",
    ),
    status: "complete" as const,
  }
  let state = setFold(
    syncTranscriptItem(initialTranscript(), item),
    item.id,
    true,
  )
  ...
  Object.defineProperty(counted, "plain", {
    enumerable: true,
    get() {
      reads += 1
  ...
  state = {
    ...state,
    projectionById: { ...state.projectionById, [item.id]: counted },
  }
  const h = await testRender(
    <scrollbox id="scroll" width="100%" height="100%">
      <box id={`transcript-item:${item.id}`}>
        <ToolCall item={item} folded />
      </box>
    </scrollbox>,
    { width: 80, height: 12 },
  )
  try {
    ...
    const layout = measureRenderedTranscript(
      h.renderer,
      h.renderer.root.findDescendantById("scroll") as ScrollBoxRenderable,
      state,
    )!
    expect(Object.keys(layout.points![item.id]!).length).toBeLessThan(32)
    expect(
      layout.points![item.id]![projection.sourceSpans.length]?.hidden,
    ).toBe(true)
