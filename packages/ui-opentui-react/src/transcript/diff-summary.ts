...
export function diffSummary(patch: string): { added: number; removed: number } | undefined {
  let added = 0, removed = 0, oldRemaining = 0, newRemaining = 0, sawHunk = false
  for (const line of patch.split("\n")) {
    const hunk = /^@@ -(\d+)(?:,(\d+))? \+(\d+)(?:,(\d+))? @@/.exec(line)
    if (hunk) { oldRemaining = Number(hunk[2] ?? 1); newRemaining = Number(hunk[4] ?? 1); sawHunk = true; continue }
    if (line.startsWith("+") && newRemaining > 0) { added++; newRemaining-- }
    else if (line.startsWith("-") && oldRemaining > 0) { removed++; oldRemaining-- }
    else if (line.startsWith(" ")) { oldRemaining = Math.max(0, oldRemaining - 1); newRemaining = Math.max(0, newRemaining - 1) }
  }
...
export function diffFiletype(path: string): string {
  ...
  if (["tsx", "mtsx", "ctsx"].includes(extension ?? "")) return "typescriptreact"
