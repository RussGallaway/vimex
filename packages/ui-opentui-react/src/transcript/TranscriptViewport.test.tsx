...
import { blockGraphemeRange, createTranscriptFrame, initialTranscript, syncTranscriptItem, type TranscriptItemBlock, type TranscriptWindow } from "@vimex/transcript"
...
test("Markdown and edit fragments keep unique native children, exact geometry, and one edit header", async () => {
  const thread = threadId("content-fragments"), markdownTurn = turnId("markdown-fragment-turn"), editTurn = turnId("edit-fragment-turn")
  const markdownId = itemId("fragment-markdown"), editId = itemId("fragment-edit")
  const paragraphs = Array.from({ length: 24 }, (_, index) => index === 8
    ? `## Section ${index}\n\n| column | value |\n| --- | ---: |\n| row | ${index} |`
    : index === 16 ? "```ts\nconst stable = true\n```"
      : `Paragraph ${index}: ${"bounded Markdown content ".repeat(8)}[link](https://vimex.test/${index}).`)
  const markdown = paragraphs.join("\n\n")
  const changes = Array.from({ length: 4 }, (_, index) => ({ path: `file-${index}.ts`, action: "update" as const,
    patch: `diff --git a/file-${index}.ts b/file-${index}.ts\n--- a/file-${index}.ts\n+++ b/file-${index}.ts\n@@ -1 +1 @@\n-old${index}\n+new${index}` }))
  const markdownItem = { id: markdownId, turnId: markdownTurn, kind: "assistant" as const, markdown, status: "complete" as const }
  const editItem = { id: editId, turnId: editTurn, kind: "edit" as const, title: "Four deterministic files",
    patch: changes.map(change => change.patch).join("\n"), changes, status: "complete" as const }
  let conversation = createConversation(thread)
  let transcript = initialTranscript()
  for (const [turn, item] of [[markdownTurn, markdownItem], [editTurn, editItem]] as const) {
    conversation = reduceConversation(conversation, { type: "turn.started", threadId: thread, turnId: turn })
    conversation = reduceConversation(conversation, { type: "item.started", threadId: thread, item })
    conversation = reduceConversation(conversation, { type: "turn.completed", threadId: thread, turnId: turn, outcome: "complete", durationMs: 1 })
    transcript = syncTranscriptItem(transcript, conversation.items[item.id]!)
  }
  const frame = createTranscriptFrame({ threadId: thread, canonicalGeneration: 0, canonicalRevision: 6,
    conversation, transcript, mode: "follow" })
  const markdownBlocks = frame.blocks.filter((block): block is TranscriptItemBlock => "projection" in block && block.key.itemId === markdownId)
  const editBlocks = frame.blocks.filter((block): block is TranscriptItemBlock => "projection" in block && block.key.itemId === editId)
  ...
  try {
    ...
    for (const block of markdownBlocks) {
      expect(setup.renderer.root.findDescendantById(`markdown:${markdownId}:${block.key.blockId}`)).toBeDefined()
    }
    for (const [index, block] of editBlocks.entries()) {
      if (block.renderItem.kind !== "edit" || !block.renderItem.changes) throw new Error("Expected edit fragment")
      for (const localIndex of block.renderItem.changes.keys()) {
        const localSuffix = localIndex === 0 ? "" : `:${localIndex}`
        expect(setup.renderer.root.findDescendantById(`diff:${editId}:${block.key.blockId}${localSuffix}`)).toBeDefined()
        expect(setup.renderer.root.findDescendantById(`decoration:edit-file:${editId}:${block.key.blockId}:${localIndex}`)).toBeDefined()
      }
      if (index > 0) expect(setup.renderer.root.findDescendantById(`decoration:edit-header:${editId}:${block.key.blockId}`)).toBeUndefined()
    }
    expect(setup.renderer.root.findDescendantById(`decoration:edit-header:${editId}:edit:header`)).toBeDefined()
    expect(setup.renderer.root.findDescendantById(`decoration:edit-header:${editId}:${editBlocks[1]!.key.blockId}`)).toBeUndefined()
    const rootsByBlock = new Map([...markdownBlocks, ...editBlocks].map(block => [block,
      setup.renderer.root.findDescendantById(transcriptBlockRenderableId(block))!]))
    for (const blocks of [markdownBlocks, editBlocks]) for (let index = 1; index < blocks.length; index++) {
    ...
    const scroll = setup.renderer.root.findDescendantById("transcript") as ScrollBoxRenderable
    const layout = measureRenderedTranscript(setup.renderer, scroll, { frame, styleRevision: "content-fragment-test" })!
    for (const blocks of [markdownBlocks, editBlocks]) {
      const id = blocks[0]!.key.itemId
      const points = layout.points![id]!
      expect(Object.keys(points)).toHaveLength(frame.transcript.projectionById[id]!.sourceSpans.length + 1)
...
test("fragmented Markdown preserves the unsplit native frame and every logical coordinate", async () => {
  const thread = threadId("markdown-parity"), turn = turnId("markdown-parity-turn"), id = itemId("markdown-parity-item")
  const markdown = Array.from({ length: 18 }, (_, index) => index === 6
    ? "## Stable table\n\n| column | value |\n| --- | ---: |\n| row | six |"
    : index === 12 ? "```ts\nconst fenced = true\n```"
      : `Paragraph ${index} has **stable emphasis**, \`code\`, and [a link](https://vimex.test/${index}). ${"wrap text ".repeat(20)}`).join("\n\n")
  const item = { id, turnId: turn, kind: "assistant" as const, markdown, status: "complete" as const }
  let conversation = createConversation(thread)
  conversation = reduceConversation(conversation, { type: "turn.started", threadId: thread, turnId: turn })
  conversation = reduceConversation(conversation, { type: "item.started", threadId: thread, item })
  const transcript = syncTranscriptItem(initialTranscript(), conversation.items[id]!)
  const frame = createTranscriptFrame({ threadId: thread, canonicalGeneration: 0, canonicalRevision: 2,
    conversation, transcript, mode: "follow" })
  const fragments = frame.blocks.filter((block): block is TranscriptItemBlock => "projection" in block)
  expect(fragments.length).toBeGreaterThan(1)
  const first = fragments[0]!
  const root: TranscriptItemBlock = Object.freeze({ ...first,
  ...
  const render = async (window: TranscriptWindow, layoutFrame: typeof frame, styleRevision: string) => {
    const syntax = createEmberTideSyntax(), scrollRef = createRef<ScrollBoxRenderable>()
    const setup = await testRender(<TranscriptViewport window={window} state={transcript}
      surface="transcript" syntax={syntax} scrollRef={scrollRef} />, { width: 80, height: 400 })
    try {
      await act(async () => { await setup.flush(); await setup.renderOnce() })
      const scroll = setup.renderer.root.findDescendantById("transcript") as ScrollBoxRenderable
      const layout = measureRenderedTranscript(setup.renderer, scroll, { frame: layoutFrame, styleRevision })!
  ...
  const fragmentedResult = await render(fragmentedWindow, fragmentedFrame, "markdown-fragment-parity")
  ...
  expect(Object.keys(fragmentedResult.points)).toEqual(Object.keys(rootResult.points))
  for (const offset of Object.keys(rootResult.points).map(Number)) {
    expect(fragmentedResult.points[offset]).toMatchObject({ screenX: rootResult.points[offset]!.screenX, screenY: rootResult.points[offset]!.screenY })
  }
})

test("fragmented multi-file edits preserve the unsplit native frame and every logical coordinate", async () => {
  const thread = threadId("edit-parity"), turn = turnId("edit-parity-turn"), id = itemId("edit-parity-item")
  const changes = Array.from({ length: 6 }, (_, index) => ({ path: `src/file-${index}.ts`, action: "update" as const,
    patch: `diff --git a/src/file-${index}.ts b/src/file-${index}.ts\n--- a/src/file-${index}.ts\n+++ b/src/file-${index}.ts\n@@ -1,2 +1,2 @@\n export const file${index} = {\n-  value: "before",\n+  value: "after",\n }` }))
  const item = { id, turnId: turn, kind: "edit" as const, title: "Six files",
    patch: changes.map(change => change.patch).join("\n"), changes, status: "complete" as const }
  let conversation = createConversation(thread)
  conversation = reduceConversation(conversation, { type: "turn.started", threadId: thread, turnId: turn })
  conversation = reduceConversation(conversation, { type: "item.started", threadId: thread, item })
  ...
  const render = async (window: TranscriptWindow, layoutFrame: typeof frame, styleRevision: string) => {
    const syntax = createEmberTideSyntax(), scrollRef = createRef<ScrollBoxRenderable>()
    const setup = await testRender(<TranscriptViewport window={window} state={transcript}
      surface="transcript" syntax={syntax} scrollRef={scrollRef} />, { width: 80, height: 200 })
    try {
      await act(async () => { await setup.flush(); await setup.renderOnce() })
      const scroll = setup.renderer.root.findDescendantById("transcript") as ScrollBoxRenderable
      const layout = measureRenderedTranscript(setup.renderer, scroll, { frame: layoutFrame, styleRevision })!
  ...
  const fragmentedResult = await render(fragmentedWindow, fragmentedFrame, "edit-fragment-parity")
