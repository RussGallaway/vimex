...
const legacyGeometryCache = new WeakMap<
  ScrollBoxRenderable,
  {
...
const measurementSchedules = new WeakMap<
  ScrollBoxRenderable,
  MeasurementSchedule
>()
...
function blockPresentation(
  ...
  presentation: Readonly<Record<string, TranscriptActivityPresentation>>,
): TranscriptBlockPresentation {
...
function scheduleFor(scrollbox: ScrollBoxRenderable): MeasurementSchedule {
  ...
  if (!schedule) {
    schedule = {
      pending: new Set(),
      rejectedRetries: new Map(),
      indexByKey: new Map(),
      keysByItem: new Map(),
      renderableByKey: new Map(),
    }
...
function indexBlocks(
  ...
  blocks.forEach((block, index) => {
    ...
    if (block.key.kind === "item")
      schedule.keysByItem.set(
        block.key.itemId,
        Object.freeze([
          ...(schedule.keysByItem.get(block.key.itemId) ?? []),
          key,
        ]),
      )
  })
...
function synchronizeMeasurementSchedule(
  ...
  const priorByKey = new Map(
    (schedule.windowBlocks ?? []).map((block) => [blockKey(block), block]),
  )
  if (lineageChanged || layoutReset) {
    ...
    for (const key of schedule.renderableByKey.keys())
      if (!mountedKeys.has(key)) {
    ...
    for (const block of blocks)
      if (priorByKey.get(blockKey(block)) !== block)
        schedule.pending.add(blockKey(block))
  }
  schedule.threadId = frame.threadId
  schedule.canonicalGeneration = frame.canonicalGeneration
...
export function releaseRenderedTranscriptLayout(
  scrollbox: ScrollBoxRenderable,
): void {
...
export function transcriptBlockRenderableId(
  block: Pick<TranscriptBlock, "key">,
): string {
...
export function transcriptItemRenderableId(
  itemId: ItemId,
  blockId = "root",
): string {
...
export function transcriptRenderableIdForPoint(
  layout: TranscriptLayout,
  point: LogicalPoint,
): string {
  return transcriptItemRenderableId(
    point.itemId,
    blockRefForPoint(layout, point)?.blockId ?? "root",
  )
}
...
function translatedLayout(
  ...
  Object.defineProperty(translated, "screenOffset", {
    enumerable: true,
    configurable: false,
    value: Object.freeze({ x, y }),
  })
  return Object.freeze(translated)
}

/** Translate cached native placement after a scroll without measuring roots. */
export function translateTranscriptLayout(
  layout: TranscriptLayout,
  deltaX: number,
  deltaY: number,
): TranscriptLayout {
  ...
  return translatedLayout(
    layout,
    (layout.screenOffset?.x ?? 0) + deltaX,
    (layout.screenOffset?.y ?? 0) + deltaY,
  )
}

function findBlockRenderable(
  scrollbox: ScrollBoxRenderable,
  block: TranscriptBlock,
): Renderable | undefined {
  return (
    scrollbox.getRenderable(transcriptBlockRenderableId(block)) ??
    (block.key.kind === "item"
      ? scrollbox.getRenderable(legacyRenderableId(block.key.itemId))
      : undefined)
  )
}
...
function linesFor(
  ...
  for (const block of blocks) {
    ...
    const itemLines = (geometry.byBlockKey[key]?.lines ?? []).map((line) => ({
      itemId: block.key.itemId,
      from: line.from,
      to: line.to,
      row: start + line.row,
    }))
    linesByItem[block.key.itemId] = [
      ...(linesByItem[block.key.itemId] ?? []),
      ...itemLines,
    ]
  ...
  return {
    ...
    linesByItem: Object.freeze(linesByItem),
  }
}
...
function lazyPoints(
  ...
  return new Proxy(
    Object.create(null) as Record<
      string,
      Readonly<Record<number, MeasuredPoint>>
    >,
    {
      ownKeys: () => itemIds,
      getOwnPropertyDescriptor: (_target, property) =>
        typeof property === "string" && itemIds.includes(property as ItemId)
      ...
      get: (_target, property) => {
        ...
        const blocksForItem = itemBlocks.filter(
          (candidate) => candidate.key.itemId === property,
        )
        if (!blocksForItem.length) return undefined
        const offsets = [
          ...new Set(
            blocksForItem.flatMap((block) =>
              Object.keys(
                layout.geometry?.byBlockKey[blockKey(block)]?.points ?? {},
              ),
            ),
          ),
        ].sort((left, right) => Number(left) - Number(right))
        const record = new Proxy(
          Object.create(null) as Record<number, MeasuredPoint>,
          {
            ownKeys: () => offsets,
            getOwnPropertyDescriptor: (_inner, offset) =>
              typeof offset === "string" && offsets.includes(offset)
                ? { enumerable: true, configurable: true }
                : undefined,
            get: (_inner, offset) => {
              if (typeof offset !== "string") return undefined
              return pointInLayout(layout, {
                itemId: blocksForItem[0]!.key.itemId,
                graphemeOffset: Number(offset),
              })
            },
          },
        )
...
function buildLayout(
  ...
  const layout = {
    ...
    blockKeysByItem: Object.freeze(
      Object.fromEntries(
        Object.entries(blockKeysByItem).map(([itemId, refs]) => [
          itemId,
    ...
    screenBlockRows: Object.freeze(
      blocks.flatMap((block) => {
        ...
        if (!placement) return []
        const rows =
          geometry.byBlockKey[key]?.rows ?? Math.max(1, block.estimatedRows)
        return rows > 0
          ? [
              Object.freeze({
                blockKey: key,
  ...
  Object.defineProperty(layout, "lines", {
    enumerable: true,
    configurable: false,
    get: () => indexes().lines,
  })
  Object.defineProperty(layout, "linesByItem", {
    enumerable: true,
    configurable: false,
    get: () => indexes().linesByItem,
  })
  Object.defineProperty(layout, "points", {
    enumerable: true,
    configurable: false,
    value: lazyPoints(layout, blocks),
  })
...
function placementsMatch(
  ...
  for (const block of blocks) {
    ...
    if (
      renderable.screenX !== placement.screenX + offset.x ||
      renderable.screenY !== placement.screenY + offset.y
    )
      return false
  }
  return true
...
function currentGeometry(
  ...
  scrollbox: ScrollBoxRenderable,
  source: RuntimeLayoutSource | TranscriptState,
): {
  blocks: readonly TranscriptBlock[]
  geometry: TranscriptFrame["geometry"]
} {
  ...
  const blocks: readonly TranscriptBlock[] =
    frame?.window.blocks ?? syntheticBlocks(source as TranscriptState)
  ...
  if (
    ...
    frame!.geometry.width !== undefined &&
    (frame!.geometry.width !== width ||
      frame!.geometry.styleRevision !== styleRevision)
  ) {
    runtimeSource.runtime.resetLayout(
      frame!.geometry.width !== width ? "width" : "style",
    )
  ...
  if (!runtimeSource?.runtime) {
    for (const block of blocks)
      if ("projection" in block) candidates.add(blockKey(block))
  } else {
    const { lineageChanged, layoutReset, windowChanged, priorByKey } =
      synchronizeMeasurementSchedule(scrollbox, frame!, diagnostics)
    if (lineageChanged || layoutReset) {
      ...
      if (windowChanged) {
        for (const block of blocks) {
          ...
          const presentation =
            frame!.window.activityPresentation[key]?.kind ?? "item"
          const folded =
            block.key.kind === "item" &&
            (presentation !== "item" || Boolean(state.folded[block.key.itemId]))
          if (
            priorByKey.get(key) !== block ||
            !geometry ||
            geometry.key.contentRevision !== block.contentRevision ||
            geometry.key.folded !== folded ||
            (geometry.key.presentation ?? "item") !== presentation ||
            geometry.key.width !== width ||
      ...
      if (frame!.damage.kind === "full" || frame!.damage.kind === "layout") {
        ...
        frame!.damage.kind === "blocks" ||
        frame!.damage.kind === "folds"
      ) {
    ...
    for (const dirty of takeDirtyRenderedBlocks()) {
      if (
        dirty.blockKey &&
        schedule.renderableByKey.get(dirty.blockKey) === dirty.renderable
      )
        candidates.add(dirty.blockKey)
    }
    for (const key of schedule.pending) {
      candidates.add(key)
    }
  ...
  const orderedCandidates = [...candidates]
    .flatMap((key) => {
      ...
      const block =
        index === undefined
          ? blocks.find((candidate) => blockKey(candidate) === key)
          : blocks[index]
      if (!block) return []
      const renderable = findBlockRenderable(scrollbox, block)
      ...
      const visible =
        renderable.screenY < bottom &&
        renderable.screenY + renderable.height > top
      const distance = visible
        ? 0
        : renderable.screenY >= bottom
          ? renderable.screenY - bottom
          : top - (renderable.screenY + renderable.height)
      return [
        {
          key,
          block,
  ...
  if (diagnostics) {
    ...
    diagnostics.visibleCandidates = orderedCandidates.filter(
      (candidate) => candidate.visible,
    ).length
    diagnostics.overscanCandidates =
      orderedCandidates.length - diagnostics.visibleCandidates
    diagnostics.attemptedMeasurements = orderedCandidates.length
    diagnostics.visibleBeforeOverscan =
      orderedCandidates
        .slice(0, diagnostics.visibleCandidates)
        .every((candidate) => candidate.visible) &&
      orderedCandidates
        .slice(diagnostics.visibleCandidates)
        .every((candidate) => !candidate.visible)
    diagnostics.attemptedKeys?.push(
      ...orderedCandidates.map((candidate) => candidate.key),
    )
  }
  ...
  for (const { key, block, renderable } of orderedCandidates) {
    ...
    const next = measureRenderedBlock({
      renderer,
      renderable,
      block,
      width,
      styleRevision,
      presentation: blockMode,
      folded:
        block.key.kind === "item" &&
        (blockMode !== "item" || Boolean(state.folded[block.key.itemId])),
    })
    ...
    if (
      !prior ||
      prior.nativeRevision !== next.nativeRevision ||
      prior.key.contentRevision !== next.key.contentRevision ||
      prior.key.width !== next.key.width ||
      prior.key.styleRevision !== next.key.styleRevision ||
      prior.key.folded !== next.key.folded ||
      (prior.key.presentation ?? "item") !== (next.key.presentation ?? "item")
    )
      measured.push(next)
    else schedule.pending.delete(key)
  }
  if (runtimeSource?.runtime && measured.length) {
    ...
    runtimeSource.runtime.reportMeasurements({
      ...measurementBase!,
      measurements: measured,
    })
    if (diagnostics) {
      diagnostics.changedMeasurements = measured.length
      diagnostics.cachedMeasurements =
        orderedCandidates.length - measured.length - rejectedMeasurements
  ...
  if (frame && runtimeSource?.runtime)
    return { blocks, geometry: frame.geometry }
  const legacy = legacyGeometryCache.get(scrollbox)
  if (
    legacy &&
    legacy.order === state.order &&
    legacy.projectionById === state.projectionById &&
    legacy.folded === state.folded &&
    legacy.width === width &&
    legacy.measurements.length === measured.length &&
    measured.every((geometry, index) => geometry === legacy.measurements[index])
  )
    return { blocks, geometry: legacy.geometry }
  const byBlockKey = Object.fromEntries(
    measured.map((geometry) => [geometry.key.blockKey, geometry]),
  )
  ...
  legacyGeometryCache.set(scrollbox, {
    order: state.order,
    projectionById: state.projectionById,
    folded: state.folded,
    width,
    measurements: measured,
    geometry,
...
export function measureRenderedTranscript(
  ...
  if (
    ...
    cached.materializedBlocks === materializedBlocks
  ) {
    ...
    if (
      placementsMatch(
        ...
        cached.layout,
        offset,
        "frame" in source ? source.diagnostics : undefined,
      )
    ) {
      if (
        offset.x === (cached.layout.screenOffset?.x ?? 0) &&
        offset.y === (cached.layout.screenOffset?.y ?? 0)
      )
...
export function rebaseTranscriptLayout(
  layout: TranscriptLayout,
  geometry: TranscriptFrame["geometry"],
): TranscriptLayout {
  const screenBlockRows = (layout.screenBlockRows ?? []).flatMap((row) => {
    const placement = layout.placementByBlockKey?.[row.blockKey]
    const rows = geometry.byBlockKey[row.blockKey]?.rows ?? row.rows
    return placement && rows > 0
      ? [Object.freeze({ ...row, screenY: placement.screenY, rows })]
      : []
  })
  return Object.freeze({
    width: layout.width,
    ...(layout.materializedBlocks
      ? { materializedBlocks: layout.materializedBlocks }
      : {}),
    lines: Object.freeze([]),
...
export function visibleMeasuredPoints(
  layout: TranscriptLayout,
  viewport: { screenY: number; height: number },
): readonly MeasuredPoint[] {
  if (!layout.geometry || !layout.screenBlockRows)
    return Object.values(layout.points ?? {}).flatMap((points) =>
  ...
  let low = 0,
    high = layout.screenBlockRows.length
  while (low < high) {
  ...
  for (let index = low; index < layout.screenBlockRows.length; index++) {
    ...
    for (let row = fromRow; row < toRow; row++) {
      const offsets =
        geometry.pointOffsetsByRow?.[row] ??
...
function viewportEdgePoint(
  ...
  for (const point of visibleMeasuredPoints(layout, scrollbox.viewport)) {
    ...
    if (
      !candidate ||
      (edge === "top"
        ? point.screenY < candidate.screenY
        : point.screenY > candidate.screenY) ||
      (point.screenY === candidate.screenY && point.screenX < candidate.screenX)
    )
      candidate = point
  }
  return candidate
...
export function topVisiblePoint(
  layout: TranscriptLayout,
  scrollbox: ScrollBoxRenderable,
): MeasuredPoint | undefined {
...
export function bottomVisiblePoint(
  layout: TranscriptLayout,
  scrollbox: ScrollBoxRenderable,
): MeasuredPoint | undefined {
