...
test("multiple file changes render independent exact patches, paths, actions, and summaries", async () => {
  const changes = [
    { path: "old.ts", action: "update" as const, movePath: "new.ts", patch: "--- a/old.ts\n+++ b/new.ts\n@@ -1 +1 @@\n-oldValue\n+newValue\n" },
    { path: "added.py", action: "add" as const, patch: "--- /dev/null\n+++ b/added.py\n@@ -0,0 +1 @@\n+addedValue\n" },
  ]
  for (const width of [80, 140]) {
    const syntax = createEmberTideSyntax()
    const setup = await testRender(<FileChange folded={false} syntax={syntax} item={{ id: itemId("multi"), turnId: turnId("turn"), kind: "edit", title: "old.ts, added.py", status: "complete", changes, patch: changes.map(change => change.patch).join("\n") }} />, { width, height: 24 })
    try {
      await act(async () => setup.flush())
      const frame = setup.captureCharFrame()
      ...
      for (const [index, change] of changes.entries()) expect((setup.renderer.root.findDescendantById(index === 0 ? "diff:multi" : `diff:multi:${index}`) as DiffRenderable).diff).toBe(change.patch)
    } finally { await act(async () => setup.renderer.destroy()); syntax.destroy() }
  }
})
...
test("collapsed edits use disclosure headers and do not instantiate diff bodies", async () => {
  const syntax = createEmberTideSyntax()
  const setup = await testRender(<FileChange folded syntax={syntax} item={{ id: itemId("fold"), turnId: turnId("turn"), kind: "edit", title: "sample.ts", status: "complete", patch: "@@ -1 +1 @@\n-old\n+new" }} />, { width: 60, height: 10 })
  try {
    await act(async () => setup.flush())
...
test("metadata-only file changes explain that no textual patch is available", async () => {
  const syntax = createEmberTideSyntax()
  const setup = await testRender(<FileChange folded={false} syntax={syntax} item={{ id: itemId("empty"), turnId: turnId("turn"), kind: "edit", title: "old.bin", status: "complete", patch: "", changes: [{ path: "old.bin", movePath: "new.bin", action: "update", patch: "" }] }} />, { width: 80, height: 10 })
  try {
    await act(async () => setup.flush())
