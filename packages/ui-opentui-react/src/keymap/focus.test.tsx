...
function fixture() {
  return transitionWorkbench(initialWorkbench(), { type: "thread.open", summary: {
    id: threadId("keys"), title: "Keys", cwd: "/tmp", model: "test", reasoningEffort: "high", status: "idle",
  } }).state
}
...
test("Space leader opens questions without inserting into the composer", async () => {
  ...
  const setup = await testRender(<VimexRoot state={fixture()} controller={{ ...inertController, dispatchInteraction: command => { commands.push(command) } }} />, { width: 80, height: 24 })
  try {
    await act(async () => setup.flush())
    await setup.mockInput.typeText(" q")
    await act(async () => setup.flush())
    expect(commands).toContainEqual({ type: "overlay.open", overlay: "questions" })
  } finally { await act(async () => setup.renderer.destroy()) }
})

test("Ctrl-w k works while typing and preserves the draft", async () => {
  let state = fixture()
  state = transitionWorkbench(state, { type: "interaction.command", command: { type: "mode.insert" } }).state
  state = transitionWorkbench(state, { type: "composer.change", text: "draft survives", cursorOffset: 14 }).state
  const commands: Parameters<VimexUiController["dispatchInteraction"]>[0][] = []
  const setup = await testRender(<VimexRoot state={state} controller={{ ...inertController, dispatchInteraction: command => { commands.push(command) } }} />, { width: 80, height: 24 })
  try {
    await act(async () => setup.flush())
...
test("Ctrl-w clears composer Visual selection before the next Normal edit", async () => {
  ...
  initial = transitionWorkbench(initial, { type: "composer.change", text: "abcd", cursorOffset: 1 }).state
  ...
  function StatefulFixture() {
    ...
    const controller = useMemo<VimexUiController>(() => ({
      ...
      changeDraft(text, cursorOffset) {
        setState(current => transitionWorkbench(current, { type: "composer.change", text, cursorOffset }).state)
      },
    }), [])
  ...
  try {
    ...
    expect(observed.workspaces[threadId("keys")]?.interaction).toMatchObject({ mode: "normal", surface: "transcript" })
...
test("leaving composer Visual through Command mode clears its native selection", async () => {
  ...
  try {
    ...
    const composer = setup.renderer.root.findDescendantById("composer") as TextareaRenderable
    expect(observed.workspaces[threadId("keys")]?.interaction.mode).toBe("command")
    expect(composer.getSelection()).toBeNull()
...
test("switching threads clears a composer selection even when the target restores Visual mode", async () => {
  ...
  initial = transitionWorkbench(initial, { type: "thread.register", summary: {
    id: target, title: "Other", cwd: "/tmp", model: "test", reasoningEffort: "high", status: "idle",
  } }).state
  initial = transitionWorkbench(initial, { type: "composer.change", threadId: target, text: "WXYZ", cursorOffset: 1 }).state
  initial = transitionWorkbench(initial, { type: "interaction.command", threadId: target, command: { type: "focus.set", surface: "composer" } }).state
  initial = transitionWorkbench(initial, { type: "interaction.command", threadId: target, command: { type: "mode.visual" } }).state
  let observed = initial
  ...
  function StatefulFixture() {
    const [state, setState] = useState(initial)
    observed = state
    switchThread = () => setState(current => transitionWorkbench(current, { type: "thread.switch", threadId: target }).state)
    const controller = useMemo<VimexUiController>(() => ({
      ...inertController,
      dispatchInteraction(command) {
        setState(current => transitionWorkbench(current, { type: "interaction.command", command }).state)
      ...
      copyText(text) { copies.push(text) },
    }), [])
    return <VimexRoot state={state} controller={controller} />
  }
  const setup = await testRender(<StatefulFixture />, { width: 80, height: 24 })
  try {
    ...
    expect(setup.renderer.root.findDescendantById("composer")?.id).toBe("composer")

    await act(async () => { switchThread(); await Bun.sleep(0); await setup.flush() })
    expect(observed.activeThreadId).toBe(target)
    expect(observed.workspaces[target]?.interaction).toMatchObject({ mode: "visual", surface: "composer" })
    const composer = setup.renderer.root.findDescendantById("composer") as TextareaRenderable
    expect(composer.getSelection()).toBeNull()

    await act(async () => { await setup.mockInput.typeText("y"); await Bun.sleep(0); await setup.flush() })
