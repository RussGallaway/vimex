...
export function normalBindings(ctx: VimBindingContext): UiBinding[] {
  return [
    ...
    { key: "?", cmd: () => ctx.openOverlay("help") },
    { key: "gx", cmd: () => ctx.controller.transcript({ type: "url.open" }) },
    { key: "f", cmd: () => ctx.controller.transcript({ type: "fork", itemId: ctx.transcript.cursor?.itemId }) },
    ...
    ...(ctx.interaction.surface === "composer" ? [
      ...["h", "j", "k", "l", "w", "b", "$", "x", "dd", "u", "ctrl+r", "shift+r", "shift+i", "shift+a"]
