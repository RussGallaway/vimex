...
export function normalBindings(ctx: VimBindingContext): UiBinding[] {
  return [
    ...
    ...(ctx.interaction.surface === "composer" ? ["/", "?"].map(prefix => ({ key: prefix, cmd: () => {
      ctx.controller.dispatchInteraction({ type: "mode.command" })
    ...
    ...vimFoldBindings(ctx),
    ...(ctx.interaction.surface === "transcript" ? [..."abcdefghijklmnopqrstuvwxyz"].flatMap(name => [
      { key: `m${name}`, cmd: () => ctx.controller.transcript({ type: "mark.set", name }) },
      { key: `'${name}`, cmd: () => ctx.controller.transcript({ type: "mark.jump", name }) },
      { key: "`" + name, cmd: () => ctx.controller.transcript({ type: "mark.jump", name }) },
    ]) : []),
