...
for (const name of themeNames)
  test(`${name} renders parsed code with its own token colors`, async () => {
    const syntax = createEmberTideSyntax(name)
    const palette = themePalette(name)
    const h = await testRender(
      <code
        content={'function greet() { return "hello" + 42 }'}
        filetype="typescript"
        syntaxStyle={syntax}
        fg={palette.text}
    ...
    try {
      for (let attempt = 0; attempt < 100; attempt++) {
        ...
        if (
          spans()
            .find((span) => span.text === "function")
            ?.fg.toString() === syntax.getStyle("keyword")!.fg!.toString()
        )
          break
      }
      for (const [text, scope] of [
        ...
        expect(token!.fg.toString()).toBe(
          syntax.getStyle(scope!)!.fg!.toString(),
        )
      }
    } finally {
