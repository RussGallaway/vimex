import { threadId, type ThreadId, type ThreadSummary } from "@vimex/conversation"

export interface SessionRow { id: ThreadId; summary?: ThreadSummary; favorite?: boolean }
...
export function searchSessions(threads: readonly ThreadId[], summaries: Readonly<Record<string, ThreadSummary>>, query: string, favorites: readonly ThreadId[] = []): SessionRow[] {
  ...
  return available.map((id, order) => {
    ...
    return { id, summary, order, favorite: favoriteIds.has(id), score: fuzzyScore(searchable, query), updatedAt: summary?.updatedAt ?? 0 }
  }).filter((row) => row.score !== undefined)
    .sort((a, b) => Number(b.favorite) - Number(a.favorite) || (query.trim() ? b.score! - a.score! || b.updatedAt - a.updatedAt || a.order - b.order : b.updatedAt - a.updatedAt || a.order - b.order))
    .map(({ id, summary, favorite }) => ({ id, summary, ...(favorite ? { favorite: true } : {}) }))
}
