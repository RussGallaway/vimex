...
async function fixture(otherCwd = "/tmp") {
  const active = threadId("active"),
    other = threadId("other")
  let initial = initialWorkbench()
  for (const id of [other, active])
    initial = transitionWorkbench(initial, {
      type: "thread.open",
      summary: {
        id,
        title: id === active ? "Active session" : "Other session",
        cwd: id === active ? "/tmp" : otherCwd,
        model: "test",
        reasoningEffort: "high",
        status: "idle",
        updatedAt: id === active ? 20 : 10,
      },
    }).state
  ...
  const renamed: { id: ThreadId; title: string }[] = [],
    favorites: ThreadId[] = []
  function Harness() {
    ...
    const controller = useMemo<VimexUiController>(
      () => ({
        ...
        toggleFavorite(id) {
          ...
          setState((current) => ({
            ...current,
            favoriteThreadIds: current.favoriteThreadIds.includes(id)
              ? current.favoriteThreadIds.filter((value) => value !== id)
              : [...current.favoriteThreadIds, id],
          }))
        },
      }),
      [],
...
test("favorites keep the selected non-active session targeted after reordering", async () => {
  ...
  try {
    ...
    await act(async () => {
      ...
      await setup.mockInput.typeText("Renamed other")
...
test("session picker defaults to cwd and toggles All without losing search", async () => {
  ...
  try {
    ...
    await act(async () => {
      await h.mockInput.typeText("Other")
      await h.flush()
      await h.renderOnce()
    })
    await act(async () => {
    ...
    expect(h.captureCharFrame()).toContain("Other session")
    const query = h.renderer.root.findDescendantById(
      "session-search",
    ) as import("@opentui/core").InputRenderable
