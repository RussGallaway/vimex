...
const first = threadId("menu-first"), second = threadId("menu-second")
function fixture() {
  let state = initialWorkbench()
  for (const [id, title] of [[first, "Alpha"], [second, "jk match"]] as const) {
    state = transitionWorkbench(state, { type: "thread.open", summary: { id, title, cwd: "/work", model: "test", reasoningEffort: "high", status: "idle" } }).state
  }
  state = { ...state, activeThreadId: first, threadOrder: [first, second] }
  return transitionWorkbench(state, { type: "interaction.command", command: { type: "overlay.open", overlay: "sessions" } }).state
}
async function harness(initial: WorkbenchState = fixture()) {
  ...
  function Harness() {
    ...
    const controller = useMemo<VimexUiController>(() => ({ ...inertController,
      ...
      openThread(id) { opened.push(id) }, answerQuestions(_id, value) { answers.push(value) },
    }), [])
  ...
  const escape = async () => {
    await act(async () => { setup.mockInput.pressKey("ESCAPE"); await Bun.sleep(30); await setup.flush() })
    await act(async () => { await setup.flush(); await setup.renderOnce() })
  }
  return { ...setup, opened, answers, keys, escape, interaction: () => observed.workspaces[first]!.interaction, close: async () => { await act(async () => setup.renderer.destroy()) } }
}
...
test("Normal sessions j/k navigate without entering search text", async () => {
  ...
  try {
    await h.keys("jkj")
    expect((h.renderer.root.findDescendantById("session-search") as InputRenderable).value).toBe("")
...
test("Insert session search keeps literal j/k and Escape returns to Normal before closing", async () => {
  ...
  try {
    ...
    expect((h.renderer.root.findDescendantById("session-search") as InputRenderable).value).toBe("jk")
    expect(h.renderer.root.findDescendantById(`session-row:${first}`)).toBeUndefined()
    await h.escape()
    expect(h.interaction()).toMatchObject({ mode: "normal", overlay: "sessions" })
    await h.keys("jk")
    expect((h.renderer.root.findDescendantById("session-search") as InputRenderable).value).toBe("jk")
    await h.escape()
...
test("session rename keeps j/k as literal input", async () => {
  const h = await harness()
  try {
    await act(async () => { h.mockInput.pressKey("r", { ctrl: true }); await h.flush() })
    await h.keys("jk")
    expect((h.renderer.root.findDescendantById("session-rename") as InputRenderable).value).toBe("Alphajk")
  } finally { await h.close() }
})

for (const allowOther of [false, true]) {
  test(`questions ${allowOther ? "preserve j/k in free text" : "navigate options with j/k"}`, async () => {
    let state = transitionWorkbench(fixture(), { type: "interaction.command", command: { type: "overlay.open", overlay: "questions" } }).state
    state = { ...state, questions: { choice: { id: "choice", threadId: first, turnId: turnId("menu-turn"), questions: [{
      id: "direction", header: "Direction", question: "Choose", allowOther, secret: false,
      ...(!allowOther ? { options: [{ label: "Continue", description: "First" }, { label: "Stop", description: "Second" }] } : {}),
    }] } } }
