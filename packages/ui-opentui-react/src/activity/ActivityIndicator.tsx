...
export function ActivityIndicator({ active, label, startedAt, animate = true, tone = "working", clock = systemActivityClock }: ActivityIndicatorProps) {
  const [tick, setTick] = useState(0)
  useEffect(() => {
    if (!active || (!animate && startedAt === undefined)) return
    return clock.schedule(() => setTick(value => value + 1), animate ? 120 : 1000)
  }, [active, animate, clock, startedAt])
  const duration = startedAt === undefined ? "" : formatDuration(clock.now() - startedAt)
  return <box height={1} flexDirection="row" gap={1} flexShrink={0}>
    ...
    {active && duration ? <text fg={emberTide.textMuted}>· {duration}</text> : null}
  </box>
}
