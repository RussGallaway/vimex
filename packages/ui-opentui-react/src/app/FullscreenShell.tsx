...
export function FullscreenShell(props: {
  ...
  return <box id="vimex-app" width="100%" height="100%" flexDirection="column" backgroundColor={emberTide.background}>
    <box height={2} flexShrink={0} flexDirection="row" alignItems="center" gap={2} paddingX={2}>
      <box flexDirection="row" gap={1} flexGrow={1} flexShrink={1} minWidth={0} overflow="hidden"><text flexShrink={0} fg={emberTide.amber}><b>VIMEX</b></text><text flexShrink={0} fg={emberTide.textMuted}>/</text><text id="thread-title" minWidth={0} flexGrow={1} flexShrink={1} wrapMode="none" truncate fg={emberTide.text}>{props.title ?? "new session"}</text></box>
      <box id="connection-status" flexDirection="row" gap={1} flexShrink={0}>{props.working || props.waiting ? <ActivityIndicator active={props.working && !props.waiting && props.connection === "connected"} label={props.activityLabel ?? "Working"} tone={props.waiting ? "waiting" : "working"} /> : null}<text fg={dot}>● {props.connection}</text></box>
    </box>
    {props.transcript}{props.composer}{props.commandLine ?? props.statusline}{props.overlay}
  </box>
}
