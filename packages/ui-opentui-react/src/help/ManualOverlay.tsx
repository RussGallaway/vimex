...
export function ManualOverlay() {
    ...
    () => ({
      ...
      bindings: [
        ...["j", "down", "ctrl+e"].map((key) => ({
          key,
          cmd: () => scroll.current?.scrollBy(1, "step"),
        })),
        ...["k", "up", "ctrl+y"].map((key) => ({
          key,
          cmd: () => scroll.current?.scrollBy(-1, "step"),
        ...
        {
          key: "ctrl+u",
          cmd: () => scroll.current?.scrollBy(-0.5, "viewport"),
        },
        ...
        {
          key: "shift+g",
          cmd: () => scroll.current?.scrollTo(scroll.current.scrollHeight),
        },
      ],
    }),
    [],
  ...
  return (
    <OverlayFrame title="vimex(1) — User manual" width={96}>
      <scrollbox
        id="manual-scroll"
        ref={scroll}
        height={Math.max(1, Math.floor(dimensions.height * 0.85) - 5)}
      >
      ...
      <text height={1} fg={emberTide.textMuted} wrapMode="none" truncate>
        j/k scroll · ctrl-d/u page · gg/G start/end · esc close
      </text>
    </OverlayFrame>
  )
}
