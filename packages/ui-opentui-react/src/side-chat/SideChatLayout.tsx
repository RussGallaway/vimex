...
export function SideChatLayout(props: VimexAppProps) {
  ...
  return <SideChatFrame layout={layout} controller={props.controller} settings={props.settings} renderPane={({ id, label, presentationId, interactive, presentationVisible }) => (
    <VimexApp {...props} state={interactive ? props.state : { ...props.state, activeThreadId: id }} interactive={interactive}
      presentationVisible={presentationVisible} paneLabel={layout.side?.visible ? label : undefined} presentationId={presentationId} />
  )} />
}

export function ConnectedSideChatLayout(props: { controller: VimexUiController & WorkbenchPublicationHost; settings?: Partial<VimexUiSettings> }) {
  const layout = useSyncExternalStore(props.controller.subscribeLayout, props.controller.getLayoutSnapshot, props.controller.getLayoutSnapshot)
  return <SideChatFrame layout={layout} controller={props.controller} settings={props.settings} renderPane={({ label, presentationId, interactive, presentationVisible }) => (
    <ConnectedPane controller={props.controller} settings={props.settings} presentationId={presentationId} interactive={interactive}
      presentationVisible={presentationVisible} paneLabel={layout.side?.visible ? label : undefined} />
  )} />
}
...
function SideChatFrame(props: {
  ...
  selectTheme(layout.theme ?? props.settings?.theme ?? "ember-tide", props.settings?.reducedColor ?? false)
  ...
  const pane = (id: ThreadId | undefined, label: "MAIN" | "SIDE", geometry: PaneGeometry) => {
    ...
    const presentationVisible = (label === "MAIN" || Boolean(side?.visible)) && (!maximized || focused)
    ...
    return <box key={id ?? "empty"} onMouseDown={() => { if (!focused && !overlay) flushSync(() => controller.sideChat(label === "MAIN" ? "parent" : "side")) }} id={label === "MAIN" ? "main-pane" : "side-pane"} visible={presentationVisible} width={geometry.width} height={geometry.height} flexShrink={0} overflow="hidden">
      <PaneGeometryContext.Provider value={geometry}>
        {props.renderPane({ id, label, presentationId, interactive: focused, presentationVisible })}
  ...
  return <box id={side?.visible ? "side-chat-layout" : "side-chat-closed"} width="100%" height="100%" flexDirection="column" backgroundColor={emberTide.background}>
    ...
    <box flexDirection={stacked ? "column" : "row"} flexGrow={1} minHeight={0}>
      ...
      {side?.threadId && layout.sideThreadReady ? pane(side.threadId, "SIDE", sideGeometry) : side?.visible && !maximized ? <box key="side-opening" id="side-opening" width={sideGeometry.width} height={sideGeometry.height} flexShrink={0} flexDirection="column" paddingX={2} paddingY={1} backgroundColor={emberTide.backgroundPanel}>
