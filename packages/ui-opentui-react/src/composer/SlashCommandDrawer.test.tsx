...
async function harness(width = 80, height = 24, kittyKeyboard = false) {
  ...
  function Harness() {
    ...
    const controller = useMemo<VimexUiController>(
      () => ({
        ...
        submit() {
          submitted.push(observed.workspaces[id]!.composer.text)
        },
      }),
      [],
    )
  ...
  await act(async () => {
    setup = await testRender(<Harness />, { width, height, kittyKeyboard })
    await setup.flush()
  })
...
test("slash choices reuse Ex commands, aliases and dynamic arguments", () => {
  ...
  expect(
    slashCommandChoices("/model model-b ", {
      models: ["model-a", "model-b"],
      modelEfforts: { "model-b": ["low", "high"] },
    }),
  ).toEqual(["model model-b low", "model model-b high"])
  expect(
    slashCommandChoices("/thinking h", {
...
for (const [width, height] of [
  ...
  test(`slash drawer opens above fixed composer and Enter executes at ${width}x${height}`, async () => {
    ...
    try {
      ...
      expect(
        (h.renderer.root.findDescendantById("composer") as TextareaRenderable)
          .plainText,
      ).toBe("/help ")
      await act(async () => {
      ...
      expect(
        h.renderer.root.findDescendantById("slash-command-drawer"),
      ).toBeUndefined()
    } finally {
      await h.close()
    }
...
test("unknown slash input cannot execute or submit and Escape dismisses into Normal", async () => {
  ...
  try {
    ...
    for (
      let n = 0;
      n < 20 && h.workspace().interaction.mode !== "normal";
      n++
    ) {
      await act(async () => {
        await Bun.sleep(5)
...
test("batched slash prefix and Return activate its matching command", async () => {
  ...
  try {
    await act(async () => {
      await h.mockInput.pressKeys(["/", "h", "e", "RETURN"], 0)
      await h.flush()
    })
    expect(h.executed).toEqual(["help"])
    expect(h.submitted).toEqual([])
...
test("batched unknown slash input remains editable after Return", async () => {
  ...
  try {
    await act(async () => {
      await h.mockInput.pressKeys(["/", "z", "z", "z", "RETURN"], 0)
      await h.flush()
    })
    expect(h.executed).toEqual([])
    expect(h.submitted).toEqual([])
...
test("Ctrl-Enter executes a slash command instead of steering its text", async () => {
  ...
  try {
    await h.keys("/help")
    await act(async () => {
      h.mockInput.pressEnter({ ctrl: true })
      await h.flush()
    })
    expect(h.executed).toEqual(["help"])
    expect(h.submitted).toEqual([])
