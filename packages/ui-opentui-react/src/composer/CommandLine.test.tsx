...
async function harness(surface: "composer" | "transcript" = "composer") {
  ...
  let initial = transitionWorkbench(initialWorkbench(), { type: "thread.open", summary: {
    id, title: "Commands", cwd: "/work", model: "model-a", reasoningEffort: "high", status: "idle",
  } }).state
  initial = transitionWorkbench(initial, { type: "interaction.command", command: { type: "focus.set", surface } }).state
  initial = transitionWorkbench(initial, { type: "composer.change", text: "Keep my unfinished draft", cursorOffset: 4 }).state
  initial = { ...initial, availableModels: [
    { id: "model-a", label: "Model A", efforts: ["low", "high"] }, { id: "model-b", label: "Model B", efforts: ["high"] },
  ] }
  ...
  function Harness() {
    ...
    const controller = useMemo<VimexUiController>(() => ({ ...inertController,
      ...
      executeCommand(command) { executed.push(command); if (command === "model") setState(current => transitionWorkbench(current, { type: "interaction.command", command: { type: "overlay.open", overlay: "models" } }).state) },
    }), [])
  ...
  return { ...setup, keys, executed, workspace: () => observed.workspaces[id]!, close: async () => { await act(async () => setup.renderer.destroy()) } }
}
...
test("colon autocomplete sits above command bar without changing or moving the draft", async () => {
  ...
  try {
    ...
    await h.keys(":")
    await h.keys("he")
    const drawer = h.renderer.root.findDescendantById("command-completion-drawer")!
...
test("explicit model completion selection wins over the bare model command", async () => {
  ...
  try {
    await h.keys(":")
    await h.keys("model ")
    await act(async () => { h.mockInput.pressKey("ARROW_DOWN"); h.mockInput.pressKey("RETURN"); await h.flush() })
    expect(h.executed).toEqual(["model model-b"])
  } finally { await h.close() }
})
...
test("model picker Escape returns a stage and arrows choose an effort", async () => {
  ...
  try {
    ...
    await act(async () => { h.mockInput.pressKey("ESCAPE"); await Bun.sleep(30); await h.flush(); await h.renderOnce() })
    expect(h.captureCharFrame()).toContain("Models")
    expect(h.workspace().interaction.overlay).toBe("models")
    await act(async () => { h.mockInput.pressKey("RETURN"); await h.flush(); await h.renderOnce() })
    await act(async () => { h.mockInput.pressKey("ARROW_UP"); h.mockInput.pressKey("RETURN"); await h.flush() })
...
test("transcript slash remains search without the command completion drawer", async () => {
  ...
  try {
    await h.keys("/")
    await h.keys("needle")
    expect(h.renderer.root.findDescendantById("command-completion-drawer")).toBeUndefined()
    await act(async () => { h.mockInput.pressKey("RETURN"); await h.flush() })
