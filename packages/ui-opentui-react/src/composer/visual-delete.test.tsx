...
import { activeWorkspace, initialWorkbench, transitionWorkbench } from "@vimex/workbench"
...
async function fixture(text = "alpha beta", cursorOffset = 0) {
  let initial = transitionWorkbench(initialWorkbench(), { type: "thread.open", summary: {
    id: threadId("visual-delete"), title: "Visual delete", cwd: "/tmp", model: "test", reasoningEffort: "high", status: "idle",
  } }).state
  initial = transitionWorkbench(initial, { type: "interaction.command", command: { type: "focus.set", surface: "composer" } }).state
  initial = transitionWorkbench(initial, { type: "composer.change", text, cursorOffset }).state
  ...
  return { ...setup, composer: setup.renderer.root.findDescendantById("composer") as TextareaRenderable, state: () => activeWorkspace(observed)! }
}

for (const key of ["d", "x"]) test(`rapid Visual ${key} deletes only the inclusive selection and P restores it`, async () => {
  const setup = await fixture()
  try {
    await act(async () => { await setup.mockInput.typeText(`vl${key}`); await setup.flush() })
    ...
    await act(async () => { await setup.mockInput.typeText("P"); await setup.flush() })
...
test("rapid Visual change enters Insert and accepts replacement text without native double deletion", async () => {
  ...
  try {
    await act(async () => { await setup.mockInput.typeText("vc"); await setup.flush() })
    expect(setup.state().interaction.mode).toBe("insert")
    expect(setup.composer.plainText).toBe("AZ")
    await act(async () => { await setup.mockInput.typeText("new"); await setup.flush() })
