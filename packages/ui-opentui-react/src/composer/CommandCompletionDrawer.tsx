...
export function CommandCompletionDrawer(props: { choices: readonly string[]; selected: number; prefix: ":" | "/"; id: string; hint?: string }) {
  ...
  return <box id={props.id} position="absolute" bottom="100%" left={0} right={0} zIndex={30}
    height={Math.max(1, visible.length) + 2} backgroundColor={emberTide.backgroundPanel} border={["left"]} borderColor={emberTide.blueBright} paddingX={1}>
    <text height={1} fg={emberTide.blueBright}>Commands</text>
    {visible.length ? visible.map((choice, index) => {
    ...
    <text height={1} fg={emberTide.textMuted} wrapMode="none" truncate>{props.hint ?? "↑/↓ choose · tab complete · enter run"}</text>
  </box>
}
