...
export function CommandLine(props: {
  ...
  useEffect(() => {
    ...
    if (input) input.cursorOffset = input.value.length
  }, [props.inputRef])
  ...
  const renameHint =
    ...
    parsedCommand.name === "rename" &&
    props.currentTitle
      ? `Current name: ${props.currentTitle}`
      : undefined
  const completionOptions = {
    models: props.models?.map((model) => model.id),
    modelEfforts: Object.fromEntries(
      props.models?.map((model) => [model.id, model.efforts]) ?? [],
    ),
  ...
  const choices =
    prompt === ":"
      ? commandCompletions(props.value, completionOptions).map((value) =>
          value.slice(1),
        )
      : []
    ...
    flushSync(() =>
      setSelection((current) => ({
        ...
        index: Math.max(
          ...
          Math.min(
            choices.length - 1,
            (current.query === props.value ? current.index : 0) + delta,
          ),
        ),
      })),
    )
  const complete = () => {
    ...
    const current = commandCompletions(live, completionOptions).map((value) =>
      value.slice(1),
    )
    ...
    if (choice)
      flushSync(() => {
        ...
        props.controller.dispatchInteraction({
          type: "command.change",
          value: completed,
        })
      })
  ...
  const submit = () => {
    ...
    const matches = commandCompletions(body, completionOptions).map((value) =>
      value.slice(1),
    )
    const chosen =
      body === commandBody(props.value) &&
      selection.query === props.value &&
      selection.chosen
    const selectedChoice =
      matches[body === commandBody(props.value) ? selected : 0]
    const value =
      prompt !== ":"
        ? `${prompt}${body}`
        : chosen && selectedChoice
          ? selectedChoice
          : parseCommand(body).kind === "command" || !body.trim()
            ? body
            : (selectedChoice ?? body)
    const parsed = prompt === ":" ? parseCommand(value) : undefined
    const invalid =
      parsed?.kind === "command"
        ? validateCommand(parsed, completionOptions)
        : undefined
    if (invalid) {
  ...
  return (
    ...
    >
      {prompt === ":" ? (
        <CommandCompletionDrawer
          choices={choices}
          selected={selected}
          prefix=":"
          id="command-completion-drawer"
          hint={
            validation.query === props.value
              ? validation.message
              : (renameHint ?? "↑/↓ choose · tab complete · ctrl-p/n history")
          }
        />
      ) : null}
      <text flexShrink={0} fg={emberTide.ember}>
        {prompt}
      </text>
      <input
        id="command-line"
        ...
        flexShrink={1}
        minWidth={0}
        value={commandBody(props.value)}
        textColor={emberTide.text}
        cursorColor={emberTide.ember}
        backgroundColor={emberTide.backgroundRaised}
        onInput={(value) =>
          flushSync(() =>
            props.controller.dispatchInteraction({
              type: "command.change",
              value: prompt === ":" ? value : `${prompt}${value}`,
            }),
          )
        }
        onSubmit={submit}
      />
      <text flexShrink={0} marginLeft={1} fg={emberTide.ember}>
        COMMAND
      </text>
    </box>
  )
