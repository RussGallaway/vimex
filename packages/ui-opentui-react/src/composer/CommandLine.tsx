...
export function CommandLine(props: { value: string; models?: readonly AvailableModel[]; inputRef: RefObject<InputRenderable | null>; controller: VimexUiController; onSubmit(value: string): void }) {
  ...
  ] : [] }), [prompt, selected, choices, props.value, props.models, props.controller])
  const submit = () => {
  ...
  return <box id="command-bar" height={1} marginTop={1} flexShrink={0} flexDirection="row" paddingX={2} backgroundColor={emberTide.backgroundRaised}>
    {prompt === ":" ? <CommandCompletionDrawer choices={choices} selected={selected} prefix=":" id="command-completion-drawer" hint="↑/↓ choose · tab complete · ctrl-p/n history" /> : null}
