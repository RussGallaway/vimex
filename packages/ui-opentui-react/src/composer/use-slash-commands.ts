...
import { codeUnitOffsetToGraphemeOffset, parseCommand, validateCommand, type CommandCompletionOptions, type InteractionState } from "@vimex/interaction"
...
export function useSlashCommands(options: {
  ...
  const liveChoice = () => {
    ...
    return slashCommandChoices(text, options.completionOptions)[text === options.text ? liveSelection.current : 0]
  }
  const complete = () => {
  ...
  const execute = () => {
    ...
    const explicitlyChosen = selection.query === options.text && selection.chosen
    const completion = slashCommandChoices(text, options.completionOptions)[text === options.text ? liveSelection.current : 0]
    const command = explicitlyChosen || (typed.kind === "unknown" && completion) ? completion ?? body : body
    const parsed = parseCommand(command)
    const message = parsed.kind === "empty" ? "Enter a command, or use // for a literal message"
      : parsed.kind === "unknown" ? `Unknown command: ${parsed.name}`
        : parsed.name === "submit" ? "Use :submit to send the current draft"
        : validateCommand(parsed, options.completionOptions)?.replace(/^Usage: :/, "Usage: /")
    if (message) {
  ...
  const submit = (intent: SubmissionIntent) => {
    ...
    if (interaction.mode === "insert" && text.startsWith("//")) {
      ...
      flushSync(() => controller.changeDraft(literal, codeUnitOffsetToGraphemeOffset(literal, literal.length)))
  ...
  const feedback = validation.query === options.text ? validation.message : undefined
