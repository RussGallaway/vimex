...
export const transcriptScalingBlockCounts = Object.freeze([
...
function freezeTranscript(state: TranscriptState): TranscriptState {
  return Object.freeze({
    ...
    viewport: Object.isFrozen(state.viewport)
      ? state.viewport
      : Object.freeze({ ...state.viewport }),
    unseenItemIds: persistentTranscriptUnseenItemIds(state.unseenItemIds),
    jumps: Object.isFrozen(state.jumps)
      ? state.jumps
      : Object.freeze({
          back: Object.freeze([...state.jumps.back]),
          forward: Object.freeze([...state.jumps.forward]),
        }),
    marks: Object.isFrozen(state.marks)
      ? state.marks
      : Object.freeze({ ...state.marks }),
  })
}
...
export function buildTranscriptStructuralScalingFixture(
  blockCount: number,
): TranscriptStructuralScalingFixture {
  ...
  for (let index = 0; index < blockCount; index++) {
    ...
    const value = Object.freeze({
      ...
      markdown: `Settled structural block ${index % 16}.`,
      status: "complete" as const,
    })
    turnIds.push(turn)
    turns[turn] = Object.freeze({
      id: turn,
      status: "complete",
      itemIds: persistentTurnItemIds([item]),
    })
    items[item] = value
    projectionById[item] = projectItem(value)
    contentHash = hashText(hashText(contentHash, turn), item)
  }
  const order = persistentTranscriptOrder(
    Object.keys(items).map((id) => itemId(id)),
  )
  ...
  return Object.freeze({
    ...
    contentHash: hashText(contentHash, String(blockCount))
      .toString(16)
      .padStart(8, "0"),
  })
}
 ...
 */
export function buildTranscriptScalingFixture(
  blockCount: number,
): TranscriptScalingFixture {
  if (!Number.isInteger(blockCount) || blockCount < 1)
    throw new RangeError("blockCount must be a positive integer")
  ...
  const width = String(blockCount - 1).length
  const ids = Array.from({ length: blockCount }, (_, index) =>
    scaleItemId(index, width),
  )
  const tailItemId = ids.at(-1)!
  const targetIndex = (ratio: number) =>
    Math.min(blockCount - 1, Math.floor((blockCount - 1) * ratio))
  ...
  for (let index = 0; index < blockCount; index++) {
    ...
    if (index === blockCount - 1) {
      ...
      item = Object.freeze({
        ...
        turnId: itemTurn,
        kind: "user" as const,
        markdown: "Forkable completed user request.",
        status: "complete" as const,
      })
      ...
      item = Object.freeze({
        ...
        kind: "assistant" as const,
        markdown:
          "Selection begins at a [fixture link](https://vimex.test/selection).",
        status: "complete" as const,
      })
      ...
      item = Object.freeze({
        ...
        title: "Selection command boundary",
        detail: "deterministic command output",
        status: "complete" as const,
      })
      ...
      item = Object.freeze({
        ...
        turnId: itemTurn,
        kind: "edit" as const,
        title: "Semantic fixture edit",
        patch,
        changes: Object.freeze([
          Object.freeze({
            path: "semantic-fixture.ts",
      ...
      item = Object.freeze({
        ...
        markdown: "Foldable reasoning with Search sentinel evidence.",
        status: "complete" as const,
      })
    } else {
      item = Object.freeze({
        ...
        turnId: itemTurn,
        kind: "assistant" as const,
        markdown: scaleContent[index % scaleContent.length]!,
        status: "complete" as const,
      })
    }
    const source = scaleItemSource(item)
    contentHash = hashText(
      hashText(hashText(hashText(contentHash, id), item.kind), source),
      item.status,
    )
    items[id] = item
  ...
  const completedTurn = Object.freeze({
    id: historyTurn,
    status: "complete" as const,
    itemIds: Object.freeze(ids.slice(0, -1)),
  })
  const turn = Object.freeze({
    id: fixtureTurn,
    status: "running" as const,
    itemIds: Object.freeze([tailItemId]),
  })
  const conversation: ConversationState = Object.freeze({
    threadId: fixtureThread,
  ...
  const transcript = freezeTranscript({
    ...
    selection: {
      anchor: { itemId: targets.quarter, graphemeOffset: 0 },
      head: { itemId: targets.selectionEnd, graphemeOffset: 25 },
      shape: "character",
    },
    ...
    jumps: {
      back: [
        {
          point: { itemId: targets.first, graphemeOffset: 0 },
          preferredScreenRow: 2,
        },
      ],
      forward: [
        {
          point: { itemId: targets.threeQuarter, graphemeOffset: 0 },
          preferredScreenRow: 9,
        },
      ],
    },
    marks: {
      a: {
        point: { itemId: targets.middle, graphemeOffset: 0 },
  ...
  const afterTailDelta = appendTranscriptScalingTail(
    before,
    tailItemId,
    scaleTailDelta,
  )
  contentHash = hashText(
    hashText(contentHash, scaleTailDelta),
    String(blockCount),
  )
  return Object.freeze({
...
export function appendTranscriptScalingTail(
  snapshot: TranscriptFixtureSnapshot,
  tailItemId: ItemId,
  delta: string,
): TranscriptFixtureSnapshot {
  const prior = snapshot.conversation.items[tailItemId]
  if (!prior || !("markdown" in prior))
    throw new Error(`Missing Markdown tail item ${tailItemId}`)
  const item = Object.freeze({ ...prior, markdown: prior.markdown + delta })
  const conversation = Object.freeze({
    ...snapshot.conversation,
    items: setConversationItem(snapshot.conversation.items, item),
  })
  const transcript = freezeTranscript({
    ...snapshot.transcript,
...
function segmentedSource(
  parts: readonly Readonly<{ stableId: string; value: string }>[],
  separator: string,
): Readonly<{ source: string; segments: readonly FixtureSourceSegment[] }> {
  let source = ""
  ...
  parts.forEach((part, index) => {
    ...
    segments.push(
      Object.freeze({
        ...
        sourceSpan: Object.freeze({ from, to: source.length }),
      }),
    )
  })
...
export function buildOversizedTranscriptFixtures(): readonly OversizedTranscriptFixture[] {
  ...
  const markdownItem = Object.freeze({
    id: itemId("oversized-markdown"),
    turnId: oversizedTurn,
    kind: "assistant" as const,
  ...
  const commandParts = [
    ...
    ...Array.from({ length: 2_500 }, (_, index) => ({
      stableId: `line:${index}`,
      value: `${String(index).padStart(4, "0")}: ${"result ".repeat(12)}`,
    })),
  ]
  const command = segmentedSource(commandParts, "\n")
  const commandItem = Object.freeze({
    id: itemId("oversized-command"),
    turnId: oversizedTurn,
    kind: "command" as const,
    title: commandParts[0]!.value,
    detail: commandParts
      .slice(1)
      .map((part) => part.value)
      .join("\n"),
  ...
  const diffItem = Object.freeze({
    id: itemId("oversized-split-diff"),
    turnId: oversizedTurn,
    kind: "edit" as const,
    title: "128 deterministic file changes",
    patch: diff.source,
    changes: Object.freeze(
      diffParts.map((part) =>
        Object.freeze({
