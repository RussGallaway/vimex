...
const run = (
  ...
  command: Parameters<typeof transitionWorkbench>[1],
) => transitionWorkbench(state, command)
...
describe("workbench", () => {
  test("tracks interruption until the authoritative terminal event and ignores stale summary activity", () => {
    const thread = threadId("a"),
      turn = turnId("turn"),
      item = itemId("thinking")
    let state = run(initialWorkbench(), {
      type: "thread.open",
      summary: { ...summary("a"), status: "working" },
    }).state
    state = run(state, {
  ...
  test("registers background threads without changing the active thread", () => {
    ...
    state = run(state, {
      type: "thread.register",
      summary: { ...summary("background"), title: "renamed" },
    }).state
    expect(
      state.threadOrder.filter((id) => id === threadId("background")),
    ).toHaveLength(1)
  ...
  test("preserves drafts, anchors, folds, and modes per thread", () => {
    ...
    state = run(state, {
      ...
      command: {
        type: "cursor.move",
        point: { itemId: itemA, graphemeOffset: 2 },
        preferredScreenRow: 5,
      },
    }).state
    state = run(state, {
      type: "transcript.command",
      command: { type: "fold.set", itemId: itemA, folded: true },
    }).state
  ...
  test("fold mutations request restoration of the stable semantic anchor", () => {
    ...
    state = run(state, {
      ...
      command: {
        type: "cursor.move",
        point: { itemId: item, graphemeOffset: 1 },
    ...
    const result = run(state, {
      type: "transcript.command",
      command: { type: "fold.toggle", itemId: item },
    })
    expect(result.effects).toEqual([
      { type: "viewport.restore", threadId: thread, anchor },
    ])
  ...
  test("streams without moving pinned views and drains one queued message once", () => {
    ...
    state = run(state, {
      type: "conversation.event",
      event: { type: "turn.started", threadId: thread, turnId: firstTurn },
    }).state
    state = run(state, {
      type: "conversation.event",
      event: {
    ...
    state = run(state, {
      ...
      command: {
        type: "cursor.move",
        point: { itemId: firstItem, graphemeOffset: 0 },
    ...
    const duplicate = run(result.state, {
      type: "conversation.event",
      event: {
        type: "turn.completed",
        threadId: thread,
  ...
  test("keeps protocol telemetry canonical while omitting empty churn from the semantic transcript", () => {
    const thread = threadId("telemetry"),
      turn = turnId("turn"),
    ...
    for (const item of items)
      state = run(state, {
        ...
        event: {
          type: "item.started",
          threadId: thread,
          item: { ...item, turnId: turn },
        },
      }).state
    const workspace = state.workspaces[thread]!
    expect(Object.keys(workspace.conversation.items)).toEqual([
      "reasoning",
      "activity",
      "wait",
      "list",
    ...
    expect(
      workspace.transcript.projectionById[itemId("reasoning")],
    ).toBeUndefined()
    expect(workspace.transcript.projectionById[itemId("spawn")]?.source).toBe(
      "Review the runtime",
    )
  })
  ...
  test("serializes next-turn submissions during the turn-start race and starts an idle steer", () => {
    ...
    expect(result.effects).toEqual([])
    expect(
      activeWorkspace(result.state)?.composer.outbox.find(
        (message) => message.id === "two",
      )?.status,
    ).toBe("queued")
    const duplicate = run(result.state, {
      type: "composer.submit",
  ...
  test("promotes the next queued turn after a start failure and retries failed outgoing messages", () => {
    ...
    ).toMatchObject({ status: "failed", reason: "offline" })
    expect(result.effects).toEqual([
      {
        type: "conversation.turn.start",
        threadId: thread,
  ...
  test("retries failed approval resolution", () => {
    ...
    expect(result.state.approvals.byId.ap).toMatchObject({
      status: "resolving",
      error: undefined,
    })
    expect(result.effects).toEqual([
      { type: "approval.resolve", approvalId: "ap", choiceId: "yes" },
    ])
  })

  test("forks completed history into independent state", () => {
    const source = threadId("a"),
      turn = turnId("t"),
      item = itemId("i")
    let state = run(initialWorkbench(), {
      type: "thread.open",
    ...
    state = run(state, {
      type: "conversation.event",
      event: { type: "turn.started", threadId: source, turnId: turn },
    }).state
    state = run(state, {
      type: "conversation.event",
      event: {
    ...
    state = run(state, {
      type: "transcript.command",
      command: { type: "fold.defaults", reasoning: false, tools: true },
    }).state
    state = run(state, {
      type: "transcript.command",
      command: { type: "fold.set", itemId: item, folded: false },
    }).state
    state = run(state, {
      type: "conversation.event",
      event: {
    ...
    expect(state.workspaces[threadId("child")]?.composer.text).toBe(
      "child draft",
    )
    expect(state.workspaces[threadId("child")]?.conversation).not.toBe(
      state.workspaces[source]?.conversation,
    )
    expect(
      state.workspaces[threadId("child")]?.transcript.foldDefaults,
    ).toEqual({ reasoning: false, tools: true })
    expect(state.workspaces[threadId("child")]?.transcript.folded[item]).toBe(
      false,
    )
    const childTurn = turnId("child-turn"),
      childItem = itemId("child-tool")
    state = run(state, {
      type: "conversation.event",
      event: {
        type: "turn.started",
    ...
    expect(
      state.workspaces[threadId("child")]?.transcript.folded[childItem],
    ).toBe(true)
  })
