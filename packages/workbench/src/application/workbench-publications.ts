...
export function captureWorkbenchPublicationContext(
  state: WorkbenchState,
  previous?: WorkbenchPublicationContext,
): WorkbenchPublicationContext {
  const reuse =
    previous !== undefined &&
    previous.activeThreadId === state.activeThreadId &&
    previous.sideChats === state.sideChats
  const candidate = reuse ? previous.candidate : currentSideChat(state)
  const ready = Boolean(
    candidate?.threadId && state.workspaces[candidate.threadId],
  )
  const side =
    candidate &&
    state.workspaces[candidate.parentId] &&
    (candidate.status === "creating" || ready)
      ? candidate
      : undefined
  return Object.freeze({
    ...
    candidate,
    side,
    sideThreadReady: Boolean(side && ready),
  })
}

export function threadForPresentation(
  state: WorkbenchState,
  presentationId: TranscriptPresentationId,
  context = captureWorkbenchPublicationContext(state),
): ThreadId | undefined {
  const { side } = context
  return presentationId === "main"
    ? (side?.parentId ?? state.activeThreadId)
    : side?.threadId
}

export function captureWorkbenchLayout(
  state: WorkbenchState,
  context = captureWorkbenchPublicationContext(state),
...
export function workbenchLayoutChanged(
  before: WorkbenchState,
  after: WorkbenchState,
  beforeContext = captureWorkbenchPublicationContext(before),
  afterContext = captureWorkbenchPublicationContext(after, beforeContext),
): boolean {
  if (
    before.activeThreadId !== after.activeThreadId ||
    before.preferences?.theme !== after.preferences?.theme
  )
    return true
  const active = after.activeThreadId
  if (
    (active ? before.workspaces[active]?.interaction.overlay : undefined) !==
    (active ? after.workspaces[active]?.interaction.overlay : undefined)
  )
    return true
  if (
    beforeContext.side !== afterContext.side ||
    beforeContext.sideThreadReady !== afterContext.sideThreadReady
  )
    return true
  const leftActivity = liveActivity(before, beforeContext.side?.parentId),
    rightActivity = liveActivity(after, afterContext.side?.parentId)
  return (
    leftActivity.working !== rightActivity.working ||
    leftActivity.label !== rightActivity.label ||
    leftActivity.startedAt !== rightActivity.startedAt
  )
}

function activeTurnChanged(
  before: ThreadWorkspace | undefined,
  after: ThreadWorkspace | undefined,
): boolean {
  const leftId = before?.conversation.activeTurnId,
    rightId = after?.conversation.activeTurnId
  if (leftId !== rightId) return true
  return (
    (leftId ? before?.conversation.turns[leftId]?.startedAt : undefined) !==
    (rightId ? after?.conversation.turns[rightId]?.startedAt : undefined)
  )
}

function pendingApproval(state: WorkbenchState, id: ThreadId | undefined) {
  if (!id) return undefined
  return state.approvals.order
    .map((approvalId) => state.approvals.byId[approvalId])
    .find(
      (approval) =>
        approval?.threadId === id &&
...
function pendingQuestion(state: WorkbenchState, id: ThreadId | undefined) {
  return id
    ? Object.values(state.questions).find(
        (question) => question.threadId === id,
      )
    : undefined
}
 ...
 */
export function workbenchPresentationChanged(
  before: WorkbenchState,
  after: WorkbenchState,
  presentationId: TranscriptPresentationId,
  beforeContext = captureWorkbenchPublicationContext(before),
  ...
  const leftId = threadForPresentation(before, presentationId, beforeContext),
    rightId = threadForPresentation(after, presentationId, afterContext)
  if (leftId !== rightId) return true
  if (
  ...
  const leftInteractive =
    before.activeThreadId === leftId &&
    (presentationId === "main" || Boolean(leftId))
  const rightInteractive =
    after.activeThreadId === rightId &&
    (presentationId === "main" || Boolean(rightId))
  ...
  if (
    ...
    before.approvals.order.length !== after.approvals.order.length
  )
    return true
  if (
    before.questions !== after.questions &&
    Object.keys(before.questions).length !== Object.keys(after.questions).length
  )
    return true
  ...
  if (
    ...
    left.transcript.unseenEntries !== right.transcript.unseenEntries ||
    activeTurnChanged(left, right)
  )
    return true
  if (
    before.summaries[leftId!] !== after.summaries[rightId!] ||
    before.compactingThreads[leftId!] !== after.compactingThreads[rightId!] ||
    before.interruptingTurns[leftId!] !== after.interruptingTurns[rightId!]
  )
    return true
  ...
  if (
    before.approvals !== after.approvals &&
    pendingApproval(before, leftId) !== pendingApproval(after, rightId)
  )
    return true
  if (
    before.questions !== after.questions &&
    pendingQuestion(before, leftId) !== pendingQuestion(after, rightId)
  )
    return true

  if (!rightInteractive) return false
  const leftUrlChoices =
    before.urlChoiceOwner?.presentationId === presentationId
      ? before.urlChoices
      : undefined
  const rightUrlChoices =
    after.urlChoiceOwner?.presentationId === presentationId
      ? after.urlChoices
      : undefined
  if (
    before.availableModels !== after.availableModels ||
    before.modelCatalogError !== after.modelCatalogError ||
    before.threadOrder !== after.threadOrder ||
    before.pendingFork !== after.pendingFork ||
    leftUrlChoices !== rightUrlChoices
  )
    return true
  if (
    (right.interaction.overlay === "sessions" ||
  ...
  if (
    right.interaction.overlay === "sessions" &&
    before.favoriteThreadIds !== after.favoriteThreadIds
  )
    return true
  return false
}
...
export function captureWorkbenchPresentation(
  ...
  return Object.freeze({
    ...state,
    activeThreadId: threadForPresentation(state, presentationId, context),
  })
}
