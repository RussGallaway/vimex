...
import { graphemeCount, clampTranscript, type JumpLocation, type TranscriptState } from "@vimex/transcript"
...
function savedLocation(transcript: TranscriptState, location: JumpLocation): SavedTranscriptLocation | undefined {
  const projection = transcript.projectionById[location.point.itemId]
  if (!projection) return undefined
  return {
    itemId: location.point.itemId,
    sourceOffset: projection.sourceSpans[location.point.graphemeOffset]?.from ?? projection.source.length,
    preferredScreenRow: location.preferredScreenRow,
  }
}

const sameArray = <T>(left: readonly T[] | undefined, right: readonly T[] | undefined) => left === right
  || Boolean(left && right && left.length === right.length && left.every((value, index) => value === right[index]))
const samePoint = (left: { itemId: string; graphemeOffset: number } | undefined, right: { itemId: string; graphemeOffset: number } | undefined) => left === right
  || Boolean(left && right && left.itemId === right.itemId && left.graphemeOffset === right.graphemeOffset)
const sameLocation = (left: SavedTranscriptLocation | undefined, right: SavedTranscriptLocation | undefined) => left === right
  || Boolean(left && right && left.itemId === right.itemId && left.sourceOffset === right.sourceOffset && left.preferredScreenRow === right.preferredScreenRow)
function sameRecord<T>(left: Readonly<Record<string, T>>, right: Readonly<Record<string, T>>, same: (left: T, right: T) => boolean): boolean {
...
function sameSavedLocations(before: TranscriptState, after: TranscriptState): boolean {
  const sameJump = (left: JumpLocation, right: JumpLocation) => sameLocation(savedLocation(before, left), savedLocation(after, right))
  const sameJumps = (left: readonly JumpLocation[], right: readonly JumpLocation[]) => left === right && before.projectionById === after.projectionById
    || left.length === right.length && left.every((value, index) => sameJump(value, right[index]!))
  const beforeMarks = Object.keys(before.marks), afterMarks = Object.keys(after.marks)
  const sameMarks = before.marks === after.marks && before.projectionById === after.projectionById
    || beforeMarks.length === afterMarks.length && beforeMarks.every(name => Object.hasOwn(after.marks, name) && sameJump(before.marks[name]!, after.marks[name]!))
  return sameMarks
    && sameJumps(before.jumps.back, after.jumps.back)
    && sameJumps(before.jumps.forward, after.jumps.forward)
}
function sameWorkspaceView(before: ThreadWorkspace, after: ThreadWorkspace): boolean {
  if (before === after) return true
  const beforeVisited = Boolean(before.conversation.turnIds.length || before.composer.revision || before.transcript.cursor)
  const afterVisited = Boolean(after.conversation.turnIds.length || after.composer.revision || after.transcript.cursor)
  const sameOutbox = before.composer.outbox === after.composer.outbox || before.composer.outbox.length === after.composer.outbox.length
    && before.composer.outbox.every((message, index) => {
      const candidate = after.composer.outbox[index]
      return candidate && message.id === candidate.id && message.text === candidate.text && message.intent === candidate.intent
        && message.status === candidate.status && message.reason === candidate.reason
    })
  const sameViewport = before.transcript.viewport === after.transcript.viewport
    || before.transcript.viewport.kind === "tail" && after.transcript.viewport.kind === "tail"
    || before.transcript.viewport.kind === "point" && after.transcript.viewport.kind === "point"
      && samePoint(before.transcript.viewport.point, after.transcript.viewport.point)
      && before.transcript.viewport.preferredScreenRow === after.transcript.viewport.preferredScreenRow
  return beforeVisited === afterVisited
    ...
    && sameRecord(before.transcript.folded, after.transcript.folded, (left, right) => left === right)
    && samePoint(before.transcript.cursor, after.transcript.cursor)
    && sameViewport
    && sameSavedLocations(before.transcript, after.transcript)
}
function sameSavedSideChat(left: import("./side-chat").SideChat, right: import("./side-chat").SideChat): boolean {
  return left.parentId === right.parentId && left.threadId === right.threadId && left.visible === right.visible
    && left.maximized === right.maximized && left.contextLabel === right.contextLabel
    && sameArray(left.inheritedTurnIds, right.inheritedTurnIds)
}

/** Fast relevance gate for the serialized local-view read model. */
export function localViewChanged(before: WorkbenchState, after: WorkbenchState, changedThreadIds?: readonly string[]): boolean {
  if (!sameArray(before.favoriteThreadIds, after.favoriteThreadIds)
    || !sameArray(before.retiredSideThreadIds, after.retiredSideThreadIds)
    || !sameRecord(before.sideChats, after.sideChats, sameSavedSideChat)) return true
  if (before.workspaces === after.workspaces) return false
  if (changedThreadIds) return changedThreadIds.some(id => {
    const left = before.workspaces[id], right = after.workspaces[id]
...
export function captureLocalState(state: WorkbenchState, previous: LocalState): LocalState {
  ...
  for (const [id, workspace] of Object.entries(state.workspaces)) {
    ...
    threads[id] = {
      ...
      marks: Object.fromEntries(Object.entries(transcript.marks).flatMap(([name, location]) => { const saved = savedLocation(transcript, location); return saved ? [[name, saved]] : [] })),
      jumps: { back: transcript.jumps.back.flatMap(location => { const saved = savedLocation(transcript, location); return saved ? [saved] : [] }), forward: transcript.jumps.forward.flatMap(location => { const saved = savedLocation(transcript, location); return saved ? [saved] : [] }) },
    }
  }
