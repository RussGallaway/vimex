...
const parent = threadId("parent"), side = threadId("side")
const summary = (id: typeof parent): ThreadSummary => ({ id, title: id, cwd: "/tmp", model: "test", reasoningEffort: "high", status: "working" })
const run = (state: WorkbenchState, command: WorkbenchCommand) => transitionWorkbench(state, command).state

function splitState(): WorkbenchState {
  let state = run(initialWorkbench(), { type: "thread.open", summary: summary(parent) })
  state = run(state, { type: "thread.open", summary: summary(side) })
  state = { ...state, sideChats: { [parent]: { parentId: parent, threadId: side, visible: true, maximized: false } } }
  for (const [thread, suffix] of [[parent, "parent"], [side, "side"]] as const) {
    state = run(state, { type: "conversation.event", event: { type: "turn.started", threadId: thread, turnId: turnId(`${suffix}-turn`) } })
    state = run(state, { type: "conversation.event", event: { type: "item.started", threadId: thread, item: { id: itemId(`${suffix}-item`), turnId: turnId(`${suffix}-turn`), kind: "assistant", markdown: suffix, status: "running" } } })
  }
  return state
}
...
test("canonical token content is owned by presentation runtimes, not pane publications", () => {
  ...
  const after = run(before, { type: "conversation.event", event: { type: "item.delta", threadId: parent, itemId: itemId("parent-item"), delta: " grows" } })
...
test("cached publication context keeps token settlements independent of retained relationship counts", () => {
  ...
  const guardedSideChats = new Proxy(base.sideChats, { ownKeys() { throw new Error("side chats scanned") } })
  ...
  const changed = run(base, { type: "conversation.event", event: { type: "item.delta", threadId: parent, itemId: itemId("parent-item"), delta: " grows" } })
  const before = { ...base, sideChats: guardedSideChats, agentRelationships: guardedRelationships }
  const after = { ...changed, sideChats: guardedSideChats, agentRelationships: guardedRelationships }
  const candidate = base.sideChats[parent]!
  const beforeContext: WorkbenchPublicationContext = Object.freeze({
    activeThreadId: before.activeThreadId, sideChats: guardedSideChats, candidate, side: candidate, sideThreadReady: true,
  })
  const afterContext = captureWorkbenchPublicationContext(after, beforeContext)
  expect(afterContext.candidate).toBe(candidate)
  expect(workbenchLayoutChanged(before, after, beforeContext, afterContext)).toBe(false)
  expect(workbenchPresentationChanged(before, after, "main", beforeContext, afterContext)).toBe(false)
  expect(workbenchPresentationChanged(before, after, "side", beforeContext, afterContext)).toBe(false)
})

test("pane-local state and focus invalidate only the semantic consumers", () => {
  const before = splitState()
  const parentDraft = run(before, { type: "composer.change", threadId: parent, text: "parent draft", cursorOffset: 4 })
...
test("semantic status changes invalidate their pane while background metadata stays scoped", () => {
  const before = splitState()
  const parentUsage = run(before, { type: "thread.summary.patch", threadId: parent, patch: { contextUsed: 500, contextLimit: 10_000 } })
  ...
  const approval = run(parentUsage, { type: "approval.received", approval: {
    id: "side-approval", threadId: side, kind: "command", title: "Run", detail: "command",
...
test("a restored side association without its parent workspace cannot blank the main presentation", () => {
  const child = run(initialWorkbench(), { type: "thread.open", summary: summary(side) })
  const restored = { ...child, sideChats: { [parent]: { parentId: parent, threadId: side, visible: true, maximized: false } } }
  expect(captureWorkbenchLayout(restored).side).toBeUndefined()
  expect(threadForPresentation(restored, "main")).toBe(side)
  expect(captureWorkbenchPresentation(restored, "main").activeThreadId).toBe(side)
})
...
test("global approval and question counts invalidate an empty main presentation", () => {
  ...
  const approval = run(before, { type: "approval.received", approval: {
    id: "background-approval", threadId: parent, kind: "command", title: "Run", detail: "command",
    choices: [{ id: "yes", label: "Yes" }], status: "pending",
  } })
  expect(workbenchPresentationChanged(before, approval, "main")).toBe(true)

  const question = run(before, { type: "question.received", request: {
    id: "background-question", threadId: parent, turnId: turnId("background-turn"), questions: [{ id: "choice", header: "Choose", question: "Choose", allowOther: false, secret: false, options: [
...
test("an unrelated retained side-chat mutation leaves active publications stable", () => {
  const before = splitState()
  const otherParent = threadId("other-parent"), otherSide = threadId("other-side")
  const after = { ...before, sideChats: { ...before.sideChats, [otherParent]: { parentId: otherParent, threadId: otherSide, visible: false, maximized: false } } }
  expect(workbenchLayoutChanged(before, after)).toBe(false)
  expect(workbenchPresentationChanged(before, after, "main")).toBe(false)
  expect(workbenchPresentationChanged(before, after, "side")).toBe(false)
})
