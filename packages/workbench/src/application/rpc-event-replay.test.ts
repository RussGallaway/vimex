...
import { createConversation, reduceConversation, threadId, turnId } from "@vimex/conversation"
...
test("RPC replay admits only timing enrichment for already observed turns", () => {
  ...
  const startTiming = { type: "turn.started" as const, threadId: thread, turnId: turn, startedAt: 1_000 }
  ...
  conversation = reduceConversation(conversation, { type: "turn.completed", threadId: thread, turnId: turn, outcome: "complete" })
  replay = createRpcEventReplay(conversation)
  const completionTiming = { type: "turn.completed" as const, threadId: thread, turnId: turn, outcome: "failed" as const, completedAt: 3_500, durationMs: 2_500 }
  expect(replay(conversation, completionTiming)).toBe(true)
  conversation = reduceConversation(conversation, completionTiming)
  expect(conversation.turns[turn]).toMatchObject({ status: "complete", startedAt: 1_000, completedAt: 3_500, durationMs: 2_500 })
