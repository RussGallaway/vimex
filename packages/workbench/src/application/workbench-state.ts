...
export interface WorkbenchState {
  ...
  urlChoiceOwner?: Readonly<{
    threadId: ThreadId
    presentationId: import("./workbench-actions").TranscriptPresentationId
...
export interface WorkbenchTransition {
  ...
  effects: readonly WorkbenchEffect[]
}

type TranscriptNavigationCommand = Extract<
  TranscriptCommand,
  {
    type:
      "search.set" | "search.jump" | "jump.to" | "mark.jump" | "cursor.reveal"
  }
>
...
export type WorkbenchCommand =
  | {
      ...
      observation: import("./compaction").CompactionObservation
    }
  ...
  | {
      ...
      threadId?: ThreadId
      command: TranscriptNavigationCommand
      focusMode?: "normal" | "visual"
  ...
  | {
      ...
      owner?: WorkbenchState["urlChoiceOwner"]
      url?: string
    }
  | {
...
export function updateWorkspace(
  state: WorkbenchState,
  id: ThreadId,
  update: (workspace: ThreadWorkspace) => ThreadWorkspace,
): WorkbenchState {
  ...
  return next === workspace
    ? state
    : { ...state, workspaces: { ...state.workspaces, [id]: next } }
}
export function activeWorkspace(
  state: WorkbenchState,
): ThreadWorkspace | undefined {
  return state.activeThreadId
    ? state.workspaces[state.activeThreadId]
    : undefined
}
export function targetThread(
  ...
  return requested ?? state.activeThreadId
}
