...
export class SideChatCoordinator {
  ...
  action(action: SideChatAction, question?: string): void {
    ...
    if (action === "open") {
      if (side?.status === "quitting") { this.host.notice("Side chat is quitting"); return }
      if (side) {
        this.host.update({ ...side, visible: true })
        if (side.threadId) {
          const id = side.threadId
          const focus = this.host.focus(id)
          if (question?.trim()) this.host.launch(async () => {
            await focus
            const current = this.host.state().sideChats[side!.parentId]
            if (current?.threadId === id && current.status !== "quitting" && this.host.state().connection === "connected") this.host.send(id, question.trim())
          })
        } else if (question?.trim()) this.pendingQuestions.set(side.parentId, [...(this.pendingQuestions.get(side.parentId) ?? []), question.trim()])
        return
      }
      side = { parentId: active, visible: true, maximized: false, status: "creating", contextLabel: state.summaries[active]?.title ?? active }
      this.host.update(side)
      this.pendingQuestions.set(active, question?.trim() ? [question.trim()] : [])
      this.host.launch(async () => {
        try {
          ...
          throw error
        } finally { this.pendingQuestions.delete(active) }
      })
      return
    }
    if (!side) { this.host.notice("No side chat for this session"); return }
    if (action === "close") {
    ...
    } else if (action === "maximize" || action === "reset") this.host.update({ ...side, maximized: action === "maximize" ? !side.maximized : false })
      ...
      const latest = workspace?.transcript.order.slice(-50).map(id => workspace.transcript.projectionById[id]?.source ?? "").join("\n\n").slice(-48_000) ?? ""
      this.host.send(side.threadId, `Latest parent activity (a snapshot, not live context):\n\n${latest}`)
  ...
  private async retire(side: SideChat): Promise<void> {
    ...
    try {
      ...
      if (this.host.state().activeThreadId === id) await this.host.focus(side.parentId)
