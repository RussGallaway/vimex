...
export async function executeGoalCommand(
  ...
  if (!gateway.getGoal || !gateway.setGoal || !gateway.clearGoal)
    throw new Error("Goals are unavailable on this connection")
  if (command.kind === "show") return describeGoal(await gateway.getGoal(id))
  if (command.kind === "clear")
    return (await gateway.clearGoal(id)) ? "Goal cleared" : "No goal to clear"
  if (command.kind === "status")
    return describeGoal(await gateway.setGoal(id, { status: command.status }))
  if (command.kind === "set")
    return describeGoal(
      await gateway.setGoal(id, {
