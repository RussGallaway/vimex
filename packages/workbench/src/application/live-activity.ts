...
export interface LiveActivity { working: boolean; label?: string; startedAt?: number }
...
export function liveActivity(state: WorkbenchState, threadId: ThreadId | undefined = state.activeThreadId): LiveActivity {
  ...
  if (threadId && turnId && state.interruptingTurns[threadId] === turnId) return { working: true, label: "Stopping", startedAt: turn?.startedAt }
  if (threadId && state.compactingThreads[threadId]) return { working: true, label: "Compacting", startedAt: turn?.startedAt }
