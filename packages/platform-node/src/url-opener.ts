...
export function urlCommand(target: string, platform: NodeJS.Platform = process.platform): UrlCommand {
  ...
  if (platform === "win32") return { executable: "rundll32.exe", args: ["url.dll,FileProtocolHandler", url.href] }
...
export async function openUrl(target: string): Promise<void> {
  ...
  await new Promise<void>((resolve, reject) => {
    ...
    child.once("exit", code => code === 0 ? resolve() : reject(new Error(`URL opener exited ${code}`)))
  })
}
