...
async function fixture(link = false, complete = true) {
  const directory = await mkdtemp(join(tmpdir(), "vimex-distribution-")),
    root = join(directory, "install"),
    source = join(directory, "bundle")
  await mkdir(join(root, "versions", "old"), { recursive: true })
  await mkdir(join(source, "assets"), { recursive: true })
  await writeFile(join(root, "versions", "old", "vimex"), "old binary")
  await symlink("versions/old", join(root, "current"))
  await writeFile(
    join(root, ".vimex-install.json"),
    JSON.stringify({ schemaVersion: 1, kind: "direct", version: "0.1.0" }),
  )
  ...
  if (complete) {
    ...
    await writeFile(
      join(source, "share", "man", "man1", "vimex.1"),
      ".TH VIMEX 1\n",
    )
  }
  ...
  expect(
    (await runProcess("tar", ["-czf", archive, "-C", source, "."])).code,
  ).toBe(0)
...
test("direct upgrade atomically switches a verified executable and assets, preserving configuration and old version", async () => {
  ...
  try {
    expect(
      await detectInstallation(join(f.root, "current", "vimex")),
    ).toMatchObject({ kind: "direct", root: await realpath(f.root) })
    await installDirect(f.release, f.artifact, f.root, {
      download: async () => f.bytes,
    })
    expect(await readlink(join(f.root, "current"))).toStartWith(
      "versions/0.2.0-darwin-arm64-",
    )
    expect(await readFile(join(f.root, "current", "vimex"), "utf8")).toBe(
      "new binary",
    )
    expect(
      await readFile(join(f.root, "current", "assets", "parser.wasm"), "utf8"),
    ).toBe("parser asset")
    expect(
      await readFile(join(f.root, "versions", "old", "vimex"), "utf8"),
    ).toBe("old binary")
    expect(await readFile(join(f.root, "config.json"), "utf8")).toBe(
      "user configuration",
    )
  } finally {
    await f.close()
...
test("checksum failures, interrupted downloads, and unsafe archive links preserve current", async () => {
  const f = await fixture(),
    unsafe = await fixture(true),
    incomplete = await fixture(false, false)
  try {
    await expect(
      installDirect(
        f.release,
    ...
    await expect(
      installDirect(f.release, f.artifact, f.root, {
        download: async () => {
    ...
    await expect(
      installDirect(unsafe.release, unsafe.artifact, unsafe.root, {
        download: async () => unsafe.bytes,
      }),
    ).rejects.toThrow("links or special")
    await expect(
      installDirect(incomplete.release, incomplete.artifact, incomplete.root, {
        download: async () => incomplete.bytes,
      }),
    ).rejects.toThrow("Incomplete release archive")
    ...
    expect(await readlink(join(incomplete.root, "current"))).toBe(
      "versions/old",
    )
  } finally {
    await f.close()
...
test("ownership checks refuse current bundles outside the installation", async () => {
  ...
  try {
    ...
    await symlink(f.directory, join(f.root, "current"))
    await expect(
      installDirect(f.release, f.artifact, f.root, {
        download: async () => {
...
test("Homebrew delegates exact command; doctor only probes versions and reads configuration", async () => {
  const calls: string[][] = []
  const run = async (name: string, args: readonly string[]) => {
    calls.push([name, ...args])
    return { code: 0, stdout: `${name} version 1`, stderr: "" }
  }
  ...
  try {
    ...
    const checks = await diagnoseNode({
      version: "0.1.0",
      config,
      cwd: f.directory,
      installation: async () => ({ kind: "source" }),
      run,
    })
    ...
    expect(await readFile(config, "utf8")).toBe(
      JSON.stringify({ codexExecutable: "/fake/codex" }),
    )
  } finally {
    await f.close()
  }
...
test("GitHub manifest must match its release tag and request failures are explicit", async () => {
  ...
  const releases = new GithubReleases({
    fetch: (async (input: string | URL | Request) => {
      ...
      return new Response(JSON.stringify({ version: "9.0.0", artifacts: [] }), {
        status: 200,
      })
    }) as typeof fetch,
  })
  ...
  expect(seen).toEqual([
    "https://github.com/RussGallaway/vimex/releases/download/v0.2.0/vimex-release.json",
  ])
  const broken = new GithubReleases({
    fetch: (async () =>
...
test("receipt refresh failure reports installed warning after the atomic switch", async () => {
  const f = await fixture()
  try {
    const result = await installDirect(f.release, f.artifact, f.root, {
      download: async () => f.bytes,
      run: async (command, args, options) => {
        const result = await runProcess(command, args, options)
        if (args[0] === "-xzf") {
          await rm(join(f.root, ".vimex-install.json"))
          await mkdir(join(f.root, ".vimex-install.json"))
        }
        return result
      },
    })
...
test("diagnostic subprocess timeout is bounded even when SIGTERM is ignored", async () => {
  const directory = await mkdtemp(join(tmpdir(), "vimex-timeout-")),
  ...
  try {
    ...
    await expect(
      runProcess(process.execPath, ["-e", script], { timeoutMs: 100 }),
...
test("Homebrew libexec symlink ownership routes upgrades to the package manager", async () => {
  ...
  try {
    const cellar = join(directory, "Cellar/vimex/0.1.0/libexec"),
    ...
    expect(await detectInstallation(join(bin, "vimex"))).toEqual({
      kind: "homebrew",
    })
  } finally {
    await rm(directory, { recursive: true, force: true })
