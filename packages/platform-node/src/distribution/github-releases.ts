...
export class GithubReleases {
  ...
  private async json(url: string): Promise<unknown> {
    ...
    if (!response.ok)
      throw new Error(`Release request failed (${response.status}): ${url}`)
    return response.json()
  }
  async get(requestedVersion?: string): Promise<Release> {
    let version = requestedVersion && normalizeVersion(requestedVersion)
    if (!version) {
      const latest = (await this.json(
        `https://api.github.com/repos/${repository}/releases/latest`,
      )) as { tag_name?: unknown }
      if (!latest || typeof latest.tag_name !== "string")
        throw new Error("GitHub returned no release version")
      version = normalizeVersion(latest.tag_name)
    }
    const release = parseRelease(
      await this.json(
        `https://github.com/${repository}/releases/download/v${version}/vimex-release.json`,
      ),
    )
  ...
  async download(version: string, name: string): Promise<Uint8Array> {
    ...
    if (!/^[A-Za-z0-9][A-Za-z0-9._-]*\.tar\.gz$/.test(name))
