...
import { normalizeVersion, type Release, type ReleaseArtifact } from "@vimex/distribution"
import { runProcess, type RunProcess } from "./homebrew"

export interface DirectInstallerOptions { download(version: string, name: string): Promise<Uint8Array>; run?: RunProcess }
/** A versioned bundle is staged beside current; a single symlink rename commits it. */
export async function installDirect(release: Release, artifact: ReleaseArtifact, root: string, options: DirectInstallerOptions): Promise<{ warning?: string }> {
  normalizeVersion(release.version)
  const receipt = JSON.parse(await readFile(join(root, ".vimex-install.json"), "utf8")) as { schemaVersion?: unknown; kind?: unknown }
  if (receipt.schemaVersion !== 1 || receipt.kind !== "direct") throw new Error("Refusing to replace an installation without a direct-install receipt")
  const current = await lstat(join(root, "current"))
  if (!current.isSymbolicLink()) throw new Error("Direct installation current path must be a symlink")
  const run = options.run ?? runProcess
  const versions = join(root, "versions")
  const versionsStat = await lstat(versions)
  if (!versionsStat.isDirectory() || versionsStat.isSymbolicLink()) throw new Error("Direct installation versions must be a real directory")
  const actualRoot = await realpath(root)
  const actualVersions = await realpath(versions)
  const actualCurrent = await realpath(join(root, "current"))
  if (actualVersions !== join(actualRoot, "versions") || !relative(actualVersions, actualCurrent) || relative(actualVersions, actualCurrent).startsWith("..") || relative(actualVersions, actualCurrent).includes(sep)) throw new Error("Direct installation current bundle is outside its owned versions directory")
  const lock = join(root, ".upgrade-lock")
  try { await mkdir(lock) } catch { throw new Error("Another upgrade is running, or a previous upgrade left .upgrade-lock; inspect it before retrying") }
  let stage: string | undefined, replacement: string | undefined, bundle: string | undefined
  let committed = false
  try {
    const bytes = await options.download(release.version, artifact.name)
    const checksum = createHash("sha256").update(bytes).digest("hex")
    if (checksum !== artifact.sha256.toLowerCase()) throw new Error("Artifact checksum mismatch; installation was not changed")
    ...
    try {
      await writeFile(receiptPath, JSON.stringify({ schemaVersion: 1, kind: "direct", version: release.version }) + "\n")
      await rename(receiptPath, join(root, ".vimex-install.json"))
    } catch { warning = "The new version is active, but the installation receipt version could not be refreshed." }
    finally { await rm(receiptPath, { force: true }).catch(() => {}) }
