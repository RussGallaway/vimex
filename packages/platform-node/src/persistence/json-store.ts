...
export class JsonStore<T> {
  ...
  constructor(readonly path: string, private readonly validate: (value: unknown) => T) {}

  async read(fallback: T): Promise<T> {
    try { return this.validate(JSON.parse(await readFile(this.path, "utf8"))) }
    catch (error) {
      if ((error as NodeJS.ErrnoException).code === "ENOENT") return fallback
      throw new Error(`Unable to load ${this.path}: ${error instanceof Error ? error.message : String(error)}`)
    }
  }
