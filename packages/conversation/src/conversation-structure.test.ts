...
function reducerDiagnostics(): ConversationItemRecordDiagnostics & ConversationStructureDiagnostics {
...
describe("persistent canonical conversation structure", () => {
  test("retains array and record reflection, JSON, special ids, and immutable snapshots", () => {
    ...
    const originalTurnIds = persistentConversationTurnIds([
      turnId("named-first"), turnId("10"), turnId("2"), turnId("__proto__"), turnId("constructor"), turnId("toString"),
    ], diagnostics)
    const turnIds = appendConversationTurnId(originalTurnIds, turnId("named-last"), diagnostics)
    const originalItemIds = persistentTurnItemIds([itemId("__proto__"), itemId("constructor")], diagnostics)
    const itemIds = appendTurnItemId(originalItemIds, itemId("toString"), diagnostics)
    let turns = persistentConversationTurns()
    for (const id of turnIds) turns = setConversationTurn(turns, { id, status: "complete", itemIds }, diagnostics)

    expect(Array.isArray(turnIds)).toBe(true)
    expect(turnIds.map(String)).toEqual(["named-first", "10", "2", "__proto__", "constructor", "toString", "named-last"])
    ...
    expect(Object.getOwnPropertyDescriptor(turnIds, "0")).toMatchObject({ enumerable: true, writable: false })
    ...
    expect(Reflect.defineProperty(turnIds, "7", { value: turnId("extra") })).toBe(false)
    ...
    expect(itemIds.map(String)).toEqual(["__proto__", "constructor", "toString"])
    ...
    expect(Object.keys(turns)).toEqual(["2", "10", "named-first", "__proto__", "constructor", "toString", "named-last"])
    expect(Object.keys(JSON.parse(JSON.stringify(turns)))).toEqual(Object.keys(turns))
    expect(Object.hasOwn(turns, "__proto__")).toBe(true)
    expect(turns["__proto__"]?.id).toBe(turnId("__proto__"))
    expect(({ ...turns } as Record<string, Turn>)["constructor"]).toBe(turns["constructor"])
    expect(Reflect.set(turns, "extra", turns["2"]!)).toBe(false)
    expect(Reflect.deleteProperty(turns, "2")).toBe(false)
    expect(Reflect.defineProperty(turns, "extra", { value: turns["2"] })).toBe(false)
    ...
    expect(originalTurnIds.map(String)).toEqual(["named-first", "10", "2", "__proto__", "constructor", "toString"])
  ...
  for (const count of [100, 1_000, 10_000, 100_000]) {
    test(`admits one tail turn and item with logarithmic path copying at ${count.toLocaleString()} turns`, () => {
      ...
      const turnEvent = { type: "turn.started" as const, threadId: before.threadId, turnId: nextTurnId }
      const itemEvent = {
        type: "item.started" as const,
        threadId: before.threadId,
        item: { id: nextItemId, turnId: nextTurnId, kind: "assistant" as const, markdown: "tail", status: "running" as const },
      }
      const diagnostics = reducerDiagnostics()
      const withTurn = reduceConversationWithDiagnostics(before, turnEvent, diagnostics)
      const after = reduceConversationWithDiagnostics(withTurn, itemEvent, diagnostics)
      const referenceWithTurn = reduceConversationReference(before, turnEvent)
      const reference = reduceConversationReference(referenceWithTurn, itemEvent)
      const logarithmicBound = Math.ceil(Math.log2(count + 2)) + 3

      expect(after).toEqual(reference)
      expect(JSON.parse(JSON.stringify(after))).toEqual(JSON.parse(JSON.stringify(reference)))
      ...
      expect(isConversationTurnIdAppend(before.turnIds, after.turnIds, nextTurnId)).toBe(true)
      expect(isConversationTurnUpdate(before.turns, withTurn.turns, nextTurnId, undefined, emptyTurn)).toBe(true)
      expect(isConversationTurnUpdate(withTurn.turns, after.turns, nextTurnId, emptyTurn, nextTurn)).toBe(true)
      expect(isConversationTurnAdditionThenItemAppend(before.turns, after.turns, nextTurnId, nextItemId, nextTurn)).toBe(true)
      expect(isTurnItemIdAppend(emptyTurn.itemIds, nextTurn.itemIds, nextItemId)).toBe(true)
      expect(isConversationItemAddition(before.items, after.items, itemEvent.item)).toBe(true)
      expect(isConversationTurnIdAppend(before.turnIds, persistentConversationTurnIds([...after.turnIds]), nextTurnId)).toBe(false)
      ...
      expect(diagnostics.conversationTurnIdSequenceAppends).toBe(1)
      expect(diagnostics.conversationTurnIdSequenceNodeVisits).toBeLessThanOrEqual(2 * logarithmicBound)
      expect(diagnostics.conversationTurnIdSequenceNodesCopied).toBeLessThanOrEqual(6 * logarithmicBound)
      expect(diagnostics.conversationTurnItemIdSequenceNormalizations).toBe(0)
      expect(diagnostics.conversationTurnItemIdSequenceNormalizationVisits).toBe(0)
      expect(diagnostics.conversationTurnItemIdSequenceAppends).toBe(1)
      ...
      expect(diagnostics.conversationTurnRecordLookupNodeVisits).toBeLessThanOrEqual(4 * logarithmicBound)
      expect(diagnostics.conversationTurnRecordUpdates).toBe(2)
      expect(diagnostics.conversationTurnRecordNodeVisits).toBeLessThanOrEqual(4 * logarithmicBound)
      expect(diagnostics.conversationTurnRecordNodesCopied).toBeLessThanOrEqual(12 * logarithmicBound)
  ...
  test("normalizes fork structure without aliasing semantic values", () => {
    ...
    const completed = reduceConversationWithDiagnostics(source, {
      type: "item.completed",
      threadId: source.threadId,
      item: { id: itemId("fork-item"), turnId: boundary, kind: "assistant", markdown: "done", status: "complete" },
    }, reducerDiagnostics())
    ...
    expect(fork.turns[boundary]?.itemIds).not.toBe(completed.turns[boundary]?.itemIds)
    expect(fork.items[itemId("fork-item")]).not.toBe(completed.items[itemId("fork-item")])
    expect(Reflect.set(fork.turnIds, "0", turnId("changed"))).toBe(false)
    expect(Reflect.set(fork.turns, "changed", fork.turns[boundary]!)).toBe(false)
  })
})
