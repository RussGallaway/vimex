...
const sequenceHeight = <T extends string>(node: SequenceNode<T> | undefined) =>
  node?.height ?? 0
const sequenceSize = <T extends string>(node: SequenceNode<T> | undefined) =>
  node?.size ?? 0

function sequenceNode<T extends string>(
  value: T,
  left?: SequenceNode<T>,
  right?: SequenceNode<T>,
): SequenceNode<T> {
...
function rotateSequenceLeft<T extends string>(
  root: SequenceNode<T>,
  kind: SequenceKind,
  diagnostics?: StructureDiagnostics,
): SequenceNode<T> {
  const right = root.right!
  return copiedSequenceNode(
    right.value,
    copiedSequenceNode(root.value, root.left, right.left, kind, diagnostics),
    right.right,
    kind,
    diagnostics,
  )
...
function rotateSequenceRight<T extends string>(
  root: SequenceNode<T>,
  kind: SequenceKind,
  diagnostics?: StructureDiagnostics,
): SequenceNode<T> {
  ...
  return copiedSequenceNode(
    left.value,
    left.left,
    copiedSequenceNode(root.value, left.right, root.right, kind, diagnostics),
    kind,
    diagnostics,
  )
}

function balanceSequence<T extends string>(
  root: SequenceNode<T>,
  kind: SequenceKind,
  diagnostics?: StructureDiagnostics,
): SequenceNode<T> {
  ...
  if (delta > 1) {
    const left = root.left!
    return rotateSequenceRight(
      sequenceHeight(left.left) < sequenceHeight(left.right)
        ? copiedSequenceNode(
            root.value,
            rotateSequenceLeft(left, kind, diagnostics),
            root.right,
            kind,
            diagnostics,
          )
  ...
  if (delta < -1) {
    const right = root.right!
    return rotateSequenceLeft(
      sequenceHeight(right.right) < sequenceHeight(right.left)
        ? copiedSequenceNode(
            root.value,
            root.left,
            rotateSequenceRight(right, kind, diagnostics),
            kind,
            diagnostics,
          )
        : root,
...
function appendSequenceNode<T extends string>(
  ...
  if (!root)
    return copiedSequenceNode(value, undefined, undefined, kind, diagnostics)
  const right = appendSequenceNode(root.right, value, kind, diagnostics)
  return balanceSequence(
    copiedSequenceNode(root.value, root.left, right, kind, diagnostics),
    kind,
    diagnostics,
  )
}

function sequenceValue<T extends string>(
  root: SequenceNode<T> | undefined,
  index: number,
): T | undefined {
  while (root) {
...
function membershipNode(
  ...
  index: number,
  left?: MembershipNode,
  right?: MembershipNode,
): MembershipNode {
  return Object.freeze({
    key,
    index,
    height: Math.max(membershipHeight(left), membershipHeight(right)) + 1,
    ...(left ? { left } : {}),
    ...(right ? { right } : {}),
  })
}
...
function rotateMembershipLeft(
  root: MembershipNode,
  kind: SequenceKind,
  diagnostics?: StructureDiagnostics,
): MembershipNode {
  const right = root.right!
  return copiedMembershipNode(
    right.key,
    right.index,
    copiedMembershipNode(
      root.key,
      root.index,
      root.left,
      right.left,
...
function rotateMembershipRight(
  root: MembershipNode,
  kind: SequenceKind,
  diagnostics?: StructureDiagnostics,
): MembershipNode {
...
function balanceMembership(
  ...
  if (delta > 1) {
    const left = root.left!
    return rotateMembershipRight(
      membershipHeight(left.left) < membershipHeight(left.right)
        ? copiedMembershipNode(
            root.key,
            root.index,
            rotateMembershipLeft(left, kind, diagnostics),
            root.right,
            kind,
            diagnostics,
          )
  ...
  if (delta < -1) {
    const right = root.right!
    return rotateMembershipLeft(
      membershipHeight(right.right) < membershipHeight(right.left)
        ? copiedMembershipNode(
            root.key,
            root.index,
            root.left,
            rotateMembershipRight(right, kind, diagnostics),
            kind,
            diagnostics,
          )
        : root,
...
function membershipIndex(
  ...
  while (root) {
    bump(
      diagnostics,
      kind === "turn"
        ? "conversationTurnIdSequenceLookupNodeVisits"
        : "conversationTurnItemIdSequenceLookupNodeVisits",
    )
...
function persistentSequence<T extends string>(
  data: SequenceData<T>,
): readonly T[] {
  const target: T[] = []
  ...
  const proxy = new Proxy(target, {
    ...
    has: (_target, property) =>
      ...
      arrayIndex(property, data.size) !== undefined
        ? true
        : Reflect.has(target, property),
    ownKeys: () => [
      ...Array.from({ length: data.size }, (_, index) => String(index)),
      "length",
    ],
    getOwnPropertyDescriptor: (_target, property) => {
      ...
      return index === undefined
        ...
        : {
            ...
            enumerable: true,
            writable: false,
            value: sequenceValue(data.root, index),
          }
    },
    set: () => false,
...
function normalizeSequence<T extends string>(
  ...
  const buildSequence = (
    ...
    return sequenceNode(
      value[middle]!,
      buildSequence(from, middle),
      buildSequence(middle + 1, to),
    )
  }
  const sorted = value
  ...
  const buildMembership = (
    ...
    return membershipNode(
      key,
      index,
      buildMembership(from, middle),
      buildMembership(middle + 1, to),
    )
  }
  const result = persistentSequence({
    token: Object.freeze({}),
    root: buildSequence(0, value.length),
    membership: buildMembership(0, sorted.length),
    size: value.length,
  })
...
function appendSequence<T extends string>(
  value: readonly T[],
  next: T,
  kind: SequenceKind,
  diagnostics?: StructureDiagnostics,
  ...
  const normalized = normalizeSequence(
    ...
    (kind === "turn"
      ? normalizedTurnIds
      : normalizedItemIds) as unknown as WeakMap<object, readonly T[]>,
    diagnostics,
  )
  ...
  return persistentSequence({
    ...
    lineage: {
      ...
      previousMembership: data.membership,
...
export function persistentConversationTurnIds(
...
export function appendConversationTurnId(
  value: readonly TurnId[],
  next: TurnId,
  diagnostics?: StructureDiagnostics,
): readonly TurnId[] {
...
export function conversationTurnIdsHave(
  value: readonly TurnId[],
  turnId: TurnId,
  diagnostics?: StructureDiagnostics,
): boolean {
  ...
  return data
    ? membershipIndex(data.membership, turnId, "turn", diagnostics) !==
        undefined
    : value.includes(turnId)
}

export function persistentTurnItemIds(
...
export function appendTurnItemId(
  value: readonly ItemId[],
  next: ItemId,
  diagnostics?: StructureDiagnostics,
): readonly ItemId[] {
...
export function turnItemIdsHave(
  value: readonly ItemId[],
  itemId: ItemId,
  diagnostics?: StructureDiagnostics,
): boolean {
  ...
  const data = sequenceData.get(value)
  return data
    ? membershipIndex(data.membership, itemId, "item", diagnostics) !==
        undefined
    : value.includes(itemId)
}

export function isConversationTurnIdAppend(
  previous: readonly TurnId[],
  next: readonly TurnId[],
  appended: TurnId,
): boolean {
  const data = sequenceData.get(next) as SequenceData<TurnId> | undefined
  const previousData = sequenceData.get(previous) as
    SequenceData<TurnId> | undefined
  return (
    ...
    data.lineage.appended === appended &&
    data.size === previousData.size + 1
  )
}

export function isTurnItemIdAppend(
  previous: readonly ItemId[] | undefined,
  ...
  const previousData =
    previous === undefined
      ? undefined
      : (sequenceData.get(previous) as SequenceData<ItemId> | undefined)
  const matchesPrevious =
    previous === undefined
      ? lineage.previousSize === 0
      : previousData !== undefined &&
        lineage.previousRoot === previousData.root &&
        lineage.previousMembership === previousData.membership &&
        lineage.previousSize === previousData.size &&
        lineage.previousToken === previousData.token
  return (
    matchesPrevious &&
    lineage.appended === appended &&
    data.size === lineage.previousSize + 1
  )
}
