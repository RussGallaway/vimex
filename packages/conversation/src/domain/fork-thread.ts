...
export function forkConversation(
  ...
  if (boundary < 0 || state.turns[throughTurnId]?.status === "running")
    return undefined
  const turnIds = state.turnIds.slice(0, boundary + 1)
  const turns: Record<string, Turn> = {}
  const items = Object.create(null) as Record<string, ConversationItem>
  for (const id of turnIds) {
    const turn = state.turns[id]
    if (!turn) continue
    turns[id] = { ...turn, itemIds: persistentTurnItemIds([...turn.itemIds]) }
    for (const item of turn.itemIds) {
      const value = state.items[item]
      if (value) items[item] = { ...value }
    }
  }
  return {
