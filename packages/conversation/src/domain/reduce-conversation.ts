...
import { conversationTurnAt, persistentConversationTurns, setConversationTurn } from "./conversation-turns"
import type { ItemId, TurnId } from "./identifiers"
import type { Turn } from "./turn"

export type ConversationReductionDiagnostics = ConversationItemRecordDiagnostics & Partial<ConversationStructureDiagnostics>

export function createConversationReductionDiagnostics(): ConversationItemRecordDiagnostics & ConversationStructureDiagnostics {
  return {
  ...
  switch (event.type) {
    case "turn.started": {
      ...
      if (exists && exists.status !== "running") {
        ...
        return { ...state, turns: storage.writeTurn(state.turns, { ...exists, startedAt: event.startedAt }, diagnostics) }
      }
      ...
      const turn = exists ? { ...exists, startedAt: exists.startedAt ?? event.startedAt }
        : { id: event.turnId, status: "running" as const, itemIds: storage.emptyItemIds(), startedAt: event.startedAt }
      return { ...state, activeTurnId: event.turnId,
        turnIds: exists ? state.turnIds : storage.appendTurnId(state.turnIds, event.turnId, diagnostics),
    ...
    case "turn.completed": {
      ...
      if (current && current.status !== "running") {
        ...
        return { ...state, activeTurnId: state.activeTurnId === event.turnId ? undefined : state.activeTurnId,
          turns: changed ? storage.writeTurn(state.turns, turn, diagnostics) : state.turns }
      }
      const turn = current ?? { id: event.turnId, status: event.outcome, itemIds: storage.emptyItemIds() }
      return { ...state, activeTurnId: state.activeTurnId === event.turnId ? undefined : state.activeTurnId,
        turnIds: current ? state.turnIds : storage.appendTurnId(state.turnIds, event.turnId, diagnostics),
        turns: storage.writeTurn(state.turns, { ...turn, status: event.outcome, startedAt: turn.startedAt ?? event.startedAt,
          completedAt: event.completedAt ?? turn.completedAt, durationMs: event.durationMs ?? turn.durationMs }, diagnostics) }
    }
    case "item.started": {
      const existingTurn = storage.turnAt(state.turns, event.item.turnId, diagnostics)
      const turn = existingTurn ?? { id: event.item.turnId, status: "running" as const, itemIds: storage.emptyItemIds() }
      ...
      return { ...state,
        turnIds: existingTurn ? state.turnIds : storage.appendTurnId(state.turnIds, event.item.turnId, diagnostics),
        turns: storage.writeTurn(state.turns,
          { ...turn, itemIds: linked ? turn.itemIds : storage.appendItemId(turn.itemIds, event.item.id, diagnostics) }, diagnostics),
        items: storage.writeItem(state.items, existing && existing.status !== "running" ? existing : event.item, diagnostics) }
    }
    case "item.delta": {
      const current = storage.itemAt(state.items, event.itemId, diagnostics)
      return !current || current.status !== "running" ? state
        : { ...state, items: storage.writeItem(state.items, appendDelta(current, event.delta), diagnostics) }
    }
    case "item.completed": {
      const existing = storage.itemAt(state.items, event.item.id, diagnostics)
      const existingTurn = storage.turnAt(state.turns, event.item.turnId, diagnostics)
...
interface ConversationStorage {
  turnAt(value: Readonly<Record<string, Turn>>, turnId: TurnId, diagnostics?: ConversationReductionDiagnostics): Turn | undefined
  writeTurn(value: Readonly<Record<string, Turn>>, turn: Turn, diagnostics?: ConversationReductionDiagnostics): Readonly<Record<string, Turn>>
  appendTurnId(value: readonly TurnId[], turnId: TurnId, diagnostics?: ConversationReductionDiagnostics): readonly TurnId[]
  emptyItemIds(): readonly ItemId[]
  hasItemId(value: readonly ItemId[], itemId: ItemId, diagnostics?: ConversationReductionDiagnostics): boolean
  appendItemId(value: readonly ItemId[], itemId: ItemId, diagnostics?: ConversationReductionDiagnostics): readonly ItemId[]
  itemAt(value: Readonly<Record<string, ConversationItem>>, itemId: ItemId,
    diagnostics?: ConversationReductionDiagnostics): ConversationItem | undefined
  writeItem(value: Readonly<Record<string, ConversationItem>>, item: ConversationItem,
    diagnostics?: ConversationReductionDiagnostics): Readonly<Record<string, ConversationItem>>
}
...
function denseSetConversationTurn(value: Readonly<Record<string, Turn>>, turn: Turn): Readonly<Record<string, Turn>> {
