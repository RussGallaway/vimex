...
const normalizedItemRecords = new WeakMap<
  object,
  ...
  const proxy = new Proxy(target, {
    get: (_target, property) =>
      typeof property === "string"
        ? itemValue(data.root, property)
        : Reflect.get(target, property),
    has: (_target, property) =>
      typeof property === "string"
        ? itemValue(data.root, property) !== undefined
        : false,
    ownKeys: () => {
      const values: ItemEntry[] = []
    ...
    getOwnPropertyDescriptor: (_target, property) =>
      ...
      itemValue(data.root, property) !== undefined
        ? {
            ...
            enumerable: true,
            writable: false,
            value: itemValue(data.root, property),
          }
        : undefined,
    set: () => false,
...
export function persistentConversationItems(
  ...
  if (diagnostics) {
    ...
    diagnostics.conversationItemRecordNormalizationItemVisits +=
      sourceEntries.length
  }
  const values = sourceEntries
    .map(([key, item], ordinal) => ({ key, item, ordinal }))
    .sort((left, right) =>
...
export function isConversationItemUpdate(
  ...
  return (
    ...
    data.lineage.previousValue === previousItem &&
    data.lineage.updatedValue === nextItem &&
