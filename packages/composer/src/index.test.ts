...
import { failOutgoing, firstQueuedMessage, initialComposer, markOutgoingSending, retryOutgoing, submitDraft, updateDraft } from "./index"
...
describe("composer", () => {
  test("uses grapheme cursor offsets and queues next-turn messages", () => {
    ...
    expect(firstQueuedMessage(state)).toMatchObject({ id: "m1", text: "a👨‍👩‍👧‍👦b", status: "queued" })
    expect(markOutgoingSending(state, "m1").outbox[0]?.status).toBe("sending")
  })

  test("steer sends immediately even while a turn is active", () => {
    const state = submitDraft(updateDraft(initialComposer(), "redirect"), "steer", false, "m2")
    expect(state.outbox[0]?.status).toBe("sending")
  })

  test("rejects duplicate client message ids", () => {
    let state = submitDraft(updateDraft(initialComposer(), "first"), "next-turn", false, "same")
  ...
  test("filters queued intents and exposes failed messages for retry", () => {
    let state = submitDraft(updateDraft(initialComposer(), "redirect"), "steer", true, "steer")
    ...
    expect(state.outbox[0]).toMatchObject({ status: "failed", reason: "offline", text: "redirect" })
    expect(retryOutgoing(state, "steer", false).outbox[0]).toMatchObject({ status: "sending", reason: undefined })
  })
})
