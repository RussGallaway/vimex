...
test("validation preserves literal arguments but rejects invalid enumerated arguments", () => {
  const valid = [":cwd /tmp/path with  spaces", ":new /tmp/another project", ":sessions exact-thread-id", ":rename A title: with / punctuation", ":open https://example.test/a?b=c#d", ":help model", ":help q", ":favorite", ":favorite on", ":favorite off", ":follow", ":submit", ":submit queue", ":submit steer", ":syntax theme", ":yank markdown"]
  for (const line of valid) {
    const parsed = parseCommand(line)
    if (parsed.kind !== "command") throw new Error(line)
    expect(validateCommand(parsed)).toBeUndefined()
  }
  for (const line of [":rename", ":submit surprise", ":yank raw", ":favorite yes", ":theme nord extra", ":thinking high extra", ":model model high extra", ":help nonexistent"]) {
    const parsed = parseCommand(line)
    if (parsed.kind !== "command") throw new Error(line)
...
test("dynamic model and reasoning validation uses the supplied catalog only", () => {
  const options = { models: ["fast", "deep"], modelEfforts: { fast: ["low"], deep: ["high"] }, currentModel: "fast" }
  for (const [line, valid] of [[":model deep high", true], [":model fast high", false], [":model missing", false], [":thinking low", true], [":thinking high", false]] as const) {
    const parsed = parseCommand(line)
    if (parsed.kind !== "command") throw new Error(line)
