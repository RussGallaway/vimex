...
test("history recalls commands and restores unfinished input on moving forward", () => {
  let state = recordCommand(
    recordCommand(initialCommandHistory(), ":model test"),
    "sessions",
  )
  ...
  expect(recordCommand(state, "sessions").entries).toEqual([
    "model test",
    "sessions",
  ])
})
test("completion uses the shared command vocabulary and argument choices", () => {
  ...
  expect(
    commandCompletions(":model sol-5.6 ", {
      models: ["sol-5.6"],
      modelEfforts: { "sol-5.6": ["low", "medium", "high"] },
    }),
  ).toEqual([
    ":model sol-5.6 low",
  ...
  expect(
    commandCompletions(":model sol-5.6 m", {
      models: ["sol-5.6"],
      modelEfforts: { "sol-5.6": ["low", "medium", "high"] },
    }),
...
test("completes thinking from the active model and shared static choices", () => {
  const options = {
    models: ["fast", "deep"],
    modelEfforts: { fast: ["low", "medium"], deep: ["high"] },
    currentModel: "fast",
  }
  expect(commandCompletions(":thinking ", options)).toEqual([
    ":thinking low",
    ":thinking medium",
  ])
  expect(commandCompletions(":thinking h", options)).toEqual([])
  expect(
    commandCompletions(":thinking ", { ...options, currentModel: "deep" }),
  ).toEqual([":thinking high"])
  expect(commandCompletions(":syntax t")).toEqual([
    ":syntax theme",
    ":syntax tokyo-night",
  ])
  expect(commandCompletions(":submit ")).toEqual([
    ":submit queue",
    ":submit steer",
  ])
  expect(commandCompletions(":favorite o")).toEqual([
    ":favorite on",
    ":favorite off",
  ])
  ...
  expect(commandCompletions(":models fast m", options)).toEqual([
    ":models fast medium",
  ])
  expect(commandCompletions(":model\tfast\tm", options)).toEqual([
    ":model fast medium",
  ])
})
...
test("session completion suggests known exact IDs without constraining literal resume", () => {
  expect(
    commandCompletions(":sessions abc", {
      sessionIds: ["abc-123", "abc-456", "other-789"],
    }),
  ).toEqual([":sessions abc-123", ":sessions abc-456"])
  expect(
    commandCompletions(":sessions unknown", { sessionIds: ["abc-123"] }),
  ).toEqual([])
})
...
test("side completion offers lifecycle actions and focus targets without replacing questions", () => {
  ...
  expect(commandCompletions(":side ")).toEqual([
    ":side close",
    ":side quit",
    ":side refresh",
    ":side quote",
  ...
  expect(commandCompletions(":side focus ")).toEqual([
    ":side focus parent",
    ":side focus side",
  ])
