...
export function parseGoalCommand(argument: string): GoalCommand {
  ...
  if (text === "pause" || text === "resume" || text === "complete")
    return {
      kind: "status",
      status:
        text === "pause" ? "paused" : text === "resume" ? "active" : "complete",
    }
  ...
  if (objective.startsWith("--budget")) {
    ...
    if (!Number.isSafeInteger(tokenBudget) || tokenBudget <= 0)
      return {
        kind: "invalid",
        message: "Usage: :goal --budget <positive token count> <objective>",
      }
    objective = match![2]!.trim()
  }
  if (text === "set" || !objective || [...objective].length > 4000)
    return {
      kind: "invalid",
      message: "Goal objective must contain 1–4000 characters",
    }
  return {
    kind: "set",
    objective,
