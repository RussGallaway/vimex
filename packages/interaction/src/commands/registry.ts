...
export const commandDescriptions: Readonly<Record<CommandName, string>> = {
  ...
  fold: "Collapse transcript details", unfold: "Expand transcript details", yank: "Copy selection or current transcript block", open: "Open a URL",
...
export const commandDescriptors = Object.fromEntries(commandNames.map(name => [name, {
  description: commandDescriptions[name], usage: name, arguments: "none", ...argumentDescriptors[name],
...
export function commandArgumentChoices(name: CommandName, options: CommandCompletionOptions = {}): readonly string[] | undefined {
  if (name === "sessions") return options.sessionIds
  if (name === "model") return options.models
  if (name === "thinking") return options.currentModel ? options.modelEfforts?.[options.currentModel] : undefined
