...
describe("interaction", () => {
  test("implements only Vim modes and clears count/pending state on transitions", () => {
    let state = reduceInteraction(initialInteraction(), {
      type: "count.push",
      digit: 0,
    })
    ...
    expect(reduceInteraction(state, { type: "mode.insert" })).toMatchObject({
      mode: "insert",
      surface: "composer",
      count: "",
      pendingKeys: "",
  ...
  test("moving from insert to transcript returns to normal", () => {
    const insert = reduceInteraction(initialInteraction(), {
      type: "mode.insert",
    })
    expect(
      reduceInteraction(insert, { type: "focus.set", surface: "transcript" }),
    ).toMatchObject({ mode: "normal", surface: "transcript" })
  })
  ...
  test("escape enters normal mode on the composer surface", () => {
    ...
    expect(reduceInteraction(insert, { type: "mode.normal" })).toMatchObject({
      mode: "normal",
      surface: "composer",
    })
  })
