...
const composerNormal = () =>
  reduceInteraction(initialInteraction(), {
    type: "focus.set",
    surface: "composer",
  })
...
describe("composer Vim grammar", () => {
  test("owns count accumulation and consumption for motions", () => {
    ...
    expect(resolveComposerKey(composerNormal(), "e").action).toEqual({
      type: "motion",
      motion: "word-end",
      count: 1,
      select: false,
    })
    expect(resolveComposerKey(composerNormal(), "^").action).toEqual({
      type: "motion",
      motion: "first-content",
      count: 1,
      select: false,
    })
  })
  ...
  test("distinguishes zero motion from a zero in a count", () => {
    expect(resolveComposerKey(composerNormal(), "0").action).toEqual({
      type: "motion",
      motion: "line-start",
      count: 1,
      select: false,
    })
    const ten = resolveComposerKey(
      resolveComposerKey(composerNormal(), "1").state,
      "0",
    )
  ...
  test("owns g and d prefixes, including counts", () => {
    ...
    expect(resolveComposerKey(pendingD.state, "d").action).toEqual({
      type: "edit",
      operator: "delete-line",
      count: 1,
    })
    expect(resolveComposerKey(composerNormal(), "gg").action).toEqual({
      type: "motion",
      motion: "document-start",
      count: 1,
      select: false,
    })
    expect(resolveComposerKey(composerNormal(), "shift+g").action).toEqual({
      type: "motion",
      motion: "document-end",
      count: 0,
      select: false,
  ...
  test("resolves deletes, changes, paste, and insert/open placements", () => {
    const counted = resolveComposerKey(
      resolveComposerKey(composerNormal(), "3").state,
      "dd",
    )
    expect(counted.action).toEqual({
      type: "edit",
      operator: "delete-line",
      count: 3,
    })
    expect(resolveComposerKey(composerNormal(), "shift+d").action).toEqual({
      type: "edit",
    ...
    expect(resolveComposerKey(composerNormal(), "p").action).toEqual({
      type: "paste",
      placement: "after",
      count: 1,
    })
    expect(resolveComposerKey(composerNormal(), "shift+p").action).toEqual({
      type: "paste",
      placement: "before",
      count: 1,
    })

    for (const [key, placement] of [
      ["i", "before"],
      ["a", "after"],
      ["shift+i", "line-start"],
      ["shift+a", "line-end"],
    ...
    expect(resolveComposerKey(composerNormal(), "o").action).toEqual({
      type: "open-line",
      placement: "below",
      count: 1,
    })
    expect(resolveComposerKey(composerNormal(), "shift+o").action).toEqual({
      type: "open-line",
      placement: "above",
      count: 1,
    })
  ...
  test("owns visual selection motion, yank, and escape transitions", () => {
    ...
    const moved = resolveComposerKey(
      resolveComposerKey(visual.state, "2").state,
      "e",
    )
  ...
  test("submits from Normal Enter and retains retry as a semantic action", () => {
    expect(resolveComposerKey(composerNormal(), "return").action).toEqual({
      type: "submit",
    })
    expect(resolveComposerKey(composerNormal(), "enter").action).toEqual({
      type: "submit",
    })
    expect(resolveComposerKey(composerNormal(), "shift+r").action).toEqual({
      type: "retry",
    })
  })
})
