...
export type ComposerMotion =
  ...
  | "document-start"
  | "document-end"
export type ComposerOperator =
  "delete-char" | "delete-line" | "delete-to-line-end" | "undo" | "redo"
...
function countCommand(
  state: InteractionState,
  key: string,
): ComposerKeyResolution | undefined {
  if (/^[1-9]$/.test(key))
    return resolved(state, [{ type: "count.push", digit: Number(key) }])
  if (key === "0" && state.count)
    return resolved(state, [{ type: "count.push", digit: 0 }])
  return undefined
}
...
function prefixed(
  ...
  if (state.pendingKeys === "g") {
    if (key === "g") {
      return resolved(
        state,
        [{ type: "keys.clear" }, { type: "count.clear" }],
        {
          type: "motion",
...
function resolveMotion(
  ...
  const motion =
    key === "0" ? "line-start" : key === "gg" ? "document-start" : motions[key]
  if (!motion) return undefined
  return resolved(state, [{ type: "count.clear" }, { type: "keys.clear" }], {
    type: "motion",
    motion,
    count,
    select,
...
export function resolveComposerKey(
  ...
  if (state.mode === "normal") {
    ...
    if (operator)
      return resolved(state, [{ type: "count.clear" }], {
        type: "edit",
        operator,
        count,
      })
    if (key === "shift+c") {
      return resolved(
        state,
        [{ type: "count.clear" }, { type: "mode.insert" }],
        {
          type: "edit",
    ...
    if (key === "v")
      return resolved(
        state,
        [{ type: "count.clear" }, { type: "mode.visual" }],
        { type: "begin-visual" },
      )
    if (key === "return" || key === "enter")
      return resolved(state, [{ type: "count.clear" }], { type: "submit" })
    if (key === "shift+r")
      return resolved(state, [{ type: "count.clear" }], { type: "retry" })
  }
    ...
    if (key === "d" || key === "x" || key === "c")
      return resolved(
        ...
        [
          { type: "count.clear" },
          { type: "keys.clear" },
          { type: key === "c" ? "mode.insert" : "mode.normal" },
        ],
        { type: "delete-selection", enterInsert: key === "c" },
      )
    if (key === "y")
      return resolved(
        state,
        [{ type: "count.clear" }, { type: "mode.normal" }],
        { type: "yank" },
      )
    if (key === "escape")
      return resolved(
        state,
        [{ type: "mode.normal" }, { type: "focus.set", surface: "transcript" }],
        { type: "clear-selection" },
      )
  }
