...
5. Stable publication generates Formula/vimex.rb from the release manifest, opens a formula-only pull request, dispatches the required checks for its commit, and enables squash auto-merge. Formula-only PRs skip the duplicate `pull_request` event because GitHub holds bot-authored contributor workflows for maintainer approval; the explicit dispatch supplies the same protected checks without a manual gate. Prereleases leave the stable formula unchanged.
