...
5. Stable publication generates Formula/vimex.rb from the release manifest, opens a formula-only pull request, dispatches the required checks for its commit, and enables squash auto-merge. The protected checks merge it without manual intervention. Prereleases leave the stable formula unchanged.

Rerunning a failed publication reuses a draft. Published assets are never replaced; a retry downloads the already published manifest before proposing the formula update. If the automated formula proposal or merge fails, download that manifest, run `bun run release:formula PATH`, and submit the generated change through the repository's required review process. Do not move the release tag. Formula updates are skipped when the tag is no longer GitHub's latest stable release.
