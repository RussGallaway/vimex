...
Up/Ctrl-K and Down/Ctrl-J move between the composer and transcript through queued messages. Ctrl-W K/J jump directly to the transcript or composer. These bindings respect local menu and completion navigation.

Queued messages appear above the composer with a preview of the latest message and a total count. Focus the queue to see a short list and the selected message. Use Ctrl-K/J to move between messages, Ctrl-U/D to scroll the selected message, e to return it to an empty composer draft for editing, or x to remove it after confirmation. Once sending starts, the message can no longer be taken back.
...
:performance export (or :perf export) saves a local performance trace from recent Vimex interactions and shows its path. It records timings and workload counts for submission and navigation, without conversation text or tool output. Export does not send the trace anywhere. Version 2 traces group navigation inputs into bursts when adjacent inputs are at most 120 ms apart, and report the first frame after a burst's first and last observed state publication. Burst counts describe retained marks; each burst indicates whether older marks were evicted or more results may arrive after export. The audit section counts paired events, while buffer counters show records evicted by age or capacity.

Submit timing starts when Workbench dispatches the action, after UI key processing. request_sent means the app-server request was successfully written to stdin. The next thread activity or content is a best-effort observation and may not belong to that submission. Navigation burst association remains a best-effort observation for actions within 250 ms of the last input. frame_callback_start is observed before OpenTUI draws the frame; next_renderer_frame follows its native render call. If navigation publishes state during an already-running frame, frame_already_running_at_publication records that case so it is not counted as time waiting for the callback. Renderer frame IDs pair these marks. They do not measure when the terminal physically displays a frame or prove content is visible. A callback start can lack a matching frame if rendering is skipped.
