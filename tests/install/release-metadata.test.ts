...
async function execute(script: string, ...args: string[]) {
  const process = Bun.spawn([Bun.which("bun")!, join(repo, "scripts/release", script), ...args], { cwd: tmpdir(), stdout: "pipe", stderr: "pipe" })
...
async function fixture() {
  ...
  const artifacts = []
  for (const platform of ["darwin", "linux"] as const) for (const arch of ["arm64", "x64"] as const) {
    const name = `vimex-v${version}-${platform}-${arch}.tar.gz`
    const bytes = Buffer.from(`fixture ${platform}/${arch}`)
    const artifact = { platform, arch, name, sha256: createHash("sha256").update(bytes).digest("hex") }
    await writeFile(join(directory, name), bytes)
    await writeFile(join(directory, name.replace(/\.tar\.gz$/, ".json")), JSON.stringify({ version, artifacts: [artifact] }))
    artifacts.push(artifact)
  }
  return { directory, artifacts, close: () => rm(directory, { recursive: true, force: true }) }
}
test("release generator verifies four archives and emits matching manifest, sums and Homebrew formula", async () => {
  ...
  try {
    ...
    for (const artifact of f.artifacts) expect(checksums).toContain(`${artifact.sha256}  ${artifact.name}\n`)
    const formula = join(f.directory, "vimex.rb")
    expect((await execute("update-homebrew.ts", manifest, formula)).code).toBe(0)
...
test("release generator rejects tampered bytes and missing platforms before emitting manifest", async () => {
  for (const failure of ["checksum", "missing"] as const) {
    ...
    try {
      ...
      if (failure === "checksum") await writeFile(join(f.directory, artifact.name), "tampered")
      else await rm(join(f.directory, artifact.name.replace(/\.tar\.gz$/, ".json")))
      const result = await execute("manifest.ts", f.directory)
      expect(result.code).not.toBe(0)
      expect(result.stderr).toContain(failure === "checksum" ? "Checksum mismatch" : "all four")
      expect(await Bun.file(join(f.directory, "vimex-release.json")).exists()).toBe(false)
    } finally { await f.close() }
  }
})
test("formula generator refuses malformed checksums, missing platforms, and mismatched filenames", async () => {
  const f = await fixture()
  try {
    for (const artifacts of [f.artifacts.slice(1), [{ ...f.artifacts[0], sha256: "invented" }, ...f.artifacts.slice(1)], [{ ...f.artifacts[0], name: "another-release.tar.gz" }, ...f.artifacts.slice(1)]]) {
      const manifest = join(f.directory, "invalid.json"), output = join(f.directory, "invalid.rb")
      await writeFile(manifest, JSON.stringify({ version, artifacts }))
      expect((await execute("update-homebrew.ts", manifest, output)).code).not.toBe(0)
