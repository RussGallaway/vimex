...
beforeAll(async () => {
  ...
  await run(
    ["tar", "-czf", archive, "vimex", "assets", "share", "LICENSE"],
    payload,
  )
...
async function install(name: string, overrides: Record<string, string> = {}) {
  const root = join(temporary, name, "root"),
    bin = join(temporary, name, "bin")
  const child = Bun.spawn(["sh", join(repo, "install.sh")], {
    stdout: "pipe",
    stderr: "pipe",
    env: {
      ...process.env,
      PATH: `${fakeBin}:${process.env.PATH}`,
      VIMEX_INSTALL_ROOT: root,
      VIMEX_BIN_DIR: bin,
      VIMEX_VERSION: "0.1.0",
...
test("bootstrap installs verified native bundle into isolated prefix and safely upgrades current", async () => {
  ...
  expect(await readlink(join(first.bin, "vimex"))).toBe(
    join(first.root, "current/vimex"),
  )
  expect(
    JSON.parse(await readFile(join(first.root, ".vimex-install.json"), "utf8")),
  ).toEqual({ schemaVersion: 1, kind: "direct", version: "0.1.0" })
  expect((await run([join(first.bin, "vimex"), "--version"])).trim()).toBe(
    "vimex 0.1.0",
  )
...
test("installer refuses unrelated binaries and archives containing links", async () => {
  ...
  expect(await readFile(join(bin, "vimex"), "utf8")).toBe(
  ...
  await writeFile(
    evilChecksums,
    `${createHash("sha256")
      .update(await readFile(evilArchive))
      .digest("hex")}  ${artifact}\n`,
  )
  const result = await install("unsafe", {
    TEST_ARCHIVE: evilArchive,
    TEST_CHECKSUMS: evilChecksums,
  })
  expect(result.code).not.toBe(0)
...
test("installer rejects traversal entries before creating an install prefix", async () => {
  ...
  await writeFile(
    unsafeChecksums,
    `${createHash("sha256")
      .update(await readFile(unsafeArchive))
      .digest("hex")}  ${artifact}\n`,
  )
  const result = await install("traversal", {
    TEST_ARCHIVE: unsafeArchive,
    TEST_CHECKSUMS: unsafeChecksums,
  })
  expect(result.code).not.toBe(0)
...
test("installer enforces managed version containment, receipt schema, and shared upgrade lock", async () => {
  ...
  expect((await install("linked-versions")).err).toContain(
    "versions directory must not be a symlink",
  )
  const traversal = join(temporary, "current-traversal/root")
  await mkdir(traversal, { recursive: true })
  await symlink(
    "versions/../../outside-managed-root",
    join(traversal, "current"),
  )
  expect((await install("current-traversal")).err).toContain(
    "Current symlink does not point to a managed version",
  )
  const owned = await install("ownership")
  expect(owned.code).toBe(0)
  const current = await readlink(join(owned.root, "current"))
  await writeFile(
    join(owned.root, ".vimex-install.json"),
    '{"schemaVersion":2,"kind":"direct","version":"0.1.0"}',
  )
  expect((await install("ownership")).err).toContain(
    "Invalid direct-install receipt",
  )
  await writeFile(
    join(owned.root, ".vimex-install.json"),
    '{"schemaVersion":1,"kind":"direct","version":"0.1.0"}',
  )
  await mkdir(join(owned.root, ".upgrade-lock"))
  expect((await install("ownership")).err).toContain(
    "Another upgrade is running",
  )
