...
def wait_for(value: bytes, start: int = 0, seconds: float = 12) -> None:
    ...
    while time.monotonic() < deadline:
        ...
        if not pump() and process.poll() is not None:
            break
    tail = bytes(output[-2400:])
    raise AssertionError(f"Missing terminal output {value!r}; tail={tail!r}")


def send(value: bytes, pause: float = 0.03) -> int:
    offset = len(output)
...
def command(value: str, seconds: float = 12) -> int:
    ...
    end = time.monotonic() + min(seconds, 0.5)
...
def switch_session(thread: str, expected: bytes) -> None:
    offset = send(b"s")
    wait_for(b"Sessions", offset)
    send(thread.encode() + b"\r")
    wait_for(expected, offset, 30)
...
try:
    ...
    send(b"\x17j")
    send(b"i")
    wait_for(b"INSERT", plain_start)
    send(b"DRAFT_SOURCE_THREAD")
    send(b"\x1b")
    switch_session(FORK_THREAD, b"VIMEX_LIVE_OK")
    send(b"\x17j")
    send(b"i")
    send(b"DRAFT_FORK_THREAD")
    send(b"\x1b")
    switch_session(SOURCE_THREAD, b"DRAFT_SOURCE_THREAD")
    switch_session(FORK_THREAD, b"DRAFT_FORK_THREAD")
    ...
    search("pwd")
    folded = send(b"zc")
    wait_for(b"[closed]", folded)
    unfolded = send(b"zo")
    wait_for(b"[open]", unfolded)
    ...
    while time.monotonic() < deadline and not url_capture.exists():
        pump()
    assert url_capture.read_text() == "https://opentui.com/"
    ...
    wait_for(str(stream_cwd).encode(), seconds=30)
    send(b"\x17j")
    send(b"i")
    prompt = (
    ...
    stream_start = send(prompt.encode() + b"\r")
    wait_for(b"working", stream_start, 30)
    send(b"\x1b\x17kgg")
    send(b"\x17jiDRAFT_WHILE_STREAMING\x1b\x17k")
    ...
    working_at = output.find(b"working", stream_start)
    wait_for(b"idle", working_at + len(b"working"), 180)
    tail = send(b"G")
    ...
    print(json.dumps({"passed": True, "checks": checks, "terminal_bytes": len(output)}))
finally:
    if process.poll() is None:
        process.terminate()
        try:
