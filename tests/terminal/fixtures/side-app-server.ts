...
const create = (id: string, title: string) => ({ id, name: title, preview: "", model: "fixture-model", reasoningEffort: "high", cwd: process.cwd(), gitInfo: { branch: "side-tests", sha: null, originUrl: null }, status: { type: "idle" }, turns: [] })
...
const timer = setInterval(() => {
  ...
  item.text += delta
  notify("item/agentMessage/delta", { threadId: main.id, turnId: turn.id, itemId: item.id, delta })
}, 350)
for await (const line of createInterface({ input: process.stdin })) {
  ...
  switch (request.method) {
    ...
    case "thread/archive": threads.delete(request.params.threadId); result({}); break
    ...
    case "turn/start": {
      ...
      const text = thread.id === main.id ? "Main agent is working.\n" : "The main agent is progressing. This side conversation stays independent.\n"
      const message = { type: "agentMessage", id: `${id}-answer`, text, phase: "commentary", memoryCitation: null }
      const turn = { id, status: "inProgress", items: [{ type: "userMessage", id: `${id}-user`, content: request.params.input, clientId: null }, message], error: null }
      thread.turns.push(turn); thread.status = { type: "active", activeFlags: [] }
      result({ turn })
      notify("turn/started", { threadId: thread.id, turn: { ...turn, items: [] } })
      for (const item of turn.items) notify("item/started", { threadId: thread.id, turnId: id, item })
      break
    }
    case "turn/interrupt": {
      const thread = threads.get(request.params.threadId)
      const turn = thread.turns.find((turn: any) => turn.id === request.params.turnId)
      if (turn) { turn.status = "interrupted"; notify("turn/completed", { threadId: thread.id, turn }) }
      result({}); break
    }
