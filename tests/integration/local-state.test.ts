...
const saved: SavedThreadView = {
  draft: "👨‍👩‍👧‍👦 reply",
  cursorOffset: 3,
  folded: { message: true, gone: true },
  cursor: { itemId: itemId("message"), graphemeOffset: 99 },
  viewport: {
    kind: "point",
    point: { itemId: itemId("message"), graphemeOffset: 3 },
...
test("local view round-trips drafts and clamps positions to hydrated content", () => {
  const local = parseLocalState(
    JSON.parse(JSON.stringify({ version: 1, threads: { thread: saved } })),
  )
  const workspace = createWorkspace(id)
  workspace.transcript = syncTranscriptItem(workspace.transcript, {
    id: itemId("message"),
    turnId: turnId("turn"),
    kind: "assistant",
    markdown: "hello",
...
test("listing unloaded sessions preserves saved drafts", () => {
  const state = openThread(initialWorkbench(), {
    id,
    title: "thread",
    cwd: "/tmp",
    model: "model",
  ...
  expect(
    captureLocalState(state, { version: 1, threads: { thread: saved } }).threads
      .thread,
  ).toEqual(saved)
})
...
test("invalid state is rejected and missing historical anchors safely return to tail", () => {
  ...
  expect(() =>
    parseLocalState({
      version: 1,
      threads: { thread: { ...saved, cursorOffset: -1 } },
    }),
  ).toThrow()
  expect(
    restoreThreadView(createWorkspace(id), saved).transcript.viewport,
  ).toEqual({ kind: "tail" })
})
...
test("unacknowledged submissions survive restart as explicit failed retries", () => {
  ...
  const captured = parseLocalState(
  ...
  expect(restored.composer.outbox.map((message) => message.status)).toEqual([
    "failed",
    "failed",
    "failed",
  ])
...
test("version-one state without an outbox remains compatible", () => {
  ...
  expect(
    parseLocalState({ version: 1, threads: { thread: legacy } }).threads.thread
      ?.outbox,
  ).toEqual([])
})
...
test("explicitly unfolded items survive serialization and history restoration", () => {
  const local = parseLocalState({
    version: 1,
    threads: { thread: { ...saved, folded: { message: false } } },
  })
  const workspace = createWorkspace(id)
  workspace.transcript = syncTranscriptItem(workspace.transcript, {
    id: itemId("message"),
...
test("favorites persist even for unloaded threads and older state remains valid", () => {
  ...
  expect(
    parseLocalState({ version: 1, threads: {} }).favoriteThreadIds,
  ).toBeUndefined()
  expect(() =>
    parseLocalState({ version: 1, threads: {}, favoriteThreadIds: [123] }),
  ).toThrow()
})
...
test("marks and jumplists persist as source offsets and restore across Markdown reflow", () => {
  ...
  const location = {
    point: { itemId: itemId("message"), graphemeOffset: 8 },
    preferredScreenRow: 3,
  }
  workspace.transcript = {
    ...workspace.transcript,
    marks: { a: location },
    jumps: { back: [location], forward: [] },
  }
  const state = {
    ...openThread(initialWorkbench(), {
      id,
  ...
  resumed.transcript = syncTranscriptItem(resumed.transcript, {
    id: itemId("message"),
    turnId: turnId("turn"),
    kind: "assistant",
    markdown: "prefix **bold** and more",
    status: "complete",
  })
    ...
    parseLocalState({
      ...
      threads: {
        thread: {
          ...saved,
          marks: {
            bad: { itemId: "message", sourceOffset: -1, preferredScreenRow: 0 },
          },
        },
      },
    }),
