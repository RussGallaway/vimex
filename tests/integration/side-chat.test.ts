...
function harness(
  ...
  const backend: TestRuntime = {
    ...
    forkSideThread: async () => ({
      summary: summary(threadId(`side-${++forks}`)),
      events: [],
    }),
...
test("side transcript hides inherited context and ephemeral restart retires the side", async () => {
  ...
  h.backend.forkSideThread = async () => ({
  ...
  expect(
    h.controller.getSnapshot().workspaces[side]?.conversation.items[oldItem.id]
      ?.kind,
  ).toBe("assistant")
  h.emit({
    type: "conversation",
    event: {
      type: "item.completed",
      threadId: side,
      item: { ...oldItem, markdown: "Replayed parent" },
    },
  })
  ...
  h.backend.resumeThread = async (id) => ({
    summary: summary(id),
    events: id === side ? [...inheritedEvents, newEvent] : [],
  })
  ...
  legacy.backend.resumeThread = async (id) => ({
    summary: summary(id),
    events:
      id === side
        ? [...inheritedEvents, newEvent]
        : inheritedEvents.map((event) => ({ ...event, threadId: a })),
  })
  await legacy.controller.initialize("/tmp", undefined, a)
  legacy.controller.sideChat("open")
  await legacy.controller.settle()
  expect(
...
test("side chat forks while parent works; close preserves worker, drafts, and reopening", async () => {
  ...
  h.emit({
    type: "conversation",
    event: { type: "turn.started", threadId: a, turnId: turnId("main-turn") },
  })
...
test("side-by-side token streams publish only their owned runtime frames", async () => {
  ...
  await h.controller.settle()
  const side = threadId("side-1")
  const parentTurn = turnId("parent-publication"),
    sideTurn = turnId("side-publication")
  const parentItem = itemId("parent-publication"),
    sideItem = itemId("side-publication")
  h.emit({
    type: "conversation",
    event: { type: "turn.started", threadId: a, turnId: parentTurn },
  })
  h.emit({
    type: "conversation",
    event: {
  ...
  h.emit({
    type: "conversation",
    event: { type: "turn.started", threadId: side, turnId: sideTurn },
  })
  h.emit({
    type: "conversation",
    event: {
  ...
  const mainRuntime = h.controller.transcriptRuntime("main")!,
    sideRuntime = h.controller.transcriptRuntime("side")!
  let mainFrame = mainRuntime.getSnapshot(),
    sideFrame = sideRuntime.getSnapshot()
  const layout = h.controller.getLayoutSnapshot(),
    main = h.controller.getPresentationSnapshot("main"),
    sidePane = h.controller.getPresentationSnapshot("side")
  let layoutCalls = 0,
    mainCalls = 0,
    sideCalls = 0
  ...
  mainFrame = mainRuntime.getSnapshot()
  sideFrame = sideRuntime.getSnapshot()
  h.emit({
    type: "conversation",
    event: {
...
test("quit interrupts, archives, removes navigation targets, and next side forks anew", async () => {
  ...
  h.controller.openThread(threadId("side-1"))
  await h.controller.settle()
  expect(h.controller.getSnapshot().activeThreadId).toBe(a)
...
test("failed creation can retry and failed retirement leaves a usable side", async () => {
  ...
  h.controller.sideChat("quit")
  await h.controller.settle()
  expect(h.controller.getSnapshot().sideChats[a]?.threadId).toBe(
    threadId("side-1"),
  )
...
test("quit waits for an in-flight submission and suppresses queued follow-ups", async () => {
  ...
  h.controller.changeDraft("pending", 7)
  h.controller.submit("next-turn")
  h.controller.sideChat("quit")
  expect(calls).toEqual([])
  complete([
    { type: "turn.started", threadId: side, turnId: turnId("late-turn") },
  ])
  await h.controller.settle()
...
test("retired side tombstones survive local state and ignore late events", async () => {
  ...
  await restored.controller.initialize("/tmp")
  restored.emit({ type: "summary", summary: summary(threadId("side-1")) })
  restored.controller.openThread(threadId("side-1"))
  await restored.controller.settle()
...
test("side pane focus, maximize, refresh preserve drafts until restart", async () => {
  ...
  h.controller.sideChat("parent")
  await h.controller.settle()
  h.controller.changeDraft("parent draft", 3)
  h.controller.sideChat("cycle")
  await h.controller.settle()
  expect(h.controller.getSnapshot().activeThreadId).toBe(side)
...
test("side quote appends selected text to parent draft without submitting", async () => {
  ...
  h.controller.transcript({
    type: "cursor.move",
    target: { itemId: item, graphemeOffset: 0 },
  ...
  h.controller.transcript({
    type: "cursor.move",
    target: { itemId: item, graphemeOffset: 5 },
    preferredScreenRow: 0,
    extend: true,
  })
  h.controller.executeCommand(":side quote")
  await h.controller.settle()
  expect(h.controller.getSnapshot().activeThreadId).toBe(a)
  expect(h.controller.getSnapshot().workspaces[a]?.composer.text).toBe(
    "parent draft\n\n> Useful",
  )
  expect(h.controller.getSnapshot().workspaces[a]?.interaction.surface).toBe(
    "composer",
  )
  expect(h.starts).toEqual([])
  h.controller.executeCommand(":side focus side")
  await h.controller.settle()
  expect(h.controller.getSnapshot().activeThreadId).toBe(side)
  expect(h.starts).toEqual([])
  await h.controller.close()
...
test("side quote reads the detached side presentation instead of hidden canonical tail", async () => {
  ...
  h.emit({
    ...
    event: {
      ...
      item: {
        id: item,
        turnId: turnId("detached-quote-turn"),
        kind: "assistant",
        markdown: "visible",
        status: "running",
      },
  ...
  expect(
    h.controller.getSnapshot().workspaces[side]?.transcript.projectionById[item]
      ?.source,
  ).toBe("visible hidden")

  h.controller.executeCommand(":side quote", "side")
  await h.controller.settle()
  expect(h.controller.getSnapshot().activeThreadId).toBe(a)
  expect(h.controller.getSnapshot().workspaces[a]?.composer.text).toBe(
    "> visible",
  )
  expect(runtime.getSnapshot().transcript.projectionById[item]?.source).toBe(
    "visible",
  )
  await h.controller.close()
})
...
test("explicit main presentation reads target the parent while the side thread is active", async () => {
  const h = harness()
  await h.controller.initialize("/tmp")
  const parentItem = itemId("parent-read"),
    parentTurn = turnId("parent-read-turn")
  h.emit({
    type: "conversation",
    event: {
      type: "item.completed",
  ...
  expect(
    h.controller.getSnapshot().workspaces[a]?.interaction.unnamedRegister,
  ).toEqual({ text: "parent presentation", shape: "character" })
  expect(
    h.controller.getSnapshot().workspaces[side]?.interaction.unnamedRegister
      .text,
  ).toBe("")
  expect(h.controller.getSnapshot().workspaces[side]?.interaction.mode).toBe(
    "command",
  )
  expect(h.controller.getSnapshot().activeThreadId).toBe(side)
  await h.controller.close()
...
test("independent parents keep side forks isolated when creation completes out of order", async () => {
  ...
  expect(h.controller.getSnapshot().sideChats[b]?.threadId).toBe(
    threadId("side-1"),
  )
...
test("late retirement of one side does not dispose another parent's presentation runtime", async () => {
  ...
  h.backend.retireThread = (id) =>
    id === sideA
      ...
      : Promise.resolve()
  h.controller.sideChat("quit")
  for (let attempt = 0; attempt < 10 && !finishRetirement; attempt++)
    await Promise.resolve()
...
test("quit drains in-flight approval and question responses and blocks further work", async () => {
  ...
  await h.controller.settle()
  const side = threadId("side-1"),
    active = turnId("approved-turn")
  const calls: string[] = []
  let approve!: () => void, answer!: () => void
  h.backend.resolveApproval = async () => {
    calls.push("approve")
    await new Promise<void>((resolve) => {
      approve = resolve
    })
    h.emit({
      type: "conversation",
      event: { type: "turn.started", threadId: side, turnId: active },
    })
  }
  h.backend.respondToQuestions = async () => {
    calls.push("answer")
    await new Promise<void>((resolve) => {
      answer = resolve
    })
  ...
  for (const id of ["approval", "later-approval"])
    h.emit({
      type: "approval",
      approval: {
        id,
  ...
  for (const id of ["question", "later-question"])
    h.emit({
...
test("quitting cancels queued turns without consuming drafts and failed retirement allows explicit retry", async () => {
  const h = harness()
  await h.controller.initialize("/tmp")
  h.controller.sideChat("open", "first turn")
  await h.controller.settle()
  const side = threadId("side-1")
  ...
  const queued = h.controller
    .getSnapshot()
    .workspaces[side]?.composer.outbox.find(
      (message) => message.text === "queued follow-up",
    )
  expect(queued?.status).toBe("failed")
  expect(h.controller.getSnapshot().sideChats[a]?.status).toBeUndefined()
  h.controller.retryOutgoing(queued!.id)
  await h.controller.settle()
