...
The Stage 3 implementation now follows that boundary. `packages/transcript/src/geometry.ts` defines immutable renderer-neutral block geometry, and `TranscriptRuntime` owns the accepted geometry generation and revision. `transcript/measure-rendered-block.ts` contains OpenTUI-private text, Markdown, table, and diff inspection. `rendered-layout.ts` schedules dirty render blocks and composes native placements; `use-transcript-layout.ts` restores semantic anchors and exposes only geometry compatible with the current runtime frame. The UI has no second production geometry authority.
