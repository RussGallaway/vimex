...
Delegated assignments appear inline as compact foldable rows named for the child agent. Expanded rows show the assignment, reported result, and `gc` Open transcript action. Normal `gc` opens the exact child referenced by the current transcript row, or a chooser scoped to that row's children. Messages and follow-ups remain separate events. Telemetry-only lifecycle reports update the task row without adding transcript noise. While a wait call is active, the parent heartbeat says Waiting for agents; completing the wait returns to Working without implying that the children finished. Opening a child thread must preserve the parent's exact view state, and returning must restore it. Codex children that do not accept direct input are readable and navigable, with instructions sent through the parent.
